...
{
    public class CopiarInformacionHelper : HelperBase
    {
        public CopiarInformacionHelper() : base(Consultas.CopiarInformacionSql)
        {
        }
        ...
        public CopiarInformacionDTO Create(IDataReader dr)
        {
            CopiarInformacionDTO entity = new CopiarInformacionDTO();

            entity.IdCopia = Convert.ToInt32(valorReturn(dr, IdCopia) ?? 0);
            entity.GPSCodiOrigen = Convert.ToInt32(valorReturn(dr, GPSCodiOrigen) ?? 0);
            entity.GPSCodiDest = Convert.ToInt32(valorReturn(dr, GPSCodiDestino) ?? 0);
            entity.GPSDescOrigen = valorReturn(dr, GPSDescOrigen)?.ToString();
            entity.GPSDescDestino = valorReturn(dr, GPSDescDestino)?.ToString();
            entity.FecHoraInicio = valorReturn(dr, FecHoraInicio)?.ToString();
            entity.FecHoraFin = valorReturn(dr, FecHoraFin)?.ToString();
            entity.Motivo = valorReturn(dr, Motivo)?.ToString();
            entity.Estado = valorReturn(dr, Estado)?.ToString();
            entity.FecCrea = valorReturn(dr, FecCreacion)?.ToString();
            entity.UsuarioCreacion = valorReturn(dr, UsuCreacion)?.ToString();
            entity.FechaElimina = valorReturn(dr, FecElimina)?.ToString();
            entity.UsuarioElimina = valorReturn(dr, UsuElimina)?.ToString();
            return entity;
        }

        public LecturaDTO CreateLecturaOrigen(IDataReader dr)
        {
            ...
            entity.VSF = Convert.ToDecimal(valorReturn(dr, VSF) ?? 0);
            entity.Maximo = Convert.ToDecimal(valorReturn(dr, Maximo) ?? 0);
            entity.Minimo = Convert.ToDecimal(valorReturn(dr, Minimo) ?? 0);
            entity.Voltaje = Convert.ToDecimal(valorReturn(dr, Voltaje) ?? 0);
            entity.Num = Convert.ToInt32(valorReturn(dr, NUM) ?? 0);
            entity.Desv = Convert.ToDecimal(valorReturn(dr, DESV) ?? 0);
            ...
            entity.DevSec = Convert.ToDecimal(valorReturn(dr, DEVSEC) ?? 0);
        ...
        //CargaVirtual
        public string IdCopia = "IDCOPIA";
        public string GPSCodiOrigen = "GPSCODI_ORIGEN";
        public string GPSCodiDestino = "GPSCODI_DESTINO";
        public string GPSDescOrigen = "GPSDESC_ORIGEN";
        public string GPSDescDestino = "GPSDESC_DESTINO";
        public string FecHoraInicio = "FECHAHORA_INICIO";
        public string FecHoraFin = "FECHAHORA_FIN";
        ...
        public string FecElimina = "FECELIMINA";
        public string UsuElimina = "USUELIMINA";
        ...
        #region SQL
        public string SqlListaCopiarInformacion
        {
            get { return base.GetSqlXml("GetListaCopiarInformacion"); }
        }

        public string SqlListaLecturaOrigen
        {
            get { return base.GetSqlXml("GetListaLecturaOrigen"); }
        }
        ...
        public string SqlSaveCopiarInformacion
        {
            get { return GetSqlXml("SaveCopiarInformacion"); }
        }
        #endregion SQL
    }
}
