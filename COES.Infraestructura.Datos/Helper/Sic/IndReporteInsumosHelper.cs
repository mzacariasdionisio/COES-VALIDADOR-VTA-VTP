...
{
    /// <summary>
    /// Clase que contiene el mapeo de la tabla IND_REPORTE_INSUMOS
    /// </summary>
    public class IndReporteInsumosHelper : HelperBase
    {
        #region Mapeo de Campos
        public string Irpinscodi = "IRPINSCODI";
        public string Itotcodi = "ITOTCODI";
        public string Irpinstipo = "IRPINSTIPO";
        public string D1 = "D1";
        public string D2 = "D2";
        public string D3 = "D3";
        public string D4 = "D4";
        ...
        public string Irpinsusucreacion = "IRPINSUSUCREACION";
        public string Irpinsfeccreacion = "IRPINSFECCREACION";
        #endregion

        #region Constructor
        public IndReporteInsumosHelper() : base(Consultas.IndReporteInsumosSql)
        {
        }
        #endregion

        #region Create
        public IndReporteInsumosDTO Create(IDataReader dr)
        {
            IndReporteInsumosDTO entity = new IndReporteInsumosDTO();

            int iIrpinscodi = dr.GetOrdinal(this.Irpinscodi);
            if (!dr.IsDBNull(iIrpinscodi)) entity.Irpinscodi = Convert.ToInt32(dr.GetValue(iIrpinscodi));

            int iItotcodi = dr.GetOrdinal(this.Itotcodi);
            if (!dr.IsDBNull(iItotcodi)) entity.Itotcodi = Convert.ToInt32(dr.GetValue(iItotcodi));

            int iIrpinstipo = dr.GetOrdinal(this.Irpinstipo);
            if (!dr.IsDBNull(iIrpinstipo)) entity.Irpinstipo = Convert.ToInt32(dr.GetValue(iIrpinstipo));
            ...
            int iIrpinsusucreacion = dr.GetOrdinal(this.Irpinsusucreacion);
            if (!dr.IsDBNull(iIrpinsusucreacion)) entity.Irpinsusucreacion = dr.GetString(iIrpinsusucreacion);

            int iIrpinsfeccreacion = dr.GetOrdinal(this.Irpinsfeccreacion);
            if (!dr.IsDBNull(iIrpinsfeccreacion)) entity.Irpinsfeccreacion = dr.GetDateTime(iIrpinsfeccreacion);
