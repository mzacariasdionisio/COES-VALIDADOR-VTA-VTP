...
{
    /// <summary>
    /// Clase que contiene el mapeo de la tabla WB_RESUMENMD
    /// </summary>
    public class WbResumenmdHelper : HelperBase
    {
        public WbResumenmdHelper() : base(Consultas.WbResumenmdSql)
        {
        }

        public WbResumenmdDTO Create(IDataReader dr)
        {
            ...
            int iResmdfecha = dr.GetOrdinal(this.Resmdfecha);
            if (!dr.IsDBNull(iResmdfecha)) entity.Resmdfecha = dr.GetDateTime(iResmdfecha);

            int iResmdhora = dr.GetOrdinal(this.Resmdhora);
            if (!dr.IsDBNull(iResmdhora)) entity.Resmdhora = dr.GetDateTime(iResmdhora);

            int iResmdvalor = dr.GetOrdinal(this.Resmdvalor);
            if (!dr.IsDBNull(iResmdvalor)) entity.Resmdvalor = dr.GetDecimal(iResmdvalor);

            int iResmdmes = dr.GetOrdinal(this.Resmdmes);
            if (!dr.IsDBNull(iResmdmes)) entity.Resmdmes = dr.GetDateTime(iResmdmes);
        ...
        public string Resmdfecha = "RESMDFECHA";
        public string Resmdhora = "RESMDHORA";
        public string Resmdvalor = "RESMDVALOR";
        public string Resmdmes = "RESMDMES";
        ...
        public string SqlGetMaxMdvalor
        {
            get { return base.GetSqlXml("GetMaxMdvalor"); }
        }
    }
}
