...
{
    public class CamCCGDCOptRepository : RepositoryBase, ICamCCGDCOptRepository
    {

        public CamCCGDCOptRepository(string strConn) : base(strConn) { }
        CamCCGDCOptHelper Helper = new CamCCGDCOptHelper();

        public List<CCGDCOptDTO> GetCamCCGDC(int proyCodi)
        {
            List<CCGDCOptDTO> ccgdcDTOs = new List<CCGDCOptDTO>();
            ...
            using (IDataReader dr = dbProvider.ExecuteReader(command))
            {
                while (dr.Read())
                {
                    CCGDCOptDTO ob = new CCGDCOptDTO();
                    ob.CcgdcOptCodi = !dr.IsDBNull(dr.GetOrdinal("CCGDCOPTCODI")) ? dr.GetInt32(dr.GetOrdinal("CCGDCOPTCODI")) : 0;
        ...
        public bool SaveCamCCGDC(CCGDCOptDTO ccgdcDTO)
        {
            DbCommand dbCommand = dbProvider.GetSqlStringCommand(Helper.SqlSaveCamCCGDC);
            dbProvider.AddInParameter(dbCommand, "CCGDCOPTCODI", DbType.Int32, ccgdcDTO.CcgdcOptCodi);
        ...
        public List<CCGDCOptDTO> GetCamCCGDCById(int id)
        {
            List<CCGDCOptDTO> lista = new List<CCGDCOptDTO>();
        ...
        public bool UpdateCamCCGDC(CCGDCOptDTO ccgdcDTO)
        {
        ...
        public List<CCGDCOptDTO> GetCamCCGDCByFilter(string plancodi, string empresa, string estado)
        {
            List<CCGDCOptDTO> lista = new List<CCGDCOptDTO>();
            string query = $@"
                SELECT CGB.*, TR.EMPRESANOM, TR.PROYNOMBRE, TR.PROYDESCRIPCION, TP.TIPONOMBRE, TF.TIPOFINOMBRE,TR.PROYCONFIDENCIAL  FROM CAM_CCGDCOPT CGB
                ...
                ORDER BY TR.PERICODI, CGB.PROYCODI,PL.CODEMPRESA, CGB.CCGDCOPTCODI ASC";
