...
var headersAlterno = ['', '', 'Correlativo por \n Punto de Entrega', 'Tipo de Cliente', 'Cliente', 'Punto de Entrega / \n Barra', 'N° de suministro \n cliente libre', 'Nivel de Tensión', 'Aplicación literal \n e) de numeral 5.2.4 \n Base Metodológica \n (meses de suministro en el semestre)', 'Energía Semestral(kWh)', 'Incremento de tolerancias \n Sector Distribución Típico \n 2(Mercado regulado)', 'Tipo', 'Causa', 'Ni', 'Ki', 'Tiempo Ejecutado - Inicio', 'Tiempo Ejecutado - Fin', 'Tiempo Programado - Inicio', 'Tiempo Programado - Fin', 'Responsable 1', '% Responsable 1', 'Responsable 2', '% Responsable 2', 'Responsable 3', '% Responsable 3', 'Responsable 4', '% Responsable 4', 'Responsable 5', '% Responsable 5', 'Causa resumida de interrupción', 'Ei / E', 'Resarcimiento(US$)', 'Evidencia',
    ...
    'Conformidad Responsable 5', 'Observación Responsable 5', ' Detalle de campo observado Responsable 5', 'Comentarios Responsable 5', 'Sustento Responsable 5', 'Conformidad Suministrador - Responsable 5', 'Comentarios Suministrador - Responsable 5', 'Sustento Suministrador - Responsable 5'];
...
function cargarGrillaInterrupcion(result) {
    ...
    for (var k = 33; k <= 72; k++) {
        camposTabla.push({ id: k, required: false });
    }
    ...
    var merge = [
        ...
        { row: 0, col: 33, rowspan: 1, colspan: 5 },
        { row: 0, col: 38, rowspan: 1, colspan: 3 },
        { row: 0, col: 41, rowspan: 1, colspan: 5 },
        { row: 0, col: 46, rowspan: 1, colspan: 3 },
        { row: 0, col: 49, rowspan: 1, colspan: 5 },
        { row: 0, col: 54, rowspan: 1, colspan: 3 },
        { row: 0, col: 57, rowspan: 1, colspan: 5 },
        { row: 0, col: 62, rowspan: 1, colspan: 3 },
        { row: 0, col: 65, rowspan: 1, colspan: 5 },
        { row: 0, col: 70, rowspan: 1, colspan: 3 }
    ...
    hot = new Handsontable(grilla, {
        ...
        cells: function (row, col, prop) {
            ...
            if (row < 4) {
                ...
                if ((col >= 33 && col <= 37) || (col >= 41 && col <= 45) || (col >= 49 && col <= 53) || (col >= 57 && col <= 61) || (col >= 65 && col <= 69)) {
            ...
            if (row > 3) {
                ...
                if (this.instance.getDataAtCell(row, 0).split("-")[1] == "S") {
                    trimestral = true;
                }


                //- Eliminar interrupcion
                if (col == 1 && trimestral == false) {
                    cellProperties.renderer = deleteRenderer;
                    cellProperties.readOnly = false;
                }
                else if (col == 1 && trimestral == true) {
                ...
                if (col == 2) {
                    ...
                    if (trimestral == true) {
                        this.instance.setCellMeta(row, col, 'className', 'yellow-trimestral');
                        cellProperties.readOnly = true;
                    }
                }
                ...
                if (col == 32) {
                    if (this.instance.getDataAtCell(row, col) != null && this.instance.getDataAtCell(row, col) != "") {
                        ...
                        if (trimestral == true) {
                            cellProperties.renderer = openFileTrimestralRenderer;
                            cellProperties.readOnly = false;
                        }
                    }
                    else {
                ...
                if ((col >= 33 && col <= 36) || (col >= 41 && col <= 44) || (col >= 49 && col <= 52) || (col >= 57 && col <= 60) || (col >= 65 && col <= 69)) {
                    cellProperties.renderer = disabledRenderer;
                    cellProperties.readOnly = true;
                ...
                if (col == 37 || col == 45 || col == 53 || col == 61 || col == 69) {
                    if (this.instance.getDataAtCell(row, col) != null && this.instance.getDataAtCell(row, col) != "") {
                ...
                //- Conformidad suministrador
                if (col == 38 || col == 46 || col == 54 || col == 62 || col == 70) {
                    cellProperties.editor = 'select2';
                    cellProperties.renderer = dropdownSiNoRenderer;
                    cellProperties.select2Options = {
                        data: arregloSiNo,
                ...
                //- Comentario suministrador
                if (col == 39 || col == 47 || col == 55 || col == 63 || col == 71) {
                    if (validarTexto(this.instance.getDataAtCell(row, col), 300)) {
                        cellProperties.renderer = defaultRenderer;
                        cellProperties.readOnly = false;
                    }
                ...
                if (col == 40 || col == 48 || col == 56 || col == 64 || col == 72) {
                    ...
                    if (this.instance.getDataAtCell(row, col) != null && this.instance.getDataAtCell(row, col) != "") {
                        ...
                        if (trimestral == true) {
                            cellProperties.renderer = showFileSuministradorRenderer1;
                            cellProperties.readOnly = false;
                        }
                    }
                    else {
                ...
                //- Deshabilitamos los campos en caso no haya observaciones
                if (col == 38 || col == 39 || col == 40) {
                    if (this.instance.getDataAtCell(row, 33) == null || this.instance.getDataAtCell(row, 33) == "" || this.instance.getDataAtCell(row, 33) == "Si") {
                        cellProperties.renderer = disabledRenderer;
                        cellProperties.readOnly = true;
                ...
                if (col == 46 || col == 47 || col == 48) {
                    if (this.instance.getDataAtCell(row, 41) == null || this.instance.getDataAtCell(row, 41) == "" || this.instance.getDataAtCell(row, 41) == "Si") {
                        cellProperties.renderer = disabledRenderer;
                        cellProperties.readOnly = true;
                ...
                if (col == 54 || col == 55 || col == 56) {
                    if (this.instance.getDataAtCell(row, 49) == null || this.instance.getDataAtCell(row, 49) == "" || this.instance.getDataAtCell(row, 49) == "Si") {
                        cellProperties.renderer = disabledRenderer;
                        cellProperties.readOnly = true;
                ...
                if (col == 62 || col == 63 || col == 64) {
                    if (this.instance.getDataAtCell(row, 57) == null || this.instance.getDataAtCell(row, 57) == "" || this.instance.getDataAtCell(row, 57) == "Si") {
                        cellProperties.renderer = disabledRenderer;
                        cellProperties.readOnly = true;
                ...
                if (col == 70 || col == 71 || col == 72) {
                    if (this.instance.getDataAtCell(row, 65) == null || this.instance.getDataAtCell(row, 65) == "" || this.instance.getDataAtCell(row, 65) == "Si") {
                        cellProperties.renderer = disabledRenderer;
                        cellProperties.readOnly = true;
...
var deleteRenderer = function (instance, td, row, col, prop, value, cellProperties) {
    ...
    $(div).on('mouseup', function () {
        var id = hot.getData()[row][col - 1].split("-")[0];
...
var cargarFileRenderer = function (instance, td, row, col, prop, value, cellProperties) {
    ...
    var idInterrupcion = hot.getData()[row][0].split("-")[0];
    var idInterrupcionDetalle = -1;
    if (col > 32) idInterrupcionDetalle = col / 8 - 4;

    $(div).on('mouseup', function () {
        showFile(idInterrupcion, idInterrupcionDetalle, row, col, hot.getData()[row][col]);
    });
    $(td).addClass("estilofolder");
    return td;
}

var openFileTrimestralRenderer = function (instance, td, row, col, prop, value, cellProperties) {
    var div;
    $(td).children('.btn').remove();
    $(td).html('');
    div = document.createElement('div');
    ...
    td.appendChild(div);
    $(div).on('mouseup', function () {
        showFileInterrupcionDetalle(hot.getData()[row][0].split("-")[0], row, -1, hot.getData()[row][col]);
    });
...
var showFileRenderer = function (instance, td, row, col, prop, value, cellProperties) {
    ...
    $(div).on('mouseup', function () {
        var idInterrupcionDetalle = -1;
        if (col > 32) idInterrupcionDetalle = (col + 3) / 8 - 4;
        //if (col == 37 || col == 45 || col == 53 || col == 61 || col == 69) {
        showFileInterrupcion(hot.getData()[row][0].split("-")[0], row, idInterrupcionDetalle, hot.getData()[row][col]);
    });
    $(td).addClass("openfolder");
    return td;
}
...
var showFileSuministradorRenderer1 = function (instance, td, row, col, prop, value, cellProperties) {
    var div;
    $(td).children('.btn').remove();
    $(td).html('');
    div = document.createElement('div');
    ...
    $(div).on('mouseup', function () {
        ...
        if (col > 32) idInterrupcionDetalle = col / 8 - 4;
        //if (col == 37 || col == 45 || col == 53 || col == 61 || col == 69) {
        showFileInterrupcionDetalle(hot.getData()[row][0].split("-")[0], row, idInterrupcionDetalle, hot.getData()[row][col]);
    });
    $(td).addClass("openfolder");
    return td;
}
...
function validarDatos() {
    ...
    for (var i = 4; i < data.length; i++) {
        ...
        if (true) {
            ...
            var validacionSustento = true;

            //- Conformidad suministrador obligatorio cuando conformidad responsable es no
            for (var m = 0; m < 5; m++) {
                if (data[i][33 + m * 8] == "No" && data[i][38 + m * 8] == "") {
                    validaciones.push({ row: i, col: 38 + m * 8, validation: Validacion.ConformidadSuministradorObligatorio });
                }

                //- En caso observaciones FM o EXONERACION                
                if (
                    data[i][34 + m * 8].includes("En trámite de FM o FM Fundada") ||
                    data[i][34 + m * 8].includes("Exonerado por Expansión o Reforzamiento")
                ) {
                    validacionSustento = false;
                }
            }

            //- Comentarios suministrador obligatorio cuando conformidad suministrador es no
            for (var m = 0; m < 5; m++) {
                if (data[i][38 + m * 8] == "N" && data[i][39 + m * 8] == "") {
                    validaciones.push({ row: i, col: 39 + m * 8, validation: Validacion.ComentarioSuministradorObligatorio });
                }
                if (data[i][39 + m * 8] != "" && data[i][39 + m * 8].length > 300) {
                    validaciones.push({ row: i, col: 39 + m * 8, validation: Validacion.ComentarioSuministradorMaximo });
                }
            }
            ...
            if (data[i][12] != "") {
                if (parseInt(data[i][12]) == 7 || parseInt(data[i][12]) == 8 || parseInt(data[i][12]) == 4) {
                    if ((data[i][32] == null || data[i][32] == "") && validacionSustento) {
                        validaciones.push({ row: i, col: 32, validation: Validacion.ArchivoSustentoObligatorio });
                    }
                }
            }
...
function actualizarFile(row, idColumna, extension) {
    var col = (parseInt(idColumna) == -1) ? 32 : (parseInt(idColumna) + 4) * 8;
    console.log(idColumna);
    hot.setDataAtCell(row, col, extension);
    hot.render();
}
...
function obtenerDatosAnulacion(idInterrupcion) {
    ...
    for (var i = 4; i < data.length; i++) {       
        if (data[i][0].split('-')[0] == idInterrupcion) {
            cliente = data[i][4];
            ptoEntrega = data[i][5];
            break;
        }
    ...
    for (var i = 4; i < data.length; i++) {
        if (
            data[i][4] == cliente &&
            data[i][5] == ptoEntrega &&
            data[i][0].split('-')[0] != idInterrupcion
        ) {
            data[i][1] = i;
            arreglo.push(data[i]);
        }
