...
using COES.MVC.Extranet.Areas.CalculoResarcimiento.Models;
...
namespace COES.MVC.Extranet.Areas.CalculoResarcimiento.Controllers
{
    public class MedicionController : BaseController
    {
        ...
        /// </summary>
        /// <returns></returns>
        public ActionResult Index(int id, int empresa)
        {
            ...
            model.AnioMes = COES.Base.Tools.Util.ObtenerNombreMes((int)model.Entidad.Reevprmes) + " " + model.Entidad.Reevpranio;
            model.Suministrador = (new EmpresaAppServicio()).ObtenerEmpresa(empresa).Emprnomb;
            model.IdEmpresa = empresa;            
            model.Acceso = model.Entidad.Reevpracceso;
            return View(model);
        }
