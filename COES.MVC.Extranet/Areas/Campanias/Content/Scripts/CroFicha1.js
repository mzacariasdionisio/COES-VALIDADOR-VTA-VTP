...
var hotCroFicha1 = null;
var valoresListaC = [];
var catRequisitoCroFicha1 = 43;

$(function () {
    cargarCatalogoTablaCroFicha1(catRequisitoCroFicha1);
    ...
    var formulariocro = document.getElementById('FichaT3A');
    $(formulariocro).off('change').on('change', function () {
        cambiosRealizados = true;
    });
});


getDataCroFicha1 = function () {
    var param = {};
    param.Fecpuestaope = $("#txtFechaPuertaOperacion").val();
    return param;
}
function getDataDetCroFicha1() {
    var hotInstance = hotCroFicha1.getInstance(); // Obtener la instancia de Handsontable
...
function cargarCatalogoTablaCroFicha1(id) {
    $.ajax({
        type: 'POST',
        ...
        success: function (eData) {
            valoresListaC = eData;
            tablaCroFicha1(anioPeriodo, horizonteFin);
        },
        error: function (err) {
...
function tablaCroFicha1(inicioan, finan) {
    ...
    var grilla = document.getElementById("tablaRequisitoCroFicha1");
    console.log("grilla:", grilla);

    if (typeof hotCroFicha1 !== "undefined" && hotCroFicha1 !== null) {
        hotCroFicha1.destroy();
    }

    hotCroFicha1 = new Handsontable(grilla, {
        data: data,
        // rowHeaders: true,
        colHeaders: true,
...
function grabarCroFicha1() {
    var param = {};
    param.TransmisionProyectoDTO = getCabeceraProyecto();
    var idProyecto = $("#txtPoyCodi").val();
    if (idProyecto != "0") {
        param.ProyCodi = idProyecto;
        param.CroFicha1DTO = getDataCroFicha1();
        param.ListaCroFicha1DetDTO = getDataDetCroFicha1();
        $.ajax({
            type: 'POST',
            url: controlador + 'GrabarCroFicha1',
            data: param,
...
function cargarCroFicha1() {
    console.log("Editar");
    if (modoModel == "editar" || modoModel == "consultar") {
        var idProyecto = $("#txtPoyCodi").val();
        $.ajax({
            type: 'POST',
            url: controladorFichas + 'GetCroFicha1',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ id: idProyecto }),
            
            success: function (response) {
                console.log(response);
                var CroFicha1Res = response.responseResult;
                setDataCroFicha1(CroFicha1Res);
                setDataDetCroFicha1(CroFicha1Res.ListaCroFicha1DetDTO);
            },
            error: function () {
...
setDataCroFicha1 = function (dataResponse) {
    fechacro = convertirFechaMesAnio(dataResponse.Fecpuestaope);
    setFechaPickerGlobal("#txtFechaPuertaOperacion", fechacro, "mm/aaaa");
    if (modoModel == "consultar") {
        desactivarCamposFormulario('FichaT3A');
        $("#btnGrabarTCro").hide();
        hotCroFicha1.updateSettings({
...
function setDataDetCroFicha1(datosSeteo) {
    var hotInstance = hotCroFicha1.getInstance(); // Obtener la instancia de Handsontable
    var data = hotInstance.getData(); // Obtener los datos actuales de la tabla
