...
const configuracionColumnasD1A4 = {

    1: { tipo: "decimal", decimales: 2 }
...
$(function () {
    ...
    var formularioad = document.getElementById('DGPFormatoD1A');
    $(formularioad).off('change').on('change', function () {
        cambiosRealizados = true;
    });
});
...
function EnergiaColumnRenderer(instance, td, row) {
    ...
    let total = 0;
    let energiaDemanda = parseFloat(instance.getDataAtCell(row, 1)) || null;
    let energiaGeneracion = parseFloat(instance.getDataAtCell(row, 5)) || null;
...
function crearTableInversEstim(inicioan, finan) {
    ...
    hot4InversEstim = new Handsontable(grilla, {
        ...
        beforeChange: function (changes, source) {
            if (["edit", "paste", "autofill"].includes(source)) {
                return generalBeforeChange_4(this, changes, configuracionColumnasD1A4);
            }
        },
        afterChange: function (changes, source) {
            if (["edit", "paste", "autofill"].includes(source)) {
                generalAfterChange_global(this, changes, configuracionColumnasD1A4);
            }
        }
    });
...
function grabarDForm() {
    ...
    //if (!validarTablaAntesDeGuardar2(hotPeMedio,'Medio')) {
    //    mostrarNotificacion("No se puede guardar porque hay datos inválidos en la tabla.");
    //    return; // Detener el guardado si hay errores
    //}
    //if (!validarTablaAntesDeGuardar2(hot2PeOptimista,'Optimista')) {
    //    mostrarNotificacion("No se puede guardar porque hay datos inválidos en la tabla.");
    //    return; // Detener el guardado si hay errores
    //}

    //if (!validarTablaAntesDeGuardar2(hot3PePesimista,'Pesimista')) {
    //    mostrarNotificacion("No se puede guardar porque hay datos inválidos en la tabla.");
    //    return; // Detener el guardado si hay errores
    //}
    if (!validarTablaAntesDeGuardar2(hot4InversEstim, 'Inv/Estimada')) {
        mostrarNotificacion("No se puede guardar porque hay datos inválidos en la tabla.");
        return; // Detener el guardado si hay errores
    }
...
function setDataDFormatoDet4(listaDet1, inicioan, finan) {
    ...
    for (var year = inicioan; year <= finan; year++) {
        if (!añosPresentes.has(year)) {
            listaDet1.push({ Anio: year, MontoInversion: null });
        }
    }
...
function importD1ADet4() {
    ...
    fileInput.onchange = function (e) {
        ...
        reader.onload = function (e) {
            ...
            updateTableFromExcelD14(jsonData);
            cambiosRealizados = true;
        };
...
function updateTableFromExcelD14(jsonData) {
    const headers = jsonData[0]; // Cabecera
    let datosImportados = jsonData.slice(1); // Datos desde la segunda fila

    const datosActuales = hot4InversEstim.getData();
    const totalFilas = datosActuales.length;
    const totalColumnas = datosActuales[0]?.length || 0;

    // 🔹 Truncar filas sobrantes
    if (datosImportados.length > totalFilas) {
        datosImportados = datosImportados.slice(0, totalFilas);
    }

    // 🔹 Truncar columnas sobrantes
    datosImportados = datosImportados.map(row => row.slice(0, totalColumnas));
    
    // 🔹 Convertir numéricos a 2 decimales reales
    datosImportados = datosImportados.map(row =>
        row.map(value =>
    ...
    // 🔹 Fusionar con datos actuales (conservando columna 0)
    const nuevaData = datosActuales.map((filaActual, index) => {
        const filaNueva = [...filaActual];

        if (index < datosImportados.length) {
            const filaImportada = datosImportados[index];
            for (let col = 1; col < filaImportada.length && col < filaNueva.length; col++) {
                filaNueva[col] = filaImportada[col];
            }
        }
    ...
    // 🔹 Validar y cargar
    const datosValidados = validarYFormatearDatos2(nuevaData, configuracionColumnasD1A4, mostrarNotificacion, hot4InversEstim);
    hot4InversEstim.loadData(datosValidados);
    ...
    datosValidados.forEach((row, rowIndex) => {
        row.forEach((value, colIndex) => {
            ...
            const config = configuracionColumnasD1A4[colIndex];
            if (config && value !== null && value !== "" && value !== undefined) {
                let parsedValue = parseFloat(value);
                let esValido = true;

                if (isNaN(parsedValue)) esValido = false;
                else if (!config.permitirNegativo && parsedValue < 0) esValido = false;
                else if (config.tipo === "decimal" && value.toString().split(".")[1]?.length > config.decimales) esValido = false;
                else if (config.tipo === "entero" && !Number.isInteger(parsedValue)) esValido = false;

                if (!esValido) {
                    celdasConErrores.push(`Fila ${rowIndex + 1}, Columna ${colIndex + 1}`);
                    hot4InversEstim.setCellMeta(rowIndex, colIndex, "valid", false);
                    hot4InversEstim.setCellMeta(rowIndex, colIndex, "style", { backgroundColor: "#ffcccc" });
                } else {
                    hot4InversEstim.setCellMeta(rowIndex, colIndex, "valid", true);
                    hot4InversEstim.setCellMeta(rowIndex, colIndex, "style", { backgroundColor: "" });
                }
            }
        });
    });
    ...
    if (celdasConErrores.length > 0) {
        mostrarNotificacion(`Se importaron datos con errores.<br>Errores detectados en:<br>${celdasConErrores.join("<br>")}`);
    } else {
