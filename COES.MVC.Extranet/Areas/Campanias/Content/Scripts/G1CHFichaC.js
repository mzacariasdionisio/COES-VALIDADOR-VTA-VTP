...
$(function () {
    ...
    var formularioc = document.getElementById('HFichaC');
    $(formularioc).off('change').on('change', function () {
...
function setDataDetHojaC(datosSeteo) {
    var hotInstance = hotTableHojaC.getInstance(); // Obtener la instancia de Handsontable
...
setDataHojaC = function (dataResponse) {
    ...
    $("#txtFechaPuertaOperacion").val(fechaConvertida);
    if (modoModel == "consultar") {
        desactivarCamposFormulario('HFichaC');
        $("#btnGrabarFichaC").hide();
        hotTableHojaC.updateSettings({
...
function tablaHojaCHan(inicioan, finan) {
    ...
    if (typeof hotTableHojaC !== "undefined" && hotTableHojaC !== null) {
        hotTableHojaC.destroy();
    }

    hotTableHojaC = new Handsontable(grilla, {
        data: data,
        // rowHeaders: true,
        colHeaders: true,
