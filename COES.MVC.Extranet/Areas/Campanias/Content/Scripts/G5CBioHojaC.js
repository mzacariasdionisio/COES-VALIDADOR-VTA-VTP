...
function inicializar() {
   ...
   // $('#txtFechaPuertaOperacion').val(obtenerFechaActualMesAnio());
    ...
    $('#btnGrabarBioHojaC').on('click', function () {
        grabarBioHojaC();
    });
    var formularioaC = document.getElementById('BFichaC');
    formularioaC.addEventListener('change', function () {
        cambiosRealizados = true;
    });
}

getDataHojaCBio = function () {
    var param = {};
...
function cargarCatalogoTablaFichaC(id) {
    $.ajax({
        ...
        success: function (eData) {
            valoresListaC = eData;
            tablaHojaCBio(anioPeriodo, horizonteFin);
        },
        error: function (err) {
...
function tablaHojaCBio(inicioan, finan) {
    ...
    var grilla = document.getElementById("detalleFormatoHojaBioC");
    console.log("grilla:", grilla);

    if (typeof hotTableHojaBioC !== "undefined" && hotTableHojaBioC !== null) {
        hotTableHojaBioC.destroy();
    }

    hotTableHojaBioC = new Handsontable(grilla, {
        data: data,
        // rowHeaders: true,
        colHeaders: true,
    ...
    hotTableHojaBioC.render();
}

function grabarBioHojaC() {
    console.log("result");
    var param = {};
    ...
    if (idProyecto != "0") {
        ...
        param.BioHojaCDTO = getDataHojaCBio();
        param.ListaDetBioHojaCDTO = getDataDetHojaBioC();
        console.log(param);
        $.ajax({
            type: 'POST',
            url: controlador + 'GrabarBioHojaC',
            data: param,
...
function getDataDetHojaBioC() {
    var hotInstance = hotTableHojaBioC.getInstance(); // Obtener la instancia de Handsontable
...
function cargarDatosC() {
    ...
    if (modoModel == "editar" || modoModel == "consultar") {
        ...
        $.ajax({
            type: 'POST',
            url: controladorFichas + 'GetBioHojaC',
            contentType: "application/json; charset=utf-8",
            ...
            success: function (response) {
                ...
                setDataDetHojaC(hojaCRes.ListaDetBioHojaCDTO);
            },
            error: function () {
...
setDataHojaC = function (dataResponse) {
    fechaco = convertirFechaMesAnio(dataResponse.Fecpuestaope);
    setFechaPickerGlobal("#txtFechaPuertaOperacion", fechaco, "mm/aaaa");
    if (modoModel == "consultar") {
        desactivarCamposFormulario('BFichaC');
        $("#btnGrabarBioHojaC").hide();
        hotTableHojaBioC.updateSettings({
