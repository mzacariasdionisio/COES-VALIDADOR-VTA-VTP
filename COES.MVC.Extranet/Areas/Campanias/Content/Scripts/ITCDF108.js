...
$(function () {
    ...
    var formulariosa = document.getElementById('tabla108');
...
function grabarItcdf108() {
    if (!validarTablaAntesDeGuardar(hot108)) {
    ...
    if (idProyecto != "0") {
        ...
        param.Itcdf108DTOs = getDatos108();
        $.ajax({
            type: 'POST',
            url: controlador + 'GrabarItcdf108',
            data: param,
...
function setDatos108(lista108) {
    ...
    if (modoModel == "consultar") {
        desactivarCamposFormulario('f108');
        $('#btnGrabarFicha108').hide();
        $('#btnImportarExcel108').hide();
