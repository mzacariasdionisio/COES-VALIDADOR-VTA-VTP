...
// Función para formatear un número a 4 decimales (o devolver cadena vacía si es null/undefined)
            ...
            if (newValue !== null && newValue !== "") {
                ...
                if (config.tipo === "texto") {
                    if (newValue.length > config.largo) {
                        ...
                        cellMeta.style = { backgroundColor: "#ffcccc" };
                    ...
                    newValue = parsedValue.toFixed(decimales);
                    ...
                    const truncDecimales = config.decimales !== undefined ? config.decimales : 4;
                    ...
                    // Validar que solo sean dígitos numéricos enteros (positivos o negativos)
                    ...
                    if (!Number.isInteger(parsedValue) || stringValue.length !== 4) {
                        cellMeta.valid = false;
                        cellMeta.style = { backgroundColor: "#ffcccc" };
                        mostrarNotificacion(`Error: El año debe ser un número entero de 4 dígitos en la fila ${row + 1}, columna ${col + 1}.`);
                    } else {
                        cellMeta.valid = true;
                        cellMeta.style = { backgroundColor: "" };
                    }
...
function validarTablaAntesDeGuardar2(hotInstance, opcional = '') {
    const data = hotInstance.getData();
    let totalErrores = 0;
    let celdasConErrores = new Set(); // Usamos Set para evitar duplicados

    for (let row = 0; row < data.length; row++) {
        for (let col = 0; col < data[row].length; col++) {
            const valorCelda = data[row][col];
            const cellMeta = hotInstance.getCellMeta(row, col);
            const celdaID = `Fila ${row + 1}, Columna ${col + 1}`;

            let tieneError = false;

            // Validar si la celda no es un número ni está vacía
            if (valorCelda !== '' && isNaN(valorCelda)) {
                tieneError = true;
            }

            // Validar si la celda ya tiene un error registrado en metadata
            if (cellMeta && cellMeta.valid === false) {
                tieneError = true;
            }
            ...
            if (tieneError) {
                totalErrores++;
                celdasConErrores.add(celdaID); // Se agrega solo una vez
                hotInstance.setCellMeta(row, col, "style", { backgroundColor: "#ffcccc" });
            }
        }
    }
    ...
    if (totalErrores > 0) {
        mostrarMensaje('mensajeFicha', 'error', 'Error al guardar. Corrija los errores. Si cambia de pestaña, los datos se perderán.', true);
        const mensajeErrores = `
Total de errores: ${totalErrores}${opcional ? ` (${opcional})` : ''}.

Mostrando las primeras 10:
${[...celdasConErrores].slice(0, 10).join("\n")}`; // Convertir Set a Array y mostrar solo 10
                ...
                if (config.tipo === "texto") {
                    ...
                    if (!/^-?\d+$/.test(stringValue)) {
                        ...
                        if (!Number.isInteger(parsedValue)) {
                            ...
                            row[colIndex] = parsedValue.toString(); // Convertir a string sin decimales
                        }
                    }
                } else if (config.tipo === "especial") {
        ...
        changes.forEach(function ([row, prop, oldValue, newValue]) {
            ...
            if ([1, 2,3, 5].includes(prop)) {
                ...
                const potenciaHFP = parseFloat(rowData[3]) || 0;
                    const energiaD = parseFloat(rowData[1]) || 0;
                    const energiaG = parseFloat(rowData[5]) || 0;
                    const factorCarga2 = energiaD - energiaG;
                ...
                hotInstance.setDataAtCell(row, 8, factorCarga2.toFixed(4), 'auto');
                // Validar el rango del Factor de Carga
                if (factorCarga < 0 || factorCarga > 1) {
                    hotInstance.setCellMeta(row, 4, 'valid', false);
                    hotInstance.setCellMeta(row, 4, 'className', 'htInvalid');
                    mostrarNotificacion(
...
function aplicarEstilos(ws, hotInstance, headerRows = 3) {
    const rowCount = hotInstance.countRows() + headerRows;
    const colCount = hotInstance.countCols();

    // Definir estilo general con fondo gris y bordes
    const cellStyle = {
        fill: { fgColor: { rgb: "D3D3D3" } }, // Fondo gris
        border: {
    ...
    // Aplicar a todas las celdas del archivo
    for (let r = 0; r < rowCount; r++) {
        for (let c = 0; c < colCount; c++) {
            let cellAddress = XLSX.utils.encode_cell({ r: r, c: c });
            if (!ws[cellAddress]) ws[cellAddress] = { v: "" }; // Asegurar que la celda exista
...
function recalcularEnergiaNeta(hotInstance) {
    console.log("Recalculando Demanda Neta ENERGIA GWH...");
    hotInstance.getData().forEach((row, rowIndex) => {
        let energiaDemanda = parseFloat(hotInstance.getDataAtCell(rowIndex, 1)) || 0;
        let energiaGeneracion = parseFloat(hotInstance.getDataAtCell(rowIndex, 5)) || 0;
        let total = energiaDemanda - energiaGeneracion;

        // **Actualizar la celda calculada**
        hotInstance.setDataAtCell(rowIndex, 8, total.toFixed(4));
    });
...
function importGSein(hot, updateFunction, configuracionColumnas) {
    const currentData = hot.getData();
    var fileInput = document.createElement("input");
    fileInput.type = "file";
    fileInput.accept = ".xlsx, .xls";
    ...
    fileInput.onchange = function (e) {
        ...
        reader.onload = function (e) {
            ...
            // Ignorar las primeras 3 filas del archivo importado, pero conservar la primera columna del currentData
            var jsonData = jsonD.slice(3).map((row, index) => {
                const newRow = [currentData[index]?.[0] || "", ...row.slice(1)];
                return newRow;
            });
                ...
                for (let i = 0; i < jsonData.length; i++) {
                    ...
                    if (rowData && rowData.length >= 4) {
                        for (let j = 1; j < rowData.length; j++) {
                            ...
                            if (value === "" || value === null) {
                                hot.setCellMeta(i, j, 'className', ''); // Marcar como correcta si está vacía
                            } else if (isNaN(parseFloat(value)) || !Number.isFinite(parseFloat(value))) {
                                hot.setCellMeta(i, j, 'className', 'htInvalid'); // Marcar la celda como incorrecta
                            }
                        }
                        ...
                        // Calcular factor de carga con 4 decimales
                        const factorCarga = parseFloat(calculateLoadFactor(energia, potenciaHP, potenciaHFP, year).toFixed(4));
                        rowData[4] = factorCarga.toFixed(4); // Asegurar que se mantenga con 4 decimales

                        if (factorCarga < 0 || factorCarga > 1) {
                            invalidFactorsFound = true;
                        }

                        // Calcular columna 8 (índice 7) = columna 1 - columna 5 con 4 decimales
                        const columna1 = parseFloat(rowData[1]) || 0;
                        const columna5 = parseFloat(rowData[5]) || 0;
                        const columna8 = (columna1 - columna5).toFixed(4);

                        // Asegurar que incluso si es 0, tenga 4 decimales
                        rowData[8] = columna8;
                    }
                }
            }
            ...
            hot.loadData(datosValidados);
            
            // Aplicar configuración de celdas y renderizado personalizado
            hot.updateSettings({
                cells: function (row, col) {
                    ...
                    if (value === "" || value === null) {
                        cellProperties.isCustomInvalid = false; // Marcar como válida si está vacía
                    } else if (isNaN(parseFloat(value)) || !Number.isFinite(parseFloat(value))) {
                        cellProperties.isCustomInvalid = true; // Marcar como incorrecto
                    } else {
                        cellProperties.isCustomInvalid = false; // Marcar como válido
                    }

                    cellProperties.renderer = function (instance, td, row, col, prop, value, cellProperties) {
                        Handsontable.renderers.TextRenderer.apply(this, arguments);
                        if (cellProperties.isCustomInvalid) {
                            td.style.backgroundColor = "#ff4c42"; // Fondo rojo para inválidos
