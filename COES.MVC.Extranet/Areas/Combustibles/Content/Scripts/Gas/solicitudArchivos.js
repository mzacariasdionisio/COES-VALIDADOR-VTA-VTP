...
function updateDimensionHandsonSustentatorio() {
    var idHandsontable = 'listaArchivos_sec_doc_118';

    var container = document.getElementById(idHandsontable)
    var hot = HOT_SUSTENTO;
    var offset = {};
    try {
...
function pUploadArchivoTabSustento(idEnvio, concepcodi, prefijo) {
    var uploaderP23 = new plupload.Uploader({
        ...
        url: siteRoot + 'Combustibles/EnvioGas/UploadTemporal',
...
function descargarArchivoTabSustento(row) {
    ...
    var regArchivo = getArchivoXFilaTabSustento(row);
    if (regArchivo != null) {
        window.location = controlador + `DescargarArchivoTemporal?idConcepto=${regArchivo.Ccombcodi}&fileName=${regArchivo.Cbarchnombrefisico}`;
    }
}
