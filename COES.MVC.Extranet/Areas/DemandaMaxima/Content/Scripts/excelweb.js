var controlador = siteRoot + 'demandaMaxima/envio/';
var uploader;
var totNoNumero = 0;
var totLimSup = 0;
var totLimInf = 0;
...
$(function () {
    ...
    $('#btnEditarSuministro').click(function () {

        if (consulta) {
            if (validarSeleccionDatos()) {
                popUpEditarSuministro();
            }
            else {
...
function popUpEditarSuministro() {
    ...
    $('#btnGrabarEditor').one("click", function (e) {
        e.preventDefault();
...
function mostrarMensajeEnvio() {
    ...
    if (envio > 0) {
        ...
        var mensaje = plazo + " | Día inicial: 01 - día final: 10 | <strong> Envío más reciente de la empresa y el periodo: </strong>" + evtHot.IdEnvio + " (" + evtHot.FechaEnvio + ")";
        return mensaje;
        //var plazo = (evtHot.EnPlazo) ? "<strong style='color:green'>en plazo</strong>" : "<strong style='color:red'>fuera de plazo</strong>";
        ...
        if (!evtHot.EnPlazo) {
            return "<strong style='color:red'>Fuera de plazo</strong> | Día inicial: 01 - día final: 10 |";
        }
        else return "<strong style='color:green'>En plazo</strong> | Día inicial: 01 - día final: 10 |";
    }
}
