...
namespace COES.MVC.Extranet.Areas.DemandaMaxima.Controllers
{
    public class EnvioController : BaseController
    {
        ...
        ReporteMedidoresAppServicio reporteMedidorServ = new ReporteMedidoresAppServicio();
        ...
        public int IdLectura = Int32.Parse(ConfigurationManager.AppSettings["IdLecturaPR16"]);
        ...
        public JsonResult GenerarFormato(int idEmpresa, string mes)
        {
            ...
            try
            {
                ...
                model.Formato.Formatnombre = siEmpresaDto.Emprabrev + "_PR16_" + mes;
        ...
        protected bool ValidarFechaPr16(MeFormatoDTO formato, int idEmpresa, out int horaini, out int horafin, out bool enPlazo)
        {
            ...
            DateTime fechaMinimaEnvio = formato.FechaProceso.AddMonths(formato.Formatmesplazo).AddDays(formato.Formatdiaplazo).AddMinutes(formato.Formatminplazo);
            DateTime fechamaximaEnvio = formato.FechaProceso.AddMonths(formato.Formatmesfinplazo).AddDays(formato.Formatdiafinplazo).AddMinutes(formato.Formatminfinplazo);
            //DateTime fechamaximaEnvio = fechaMinimaEnvio.AddMonths(formato.Formatmesplazo).AddDays(formato.Formatdiaplazo - 1).AddMinutes(formato.Formatminplazo);
                ...
                case ConstantesDemandaMaxima.ResolucionCuartoHora:
                    ...
                    if (lista96.Count > 0)
                    {
                        ...
                        try
                        {
                            ...
                            this.servicio.GrabarValoresCargados96PR16(lista96, User.Identity.Name, this.IdEnvio, idEmpresa, formato);
        ...
        public JsonResult MostrarHojaExcelWeb(int idEmpresa, int idEnvio, string mes)
        {
            ...
            if (entitys.Count > 0)
            {
                ...
                DateTime diaMaximaDemanda2 = reporteMedidorServ.GetDiaPeriodoDemanda96(fec_ini, fec_fin);

                jsModel.fechaMaximaDemanda = diaMaximaDemanda2.AddMinutes(15).ToString(Constantes.FormatoFechaHora);
                //- JDEL Fin


                SiEmpresaDTO siEmpresaDto = this.logic.GetByIdSiEmpresa(idEmpresa);
                //[TEMPORAL] PARA ATENDER TICKET 2024003460
                //if (siEmpresaDto.Emprcodi == 107) siEmpresaDto.Emprestado = "A";
        ...
        public ActionResult Index()
        {
            ...
            //- pr16.JDEL - Inicio 06/07/2016: Cambio para atender el requerimiento.
            ...
            {
                model.ListaEmpresas = this.seguridad.ObtenerEmpresasActivasPorUsuario(User.Identity.Name).
            ...
            model.ListaEmpresas=model.ListaEmpresas.OrderBy(emp => emp.Emprrazsocial).ToList();

            //[TEMPORAL] PARA ATENDER TICKET 2024003460
            //SiEmpresaDTO coelsivac = listEmpresas.Where(x => x.Emprcodi == 107).FirstOrDefault();
            //coelsivac.Emprrazsocial = coelsivac.Emprnomb;
            //model.ListaEmpresas.Add(coelsivac);
            ...
            if (mes != null && mes != "")
            {
                ...
                DateTime diaMaximaDemanda2 = reporteMedidorServ.GetDiaPeriodoDemanda96(fec_ini, fec_fin);
                model.fechaMaximaDemanda = diaMaximaDemanda2.AddMinutes(15).ToString(Constantes.FormatoFechaHora);// entity.FechaMD;
            }
            else
            {
        ...
        public FormatoModel BuildHojaExcel(int idEmpresa, int idEnvio, string mes)
        {
            ...
            empresa = logic.GetByIdSiEmpresa(idEmpresa);
            ...
            int cabcodi = 0;
            if (this.IdFormato == 54 && empresa.Tipoemprcodi == 2)
            {
                cabcodi = 43;
            }
            else cabcodi = model.Formato.Cabcodi;

            var cabercera = logic.GetListMeCabecera().Where(x => x.Cabcodi == cabcodi).FirstOrDefault();
            ...
            if (model.ListaHojaPto.Count > 0)
            {
                ...
                if ((idEnvio <= 0) || (idEnvio == idUltEnvio)) // id envio < 0 => es una carga de archivo excel, id envio = 0 => envio nuevo 
                {
                    ...
                    //11/09/2020 A pesar de estar fuera de plazose debe de permitir ingresar data
                    model.Handson.ReadOnly = false;// !this.ValidarFechaPr16(model.Formato, idEmpresa, out horaIni, out horaFin, out valorEnPlazo);
                    //- JDEL Fin
                }
                else
