...
namespace COES.MVC.Extranet.Areas.DemandaMaxima.Helper
{
    public class FormatoDemandaHelper
    {
        ...
        public static void GenerarFormato(FormatoModel model, int idEmpresa, string empresa, DateTime fecha, string semana)
        {
            ...
            string fileTemplate = ConstantesDemandaMaxima.PlantillaFormatoProgDiario;
        ...
        public static void GenerarFileExcel(FormatoModel model)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(newFile, template))
            {
                ...
                ///Imprimimos cabecera de puntos de medicion
                row = ConstantesDemandaMaxima.FilaExcelData;
                int totColumnas = model.ListaHojaPto.Count;
                int columnIni = ParamFormato.ColDatos;
        ...
        public static void ObtieneMatrizWebExcel(FormatoModel model, List<object> lista,List<MeCambioenvioDTO> listaCambios,int idEnvio)
        {
            if (idEnvio > 0)
            {
                ...
                {
                    #warning JDEL.30.05.2016 - Inicio:  Revisar/coordinar antes de efectuar cualquier pase a produccion - Solo valida para el PR16.
                    string text_fecha = model.Formato.FechaProceso.ToString(Constantes.FormatoFecha);
                    DemandaMaximaAppServicio servicio = new DemandaMaximaAppServicio();
                    List<MeFormatoEmpresaDTO> list = servicio.ObtenerListaPeriodoEnvio(text_fecha, model.Formato.Formatcodi, model.IdEmpresa);
                    MeFormatoEmpresaDTO per = new MeFormatoEmpresaDTO();
                    per = list[0];

                    if (per.PeriodoFechaIni.Value.ToString(Constantes.FormatoFecha) != text_fecha)
                    {
                        TimeSpan ts = per.PeriodoFechaFin.Value - per.PeriodoFechaIni.Value;
                        model.Formato.Formathorizonte = ts.Days;
                        model.Formato.FechaInicio = per.PeriodoFechaIni.Value;
                    }
                    #warning JDEL Fin
                    ...
                    for (int z = 0; z < model.Formato.Formathorizonte; z++) //Horizonte se comporta como uno cuando resolucion es mas que un dia
                    {
                        ...
                        for (int j = 1; j <= model.Formato.RowPorDia; j++) // nBlock se comporta como horizonte cuando resolucion es mas de un dia
                        {
                            if (k == 0)
                            {
                                ...
                                model.Handson.ListaExcelData[z * model.Formato.RowPorDia + j + model.FilasCabecera - 1][model.ColumnasCabecera - 1] = ObtenerCeldaFecha((int)model.Formato.Formatperiodo, (int)model.Formato.Formatresolucion,model.Formato.Lecttipo ,z, jIni, model.Formato.FechaInicio);
                            }
                            if (reg != null)
                            {
            ...
            for (var i = 1; i < nCol; i++)
            {
                ...
                for (var j = 0; j < nFil; j++)
                {
                    ...
                    if ((j % 96) == 0)
                    {
                        if (j != 0)
                        {
                            if (totalPR16.HasValue)
                            {
                                reg.Meditotal = totalPR16.Value;
                                totalPR16 = null;
                            }
                            lista.Add(reg);
                        }
                        ...
                        //- pr16.JDEL - Inicio 06/07/2016: Cambio necesario para habilitar la nulidad del campo mediTotal.
                        //reg.Meditotal = 0;
                        reg.Meditotal = null;
                        //- JDEL Fin
                        ...
                        if (Util.EsNumero(stValor))
                        {
                            ...
                            reg.H1 = valor;
                            //- remision-pr16.JDEL - Inicio 01/03/2016: Cambio para atender el requerimiento.
                            if (!totalPR16.HasValue)
                            {
                                totalPR16 = 0;
                            }
                    ...
                    else
                    {
                        ...
                        if (Util.EsNumero(stValor))
                        {
                            ...
                            //- remision-pr16.JDEL - Inicio 21/04/2016: Cambio para atender el requerimiento.    
                            //reg.Meditotal += valor;
                            if (!totalPR16.HasValue)
                            {
                                totalPR16 = 0;
        ...
        public static Boolean LeerExcelFile(string[][] matriz,string file, int rowsHead, int nFil, int colsHead, int nCol)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i < nFil; i++)
                {
                    ...
                    {
                        string valor = (ws.Cells[i + rowsHead + ConstantesDemandaMaxima.FilaExcelData, j + colsHead + 1].Value != null) ?
                            ws.Cells[i + rowsHead + ConstantesDemandaMaxima.FilaExcelData, j + colsHead + 1].Value.ToString() : string.Empty;
                ...
                case ParametrosFormato.PeriodoMensual:
                    switch (formato.Formatresolucion)
                    {
                        ...
                        case ParametrosFormato.ResolucionHora:
                            ...
                            //DateTime fec = formato.FechaProceso;
                            //formato.FechaInicio = fec.AddDays(-2);
                            //- JDEL Fin
        ...
        public static int EnviarCorreo(string formato, bool enPlazo, string empresa, DateTime fechaInicio, DateTime fechaFin, string areaCoes, string usuario,
            DateTime fechaEnvio, int idEnvio)
        {
            int resultado = 0;
            return resultado;
        }     
