...
$(function () {
    ...
    HEIGHT_FORMULARIO = $(window).height() - 330;
    WIDTH_FORMULARIO = 1000;
    ...
    $('#btnAutoguardar').click(function () {
        guardarFormularioCargaDatos(ACCION_GUARDADO_MANUAL); //formularioExcelWeb.js
    });
...
function getObjetoFiltro() {
    ...
    filtro.tipoFormato = parseInt($("#hfTipoFormato").val()) || 0;
...
//FORMULARIO
function listarArchivosEnvio(opcionFormulario) {
    DEBE_AUTOGUARDAR = false;

    _limpiarBarraMensaje_('mensajeEvento');
    $("#detalle_ficha_tecnica").hide();
    $("#detalle_ficha_tecnica").html("");

    $("#html_archivos").unbind();
    $("#html_archivos").html("");
    ...
    $.ajax({
        ...
        success: function (evt) {
            if (evt.Resultado != "-1") {
                ...
                cargarHtmlDocumentoXEnvio(evt.ListaReqEvento, TIPO_ARCHIVO_REQUISITO);

                LISTA_VERSIONES = evt.ListaVersion;
                LISTA_AUTOGUARDADO = evt.ListaAutoguardado;
                LISTA_ERRORES = evt.ListaErrores;

                var htmlTabla = dibujarTablaEquipoEnvioConexIntegModif(evt.ListaEnvioEq, getObjetoFiltro().codigoEnvio);
                ...
                //Muestro tabla con ancho completo (envios nuevos) o solo una parte (envios existentes) 
                var element = document.getElementById("tabla_OP");
                if (getObjetoFiltro().codigoEnvio <= 0) {     //cuando es envio nuevo               
                    element.classList.add("tabla_OP_100");
                    element.classList.remove("tabla_OP_Max");
                } else {
                    element.classList.add("tabla_OP_Max");
                    element.classList.remove("tabla_OP_100");
                }
                ...
                mostrarMensaje('mensaje_popupProyecto', $tipoMensajeMensaje, evt.Mensaje);
            }
        },
        error: function (err) {
...
async function verificarAutoguardadoYlistarEq() {
    if (OPCION_GLOBAL_EDITAR && FLAG_AUTOGUARDADO_HABILITADO) {
        ...
        if (VERSION_TEMPORAL_CREADO_OK) {
            listarArchivosEnvio(OPCION_FORMULARIO_ACTUAL_BD);
        }
    } else {
        //solo lectura
        listarArchivosEnvio(OPCION_FORMULARIO_ACTUAL_BD);
...
async function generarVersionTemporal() {
    ...
    return $.ajax({
        type: 'POST',
        url: controlador + "CrearVersionTemporalReq",
        dataType: 'json',
        data: {
            tipoFormato: getObjetoFiltro().tipoFormato,
            codigoEnvio: getObjetoFiltro().codigoEnvio,
...
async function enviarExcelWeb() {
    ...
    if (LISTA_ERRORES.length > 0) {
        ...
        if (confirm("¿Desea enviar información a COES?  Una vez enviado, se procederá con la revisión por parte de COES.")) {
            ...
            $.ajax({
                type: 'POST',
                url: controlador + "EnviarSolicitudOperacionComercial",
                data: {
                    codigoEnvio: getObjetoFiltro().codigoEnvio,
                    versionEnvio: getObjetoFiltro().codigoVersion,
                },
...
async function guardarFormularioCargaDatos(tipoAutoguardado, flagAutomError) {
    ...
    if (OPCION_GLOBAL_EDITAR && FLAG_AUTOGUARDADO_HABILITADO && !FLAG_PAUSAR_AUTOGUARDADO) {
        ...
        var formulario = {
            ...
            ListaReqEvento: LISTA_REQUISITO_ARCHIVO,
...
async function limpiarEquipoTemporalExcelWeb() {
    listarArchivosEnvio(OPCION_FORMULARIO_LIMPIAR);
}
...
function mostrarDetalleErrores() {
    ...
    setTimeout(function () {
        ...
        $('#tablaError').dataTable({
            ...
            "bPaginate": false,
            "iDisplayLength": -1
            , "oLanguage": { "sZeroRecords": "No existen errores", "sEmptyTable": "" }
        });

    }, 200);
}

function dibujarTablaError(listaErrores) {
...
function mostrarEnvioExcelWeb(versionAMostrar, versionAnterior, numVersion) {
    ...
    listarArchivosEnvio(OPCION_FORMULARIO_CAMBIO_ENTRE_VERSION);
}
...
function agregarBloqueRevision(evt) {
    ...
    if (evt.ListaRevisionParametrosAFT.length > 0) {
        ...
        <table style = "width: -webkit-fill-available;">
            <tr id="bloqueTexto_${indiceColumnaBloque}_${fevrqcodi}">
                <td style='border: 0;${styleFondo}'>
                   <div id="datoCelda_${indiceColumnaBloque}_${fevrqcodi}">
                     ${htmlEditado}
                   </div>
                   <div>
                        ${htmlBtn}
...
function generarTablaListaBodyXObs(listaArchivo, accion, enPopup, nColumna, fevrqcodi) {
    ...
    for (var i = 0; i < listaArchivo.length; i++) {
        ...
        if (accion == CELDA_REV_EDITAR && enPopup) {
            html += `     
                            <td onclick="eliminarRowXObs(${nColumna}, ${fevrqcodi},${i})" title='Eliminar archivo - ${nomb}' style="width:30px;cursor:pointer;${styleFondo}">
...
function mostrarListadoHtml(accion, valEstado, listado, miFevrqcodi, interfaz) {
    ...
    if (miEstenvcodi == ESTADO_SOLICITADO && interfaz == INTRANET) {
        valEstado = "-1";
        textoEstado = "";
...
function expandirRestaurar() {
    if ($('#hfExpandirContraer').val() == "E") {
        ...
        //parte izquierda
        $("#html_archivos").css("width", ($(window).width() - 50) + "px");
        $("#html_archivos").css("height", ($(window).height() - 180) + "px");

        //parte derecha
        $(".campo_cab_add_celdas.celdaREV").css("width", 125 + "px");
