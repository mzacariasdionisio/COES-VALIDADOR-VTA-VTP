var anchoPortal = 1000; //1200px es el ancho del portal. Este debe ser el mismo que se mostrará en Intranet
var OPCION_ACTUAL = 0;
var TIPO_SUBESTACION = -2;
var DETALLE_FT_GLOBAL = null;
const COLOR_CELDA = "#CBFBE6";
const COLOR_CELDA_MODIFICADA = "#FCB700"; // naranja
...
function _extranet_generarHtmlReporteDetalleFichaTecnica(model) {
    ...
    var tieneProcesoRevision = listaDataRevisionFT != null && listaDataRevisionFT.length > 0;
    var html1fila = "";
    if (tieneProcesoRevision) {
        html1fila = `
            <tr id="1raFila">
                <td style ="width: 50%;" colspan='${colspanIni}'></td>
            </tr>
        `;
        ...
        //cuando se realiza nuevo envio
        ancho = anchoPortal + "px";
    }
    ...
    var html = `
        <table id="reporte" class="pretty tabla-icono tree_ficha_tecnica miTablaR" style="overflow-y: hidden; overflow-x: auto; width: ${ancho}; ${layout}">
            <tbody>
                ${htmlCabecera}
                ${htmlItem}
                ${htmlNota}
...
function _extranet_getHtmlItem(listaAllData, listaTreeData, listaItemConfig, idPadre, iniciaNuevaFila, nivel, numMaxCol, colspanDisponible) {
    ...
    for (var j = 0; j < hijos.length; j++) {
        ...
        if (tipoAgrup == reg.Ftittipoitem) {
            ...
            var htmlItemValor = '';
            var esItemEditable = OPCION_GLOBAL_EDITAR && reg.EsFilaEditableExtranet;
            if (OPCION_ACTUAL != 2) {
                htmlItemValor = _extranet_getHtmlItemValor(reg, esItemEditable);
            }
            ...
            var boolDeshabilitar = !esItemEditable || reg.EsReplica;
            ...
            var ColFondo = colorCelda;
            if (configItem != undefined && configItem != null &&configItem.CambioValor == 1) {
                ColFondo = COLOR_CELDA_MODIFICADA;
            }
            ...
            if (mostrarOpcionValor) {
                ...
                if (mostrarComentario == "S") {
                    htmlItem += `
                     <td class="${clasePropiedad} valor celdaIzq" style="background: ${ColFondo};">${htmlItemValor} ${htmlInputAnotacion}</td>
                    `;
                } else {
                    htmlItem += `
                    <td class="${clasePropiedad} valor celdaIzq" style="background: ${ColFondo};">${htmlItemValor}</td>
                    `;
                }
            } else {
                htmlItem += `
            ...
            if (mostrarColumnaValorConfidencial) {
                if (mostrarOpcionValor) {
                    ...
                    if (mostrarCheckValConfidencial == "S" && !reg.EsArchivo) { //solo mostrar check cuando no sea archivo
                        ...
                        htmlItem += `
                              <td class="${clasePropiedad} valor celdaIzq" style="background: ${ColFondo};">
                                    <input type="checkbox" id="chk_ValConfidencial_${reg.Ftitcodi}" name="" value="" ${checkedConf} ${htmlDisabled} ${htmlFuncionalidadCopiar}>
                              </td>                              
                            `;
                    } else {
                        htmlItem += `
                              <td class="${clasePropiedad} valor celdaIzq" style="background: ${ColFondo};"></td>
                            `;
                    }
                } else {
                    htmlItem += `
            ...
            if (mostrarColumnaSustento) {
                if (mostrarOpcionValor) {
                    var htmlArchivoXDato = configItem != undefined ? (configItem.Fitcfgflagsustento == "S" ? cargarHtmlDocumentoXEnvio(reg, TIPO_ARCHIVO_SUSTENTO_DATO, !boolDeshabilitar) : '') : '';

                    htmlItem += `                              
                              <td class="${clasePropiedad} valor " style="background: ${ColFondo};">
                                <div id='' style="width: 75px;" >${htmlArchivoXDato}</div>
                              </td>
                            `;
                } else {
                    htmlItem += `
                              <td class="${clasePropiedad} valor " style="background: ${ColFondo};"></td>
                            `;
                }
            }
...
function _extranet_getHtmlItemValor(reg, esItemEditable) {

    var formatedo = replaceSpecialChars((reg.Valor == null || reg.Valor == "") ? "" : reg.Valor.trim());
    var htmlDisabled = !esItemEditable ? " disabled " : "";
    ...
    if (reg.Ftpropcodi != null) {
        ...
        if (reg.Concepcodi != null || reg.Propcodi != null) {
            if (reg.EsArchivo) {
                var htmlArchivoXDato = cargarHtmlDocumentoXEnvio(reg, TIPO_ARCHIVO_VALOR_DATO, esItemEditable);
                ...
                return `
                    <input id='campo_input_${reg.Ftitcodi}' type="text" style='width:80px' value="${formatedo}" ${htmlDisabled} ${htmlFuncionalidadCopiar}  autocomplete="off" />
                `;
            }
        }
    }
...
function _extranet_crearHtmlInputAnotacion(reg) {
    ...
    htmlNota += `
            <input id="campo_input_anotacion_${reg.Ftitcodi}" type="text" style="width:60px; display:none;" value="${replaceSpecialChars(reg.Itemcomentario)}"  autocomplete="off" >
    `;
