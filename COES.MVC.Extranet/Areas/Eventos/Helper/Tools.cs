...
namespace COES.MVC.Extranet.Areas.Eventos.Helper
{
    public class Tools
    {
        //FormatoMedicionAppServicio servPtomedicion = new FormatoMedicionAppServicio();
        ...
        /// <returns></returns>
        public static string[][] ObtenerListaExcelDataAntiguo(PruebaUnidadModel model, List<MeMedicion24DTO> lista24, List<MeCambioenvioDTO> listaCambios, string fecha, int idEnvio)
        {
            int nFil = 27;
            ...
            {
                lista[0][i] = listaPto[i - 1].Equinomb;
        ...
        /// <param name="idEnvio"></param>
        /// <returns></returns>
        public static string[][] ObtenerListaExcelDataAntiguo2(FormatoModel model, List<MeMedicion96DTO> lista96, List<MeCambioenvioDTO> listaCambios, string fecha, int idEnvio)
        {
            ...
            foreach (var f in listaFecha)
            {
                ...
                for (var z = 0; z < nCol; z++)
                {
                    ...
                    if (z > 0)
                    {
                        idPto = listaPto[z - 1].Ptomedicodi;
                        ptoInfoCodi = listaPto[z - 1].Tipoinfocodi;
                        tipoPtomedicodi = listaPto[z - 1].Tipoptomedicodi;
                        famcodi = -1;
                        ...
                        if (model.UtilizaMedidorGeneracion && model.ValidaMedidorGeneracion)
                        {
                    ...
                    for (int k = 1; k <= 96; k++)
                    {
                        ...
                        {
                            celdaFechaHora = FormatoHelper.ObtenerCeldaFecha((int)model.Formato.Formatperiodo, (int)model.Formato.Formatresolucion, 1, 0, k, mediFechaTmp);
        ...
        public static string[][] ObtenerListaExcelData(FormatoModel model, List<MeMedicion96DTO> lista24, List<MeCambioenvioDTO> listaCambios, string fecha, int idEnvio)
        {
            FormatoMedicionAppServicio servPtomedicion = new FormatoMedicionAppServicio();
            //string ptomedidesc = servPtomedicion.GetByIdMePtomedicion(1).Ptomedielenomb;
            ...
            int ptomedicodi = listaPto[0].Ptomedicodi;
            lista[0][1] = ptomedicodi.ToString();
            lista[1][1] = servPtomedicion.GetByIdMePtomedicion(ptomedicodi).Ptomedielenomb != null ? servPtomedicion.GetByIdMePtomedicion(ptomedicodi).Ptomedielenomb : "";// "Unidad sorteada";//listaPto[0].Ptomedidesc";
            lista[2][1] = "MW";
            ...
            for (var z = 0; z < nCol; z++)
            {
                ...
                DateTime fechaRec = DateTime.ParseExact(fecha, Constantes.FormatoFecha, CultureInfo.InvariantCulture);
                ...
                for (int k = 1; k <= 96; k++)
                {
                    if (z == 0)
                    {
                        fechaRec = fechaRec.AddMinutes(15);

                        //string hora = fechaRec.ToString(Constantes.FormatoOnlyHora);////("0" + (k - 1).ToString()).Substring(("0" + (k - 1).ToString()).Length - 2, 2) + ":00";
                        lista[2 + k][0] = fechaRec.ToString(Constantes.FormatoFechaHora);// fecha + " " + hora;
                    }
                    else
                    {
                        var find = lista24.Find(x => x.Ptomedicodi == idPto);
        ...
        public static string[][] LeerExcelFile(string file, List<MeHojaptomedDTO> listaPto, string fecha)
        {
            ...
            for (int i = 1; i < nCol; i++)
            {
                ...
                */
                ...
                lista[0][i] = listaPto[i - 1].Ptomedicodi.ToString();
                lista[1][i] = listaPto[i - 1].Ptomedidesc;
                lista[2][i] = "MW";
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i < nFil - 3; i++)
                {
                    ...
                    {
                        string valor = (ws.Cells[i + 3 + ConstantesEventos.FilaExcelData, j + 1 + 1].Value != null) ?
                           ws.Cells[i + 3 + ConstantesEventos.FilaExcelData, j + 1 + 1].Value.ToString() : string.Empty;
            ...
            DateTime fechaRec = new DateTime(2018, 1, 1);
            for (var i = 1; i < 97; i++) //Recorre las 24 horas del dia
            {
                fechaRec = fechaRec.AddMinutes(15);
                //string hora = ("0" + (i - 1).ToString()).Substring(("0" + (i - 1).ToString()).Length - 2, 2) + ":00";
                string hora = fechaRec.ToString(Constantes.FormatoOnlyHora);
        ...
        public static string GenerarFileExcelConsumo(StockCombustiblesModel model)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage())//, template))
            {
                ...
                ws.Cells[ParamFormatoEvento.RowArea, 1].Value = model.Formato.Areaname;
                ws.Cells[ParamFormatoEvento.RowTitulo, 1].Value = model.Formato.Formatnombre;
                ws.Cells[1, ParamFormatoEvento.ColEmpresa].Value = model.IdEmpresa.ToString();
                ws.Cells[1, ParamFormatoEvento.ColFormato].Value = model.Formato.Formatcodi.ToString();
                ...
                row = ConstantesEventos.FilaExcelData;
        ...
        /// <param name="model"></param>
        /// <param name="ruta"></param>
        public static string GenerarFileExcel(PruebaUnidadModel model)
        {
            string fileExcel = string.Empty;
        ...
        public static void LoadMatrizExcelDisponibilidad(string[][] data, List<MeMedicionxintervaloDTO> medicion, int col)
        {
            ...
            foreach (var reg in medicion)
            {
                ...
                data[i][1] = reg.Medintfechaini.ToString(ConstantesBase.FormatoFecha);
                data[i][2] = reg.Medintfechaini.ToString(Constantes.FormatoHora);
                data[i][3] = reg.Medinth1.ToString();
                data[i][4] = reg.Medestcodi.ToString();
                i++;
            }
