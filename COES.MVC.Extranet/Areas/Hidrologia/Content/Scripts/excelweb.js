var LISTA_FORMATO_VALIDO = [6, 5, 32, 7, 43];
...
function inicializaPagina() {
    ...
    $('#cbSemana').val(periodo.mes);
    cargarSemanaAnho();
    $('#cbArea').val($("#hfIdArea").val());
    $('#cbLectura').val($("#hfIdLectura").val());
    $('#cbEmpresa').val($("#hfIdEmpresa").val());
    listFormatCodi = strFormatCodi.split(',');
    listFormatPeriodo = strFormatPeriodo.split(',');
    cargarFormatos($("#hfIdFormato").val());
    editable = $('#hfEditable').val();
}

function cargarLectura(idArea) {
    $("#tr_horario").hide();
    $('#barraHidro').css("display", "none");
    $('#detalleFormato').hide();
...
function cargarFormatos(idFormato) {
    $("#tr_horario").hide();
    $('#barraHidro').css("display", "none");
    $('#detalleFormato').hide();
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            idLectura: $('#cbLectura').val(), idEmpresa: $('#cbEmpresa').val()
        },
        cache: false,
        success: function (aData) {
...
function getModelFormato(accion) {
    ...
    _mostrarFilaTablaHorario();
    var bloqueHorario = _obtenerBloqueHorario();
    ...
    $.ajax({
        ...
        data: {
            idEmpresa: idEmpresa,
            idFormato: idFormato, idEnvio: idEnvio, fecha: fecha, semana: semana,
            mes: mes,
        ...
        success: function (evt) {
            if (evt.Resultado == "1") {
                ...
                _mostrarFilaTablaHorario(evt.FiltroBloqueHorario);
            } else {
...
function _mostrarFilaTablaHorario(horaBD) {
    var formatcodi = parseInt($("#cbFormato").val()) || 0;

    $("#tr_horario").hide();
    if (_esValorEnLista(formatcodi, LISTA_FORMATO_VALIDO)) {

        var fechaSelected = $('#txtFecha').val();
        var fechaHoy = $("#hfFechaHoy").val();
        if (fechaSelected == fechaHoy) {
            $("#tr_horario").show();

            var hora = (new Date()).getHours();
            var entero = Math.floor(hora / 3);
            var resto = hora % 3;

            var bloque = entero + 1; //(resto == 0 ? 0 : 1)
            var horaPantalla = bloque * 3;
            var horaDisabled = horaPantalla + 3 ;

            //verificar si existe un envio en bd
            if (horaBD !== undefined && horaBD > 0)
                horaPantalla = horaBD;

            $("#myradio_" + horaPantalla).prop("checked", true);

            for (var i = horaDisabled; i <= 24; i++) {
                $("#myradio_" + i).prop("disabled", true);
            }
        }
    }
}

function _obtenerBloqueHorario() {
    var bloqueHorario = parseInt($('input[name="form_tiempo"]:checked').val()) || 0; //cmgPr07
    return bloqueHorario;
}

function _esValorEnLista(valor, lista) {
...
function validarEnvio() {
    ...
    //totalB = getTotBlancos(data, rowini, colini);
    //if (totalB > 0) {
...
function descargarFormato() {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            idEmpresa: $('#hfIdEmpresa').val(), idFormato: $('#hfIdFormato').val(), fecha: $('#hfFecha').val(),
            semana: $('#Anho').val() + $('#hfSemana').val(), //$('#hfSemana').val(),
            mes: $('#hfMes').val()
        },
        success: function (result) {
            if (result.Resultado == 1) {
                window.location = controlador + 'descargarformato?archivo=' + result.Archivo;
            }
            else {
                alert("Ocurrió un error al descargar formato");
            }
        },
        error: function (err) {
            alert("Error");
...
function enviarExcelWeb() {
    ...
    if (confirm("¿Desea enviar información a COES?")) {
        ...
        var dataExel = hot.getData();
        var dataExcelFormateado = JSON.parse(JSON.stringify(dataExel));
        if (dataExcelFormateado.length > 0) {
            for (var i = 0; i < dataExcelFormateado.length; i++) {
                for (var j = 0; j < dataExcelFormateado[i].length; j++) {
                    if (dataExcelFormateado[i][j] === true) {//verificar en columnas que tiene check
                        dataExcelFormateado[i][j] = "1";
                    }
                    if (dataExcelFormateado[i][j] === false) {//verificar en columnas que tiene check
                        dataExcelFormateado[i][j] = "2";
                    }
                }
            }
        }
        ...
        if (validarEnvio()) {
            $('#hfDataExcel').val(dataExcelFormateado);
            var bloqueHorario = _obtenerBloqueHorario();
...
function agregarFilaHandson() {
    ...
    for (var i = 0; i < listaId.length; i++) {
        ...
        grillaBD.push([fechaDia, id, getHoraInicio(), getHoraFin(), "0", idEmpresa, "", "<button onclick='alert()' type='button'>X</button>"]);
        hot.updateSettings({
            maxRows: index + 1,
            data: grillaBD
        });
