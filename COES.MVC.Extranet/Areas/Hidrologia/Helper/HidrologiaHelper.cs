...
namespace COES.MVC.Extranet.Areas.Hidrologia.Helper
{
    public class HidrologiaHelper
    {
        ...
        public static void GenerarFileExcel(FormatoModel model, string ruta, string nombreArchivo)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(newFile))
            {
                ...
                UtilExcel.AddImageXY(ws, 0, 0, img);
                //Escribe  Nombre Area
                ws.Cells[3, 2].Value = model.Formato.Areaname;
                ws.Cells[4, 2].Value = model.Formato.Formatnombre;
                UtilExcel.SetFormatoCelda(ws, 3, 2, 3, 2, "Centro", "Izquierda", "#000000", "#FFFFFF", "Calibri", 11, false);
                UtilExcel.SetFormatoCelda(ws, 4, 2, 4, 2, "Centro", "Izquierda", "#000000", "#FFFFFF", "Calibri", 11, true);

                //Imprimimos Codigo Empresa y Codigo Formato
                ws.Cells[3, ParamFormato.ColEmpresaNuevo].Value = model.IdEmpresa;
                ws.Cells[4, ParamFormato.ColFormatoNuevo].Value = model.Formato.Formatcodi;
                UtilExcel.SetFormatoCelda(ws, 3, ParamFormato.ColEmpresaNuevo, 3, ParamFormato.ColEmpresaNuevo, "Centro", "Izquierda", "#FFFFFF", "#FFFFFF", "Calibri", 11, false);
                UtilExcel.SetFormatoCelda(ws, 4, ParamFormato.ColEmpresaNuevo, 4, ParamFormato.ColEmpresaNuevo, "Centro", "Izquierda", "#FFFFFF", "#FFFFFF", "Calibri", 11, true);
                ...
                ws.Cells[row + 1, column].Value = model.Anho;
                int filasDetalle = 3;
                switch (model.Formato.Formatperiodo)
                {
                    case ParametrosFormato.PeriodoDiario:
                        ...
                        ws.Cells[row + 3, column].Value = model.Dia;
                        row = row + 3;
                        filasDetalle = 4;
                    ...
                    case ParametrosFormato.PeriodoSemanal:
                        ...
                        ws.Cells[row + 2, column].Value = model.Semana;
                        row = row + 2;
                        filasDetalle = 3;
                ...
                using (var range = ws.Cells[6, 1, 6 + filasDetalle - 1, 1])
                {
                ...
                for (var i = 0; i <= model.ListaHojaPto.Count; i++) //col
                {
                    for (var j = 0; j < model.Formato.Formathorizonte * model.Formato.RowPorDia + model.Formato.Formatrows; j++) //fila
                    {
                        decimal valor = 0;
                        ...
                        if (i >= model.ColumnasCabecera)
                        {
                            ...
                            {
                                if (j == 1) ws.Cells[row + j, i + 1].Value = "ESTADO";
                                if (j == 2) ws.Cells[row + j, i + 1].Value = "E/P";
                                if (j > 2)
                                {
                                    string desc = valor == 1 ? "E" : "P";
                                    ws.Cells[row + j, i + 1].Value = desc;
                                }
                            }
                        }
                ...
                int rowFinCabecera = row + model.Formato.Formatrows - 1;
                using (var range = ws.Cells[row, 2, rowFinCabecera, model.ListaHojaPto.Count + 1])
                {
                ...
                ws.View.FreezePanes(rowFinCabecera + 1, 1);
        ...
        public static void ObtieneMatrizWebExcel(FormatoModel model, List<object> lista, List<MeCambioenvioDTO> listaCambios, int idEnvio, int bloqueHorario)
        {
            ...
            foreach (var reg in listaCambios)
            {
                if (reg.Cambenvcolvar != null)
                {
                    ...
                    for (var i = 0; i < cambios.Count(); i++)
                    {
                        ...
                        var horizon = ts.Days;
                        var col = model.ColumnasCabecera - 1;

                        foreach (var regPto in model.ListaHojaPto) 
                        {
                            col++;
                            if (regPto.Ptomedicodi == reg.Ptomedicodi) break;                            
                        }
            ...
            int nCol = model.ListaHojaPto.Count + model.ListaHojaPto.Where(x => x.Hptoindcheck == "S").Count();
            int k = 0;
            foreach (var regPto in model.ListaHojaPto)
            {
                for (int z = 0; z < model.Formato.Formathorizonte; z++) //Horizonte se comporta como uno cuando resolucion es mas que un dia
                {
                    ...
                    var reg = lista.Find(i => (int)i.GetType().GetProperty("Ptomedicodi").GetValue(i, null) == regPto.Ptomedicodi
                    ...
                    for (int j = 1; j <= model.Formato.RowPorDia; j++) // nBlock se comporta como horizonte cuando resolucion es mas de un dia
                    {
                        ...
                        if (regPto.Hptoindcheck != "S")
                        {
                            int filCelda = z * model.Formato.RowPorDia + j + model.FilasCabecera - 1;
                            int colCelda = k + model.ColumnasCabecera;

                            if (model.Handson.MatrizTipoEstado[filCelda][colCelda] != 1)
                            {
                                if (reg != null)
                                {
                                    ...
                                    if (valor != null)
                                        model.Handson.ListaExcelData[filCelda][colCelda] = valor.ToString();
                                }
                            }
                            else
                            {
                                //si la celda no es de solo lectura entonces print valor
                                model.Handson.ListaExcelData[filCelda][colCelda] = "";
                            }
                        }
                        ...
                        if (regPto.Hptoindcheck == "S")
                        {
                            if (model.Formato.RowPorDia == 24)
                            {
                                string sCheck = ConstHidrologia.TieneSinCheckExtranet; //default unchecked
                                if (regPto.Tipoinfocodi == ConstantesAppServicio.TipoinfocodiM3s) //caudal
                                {
                                    sCheck = ConstHidrologia.TieneConCheckExtranet; //default checked para caudal
                                    if (hora <= j - 1) sCheck = ConstHidrologia.TieneSinCheckExtranet; //uncheck
                                }

                                if (reg != null)
                                {
                                    int? valorHCheck = (int?)reg.GetType().GetProperty("T" + j).GetValue(reg, null);
                                    sCheck = ConstHidrologia.TieneSinCheckExtranet; //unchecked
                                    if (valorHCheck != null && valorHCheck == 1) sCheck = ConstHidrologia.TieneConCheckExtranet;//check
                                }

                                //valor por defecto sino se usa el importado en excel
                                if (string.IsNullOrEmpty(model.Handson.ListaExcelData[z * model.Formato.RowPorDia + j + model.FilasCabecera - 1][k + model.ColumnasCabecera]))
                                    model.Handson.ListaExcelData[z * model.Formato.RowPorDia + j + model.FilasCabecera - 1][k + model.ColumnasCabecera] = sCheck;

                                if (regPto.Tipoinfocodi == ConstantesAppServicio.TipoinfocodiM3s && hora <= j - 1)
                                    model.Handson.ListaExcelData[z * model.Formato.RowPorDia + j + model.FilasCabecera - 1][k + model.ColumnasCabecera] = ConstHidrologia.TieneSinCheckExtranet;
                            }
                        }
        ...
        public static short[][] IncializaMatrizEstado(List<MeHojaptomedDTO> listaPtos, int nFilCabecera, int nFilasDatos, int nColCabecera, short valorInicio, int horizonte, DateTime fechaProceso, bool esFormatoSCO)
        {
            int dif = (DateTime.Today - fechaProceso).Days;
            int hora = DateTime.Now.Hour + 1;

            int nFil = nFilCabecera + horizonte * nFilasDatos; // numero de filas de las matriz incluido cabecera
            int nCol = listaPtos.Count + nColCabecera;
            ...
            for (int i = 0; i < nFil; i++)
                for (int j = 0; j < nCol; j++)
                {
                    ...
                    if (j >= nColCabecera)
                    {
                        var regPto = listaPtos[j - nColCabecera];

                        if (esFormatoSCO && dif == 0)
                        {
                            //si es formato de coordinación y
                            //1. Si el punto de medición no tiene check entonces se deshabilita las horas futuras (el agente solo enviará datos numéricos ejecutados)
                            //1. Si el punto de medición tiene check y es Volumen entonces se deshabilita las horas futuras (el agente solo enviará datos numéricos hasta la hora actual)
                            if (!regPto.TieneCheckExtranet || (regPto.TieneCheckExtranet && regPto.Tipoinfocodi == 14)) //VOLUMEN
                            {
                                if (nFilCabecera + hora <= i)
                                    lista[i][j] = 1; //celda de solo lectura
                            }
                        }
                    }
                }
        ...
        public static string ObtenerCeldaFecha(int periodo, int resolucion, int tipoLectura, int horizonte, int indice, DateTime fechaInicio)
        {
            ...
            switch (periodo)
            {
                case ParametrosFormato.PeriodoMensual:
                    ...
                    {
                        case ParametrosFormato.ResolucionMes:
                            if (tipoLectura == ParametrosLecturaHidro.Ejecutado)
        ...
        public static List<MeHojaptomedDTO> ListaHojaptoCheck(DateTime fechaProceso, List<MeHojaptomedDTO> listaHojaPto)
        {
        ...
        public static List<MeMedicion24DTO> LeerExcelWeb24(List<string> data, List<MeHojaptomedDTO> ptos, int idLectura, int colHead, int nCol, int filaHead, int nFil)
        {
            ...
            try
            {
                ...
                {
                    MeHojaptomedDTO regPto = ptos[i - colHead];

                    for (var j = 0; j < nFil; j++)
                    {
                        //verificar inicio de dia
                        if ((j % 24) == 0 && (regPto.Hptoindcheck != "S"))
                        {
                            if (j != 0)
                                lista.Add(reg);
                            reg = new MeMedicion24DTO();
                            reg.Ptomedicodi = regPto.Ptomedicodi;
                            reg.Lectcodi = idLectura;
                            reg.Tipoinfocodi = (int)regPto.Tipoinfocodi;
                            reg.Emprcodi = regPto.Emprcodi;
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if (COES.Base.Tools.Util.EsNumero(stValor))
                                {
                                    valorT = Convert.ToInt32(ConstHidrologia.TieneSinCheckExtranet);//unchecked
                                    if (stValor == ConstHidrologia.TieneConCheckExtranet) valorT = Convert.ToInt32(ConstHidrologia.TieneConCheckExtranet);
                                }
                                reg.GetType().GetProperty("T" + indice.ToString()).SetValue(reg, valorT);
                            }
        ...
        public static Boolean LeerExcelFile(string[][] matriz, string file, int rowsHead, int nFil, int colsHead, int nCol)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i < nFil; i++)
                {
                    for (int j = 0; j < nCol; j++)
                    {
                        var celdaObj = ws.Cells[i + rowsHead + ParametrosFormato.FilaExcelData, j + colsHead + 1].Value;
                        string valor = (celdaObj != null) ? celdaObj.ToString() : string.Empty;

                        if (valor == "E") valor = ConstHidrologia.TieneConCheckExtranet;
                        if (valor == "P") valor = ConstHidrologia.TieneSinCheckExtranet;
        ...
        public static void GetFechaActualEnvio(int periodo, int tipo, ref string mes, ref string fecha, ref int semana, ref int anho)
        {
            ...
            {
                case ParametrosFormato.PeriodoDiario:
                    if (tipo == ParametrosLecturaHidro.Ejecutado)
                    {
                        fecha = fechaActual.AddDays(-1).ToString(Constantes.FormatoFecha);
                    }
                    if (tipo == ParametrosLecturaHidro.Programado)
                    {
                        fecha = fechaActual.ToString(Constantes.FormatoFecha);
                    }
                    if (tipo == ParametrosLecturaHidro.TiempoReal)
                    {
                        fecha = fechaActual.ToString(Constantes.FormatoFecha);
                    }
                    break;
        ...
        public static int VerificarIdsFormato(string file, int idEmpresa, int idFormato)
        {
            ...
            {
                ExcelWorksheet ws = xlPackage.Workbook.Worksheets[1];
                var valorEmp = ws.Cells[3, ParamFormato.ColEmpresaNuevo].Value?.ToString();
                if (!int.TryParse(valorEmp, NumberStyles.Any, CultureInfo.InvariantCulture, out idEmpresaArchivo)) ;

                var valorFormato = ws.Cells[4, ParamFormato.ColFormatoNuevo].Value?.ToString();
                if (!int.TryParse(valorFormato, NumberStyles.Any, CultureInfo.InvariantCulture, out idFormatoEmpresa)) ;

                if (idEmpresaArchivo == 0)
                {
                    var valorEmpOld = ws.Cells[1, ParamFormato.ColEmpresa].Value?.ToString();
                    if (!int.TryParse(valorEmpOld, NumberStyles.Any, CultureInfo.InvariantCulture, out idEmpresaArchivo)) ;
                }
                if (idFormatoEmpresa == 0)
                {
                    var valorFormatoOld = ws.Cells[1, ParamFormato.ColFormato].Value?.ToString();
                    if (!int.TryParse(valorFormatoOld, NumberStyles.Any, CultureInfo.InvariantCulture, out idFormatoEmpresa)) ;
                }
        ...
        public static int EnviarCorreo(string lectura, string formato, bool enPlazo, string empresa, DateTime fechaInicio, DateTime fechaFin, string areaCoes, string usuario, DateTime fechaEnvio, int idEnvio)
        {
            ...
            string toEmail = ConfigurationManager.AppSettings[ConstantesHidrologia.ToMail];
            string ccEmail = string.Empty;
            if (usuario.IndexOf("@") != -1)
                ccEmail = usuario;
            appMail.EnviaCorreoFull(ConstantesHidrologia.FromEmail, ConstantesHidrologia.FromName, toEmail,
