...
function inicializarFormato(numHoja) {
    ...
    var menor_actual = "";
    $("input[name=form_tiempo]").each(function (index) {
        valor = $(this).val();

        if (valor < hours) {
            menor_actual = valor;
            $("input:radio[value=" + valor + "]").prop('checked', true);
        }
        else if (valor == hours) {
            $("input:radio[value=" + valor + "]").prop('checked', true);
        }
        else if (hours >= 0 && hours < 3) {
            $("input:radio[value=03]").prop('checked', true);
        }
        else if (valor > hours) {
            $("input:radio[value=" + valor + "]").prop('disabled', true);
        }
    ...
    $('input:radio[name="form_tiempo"]').change(function () {
        btnConsultar(numHoja);
    });
}
...
function mostrarFormulario(numHoja, accion) {
    ...
    var fechaActual = day.toString().padStart(2, '0') + "/" + month.toString().padStart(2, '0') + "/" + year.toString();
    var horarioenvio = 0;
            ...
            if (evt != -1) {
                if (evt.Resultado != '-1') {
                    ...
                    if (VER_ULTIMO_ENVIO == verUltimoEnvio) {
                        IdEnvioLast = getVariableEvt(numHoja).IdEnvioLast;
                        setIdEnvio(numHoja, IdEnvioLast);
                        horarioenvio = getHorarioCodi(numHoja);

                        if (idEnvio > 0) {
                            accion = OPCION_ENVIAR_DATOS;
                            accionActual = 1; //Nuevo envío
                        }
                        else if (idEnvio <= 0 && cadenaFecha == fechaActual) {
                            accion = OPCION_CONSULTAR_HOY; //Consulta HOY 
                            accionActual = 2;
                            //verUltimoEnvio = NO_VER_ULTIMO_ENVIO;
                        }
                        else if (idEnvio <= 0) {
                            accion = OPCION_CONSULTAR; //Consulta AYER
                            accionActual = 3;
                        }
                    }
                    ...
                    switch (accion) {
                        case OPCION_CONSULTAR:
                            var mensaje = obtenerMensajeEnvio(numHoja, idEnvio, accionActual, val_envio, horarioenvio, evt);
                        ...
                        case OPCION_ENVIAR_DATOS:
                            ...
                            break;
                        case OPCION_ENVIO_ANTERIOR: //Envío anterior de días anteriores(AYER)
                            var mensaje = obtenerMensajeEnvio(numHoja, idEnvio, accionActual, val_envio, horarioenvio, evt);
                            mostrarMensajeInformativo(numHoja, mensaje);
                            verUltimoEnvio = VER_ENVIO;
                            break;
                        case OPCION_IMPORTAR_DATOS:
                            ...
                            break;
                        case OPCION_CONSULTAR_HOY:
                            var mensaje = obtenerMensajeEnvio(numHoja, IdEnvioLast, accionActual, val_envio, horarioenvio, evt);
                            mostrarMensajeInformativo(numHoja, mensaje);
                            break;
                    }
...
function mostrarExcelWeb(numHoja) {
    ...
    var _fecha = $(getIdElemento(numHoja, "#txtFecha")).val().split("/");
    var Fecha1 = new Date(parseInt(_fecha[2]), parseInt(_fecha[1] - 1), parseInt(_fecha[0]));
    if (Fecha1 < hoy) {
        $('#table_horario').hide();
        $('input[name=form_tiempo]').attr('checked', false);
    }
...
function actualizarViewHoja(evt, numHoja) {
    ...
    var _fechCalendario = $(getIdElemento(numHoja, "#txtFecha")).val();
    ...
    if (!getTieneHojaView(numHoja)) {
        ...
        crearGrillaExcel(evt, numHoja, val_envio, _fechCalendario);
    } else {
        var listaHoja = getListaHoja(numHoja);
        for (var nh = 0; nh < listaHoja.length; nh++) {
            //la variable evt
            ...
            crearGrillaExcel(evtView, numHojaView, val_envio, _fechCalendario);
