...
$(document).ready(function ($) {
    ...
    $('.btnok').click(function () {
        ...
        if (document.getElementById('ExcelBuscar')) { // Reemplazar
    ...
    $('#btnRegresar').click(function () {
        var idTipoProgramacion = document.getElementById('TipoProgramacion').value;
        var progrcodi = document.getElementById('idProgramacion').value;
        var emprcodi = document.getElementById('sEmprcodi').value;
        var flgSoloLectura = 99;

        //Observación parametros en la URL - Modificado 15/05/2019
        var paramList = [
            { tipo: 'input', nombre: 'progCodi', value: progrcodi },
            { tipo: 'input', nombre: 'tipoProgramacion', value: idTipoProgramacion },
            { tipo: 'input', nombre: 'IdEmpresa', value: emprcodi },
            { tipo: 'input', nombre: 'flgSoloLectura', value: flgSoloLectura }
        ];
        ...
        //window.location.href = controler + 'IntervencionesRegistro?progCodi=' + progrcodi + '&tipoProgramacion=' + idTipoProgramacion + '&IdEmpresa=' + emprcodi + '&flgSoloLectura=' + flgSoloLectura;
    });
...
async function validarReporte(nameFilem, sAccion) {
    var dataJsonExcl = null;
    var dataJsonIncl = null;
    var flagContinuar = true;
    if (!st_FlagTieneListaImportacionCompleto()) {
        flagContinuar = false;
    } else {
        flagContinuar = true;
        dataJsonExcl = JSON.stringify(st_ObtenerJsonLista(TIPO_EXCLUSION));
        dataJsonIncl = JSON.stringify(st_ObtenerJsonLista(TIPO_INCLUSION));
    }

    if (flagContinuar)
        procesarImportacionBD(nameFilem, sAccion, dataJsonExcl, dataJsonIncl);
    else {
        st_mostrarTablaFormularioImportacion(nameFilem, sAccion);
    }
}

async function procesarImportacionBD(nameFilem, sAccion, dataJsonExcl, dataJsonIncl) {
    var tipoProgramacion = document.getElementById('TipoProgramacion').value;
    var progrCodi = document.getElementById('idProgramacion').value;
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            filename: nameFilem,
            accion: sAccion,
            dataJsonExcl: dataJsonExcl,
            dataJsonIncl: dataJsonIncl
        }),
        ...
        success: function (evt) {
            if (evt.Resultado != '-1') {
                ...
                //sustento
                LISTA_PLANTILLA_FORMULARIO_EXCLUSION = evt.ListaExclusion;
                LISTA_PLANTILLA_FORMULARIO_INCLUSION = evt.ListaInclusion;
                st_mostrarTablaFormularioImportacion(nameFilem, sAccion);
            }
