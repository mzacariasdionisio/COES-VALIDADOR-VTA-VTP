...
var LISTA_PLANTILLA_FORMULARIO_INTERCODIS = '';

var LISTA_FAMCODI_SUSTENTO_OBLIGATORIO = [];
var LISTA_FAMCODI_SUSTENTO_OPCIONAL = [];

var IMG_SUSTENTO_LLENO = `<img src="${siteRoot}Content/Images/complete.png" title="El formato está completo" width="19" height="19" />`;
var IMG_SUSTENTO_FALTA = `<img src="${siteRoot}Content/Images/ico-warning.gif" title="El formato está incompleto"  width="19" height="19" />`;
...
function st_FlagTieneFormularioCompleto() {
    if (PLANTILLA_FORMULARIO_INCLUSION == null && PLANTILLA_FORMULARIO_EXCLUSION == null) {
        ...
        var flagEqObligIncl = false;
        if (PLANTILLA_FORMULARIO_INCLUSION != null) {
            flagEqObligIncl = LISTA_FAMCODI_SUSTENTO_OBLIGATORIO.find(x => x == PLANTILLA_FORMULARIO_INCLUSION.Famcodi) != null;
            if (flagEqObligIncl)
                estaCompleto = estaCompleto && PLANTILLA_FORMULARIO_INCLUSION.Sustento.TienePlantillaCompleta;
        }

        var flagEqObligExcl = false;
        if (PLANTILLA_FORMULARIO_EXCLUSION != null) {
            flagEqObligExcl = LISTA_FAMCODI_SUSTENTO_OBLIGATORIO.find(x => x == PLANTILLA_FORMULARIO_EXCLUSION.Famcodi) != null;
            if (flagEqObligExcl)
...
function st_mostrarBtnEnFormularioPrincipal() {
    ...
    var listaMsj = [];
    if (PLANTILLA_FORMULARIO_INCLUSION != null && PLANTILLA_FORMULARIO_INCLUSION.Sustento != null && flagEqObligIncl && !PLANTILLA_FORMULARIO_INCLUSION.Sustento.TienePlantillaCompleta)
        listaMsj.push("<li>Debe ingresar Sustento de Inclusión.</li>");
    if (PLANTILLA_FORMULARIO_EXCLUSION != null && PLANTILLA_FORMULARIO_EXCLUSION.Sustento != null && flagEqObligExcl && !PLANTILLA_FORMULARIO_EXCLUSION.Sustento.TienePlantillaCompleta)
        listaMsj.push("<li>Debe ingresar Sustento de Exclusión.</li>");
...
async function st_CargarFormularioSustento(tipo, intercodiEliminar, nroItem) {
    var flagMostrarFormulario = true;
        ...
        if (tipo == 'I') {
            if (nroItem > 0) { } else {
                if (intercodiEliminar == 0) {
                    ...
                    if (msj != "") {
                        mostrarAlerta("Debe completar los datos faltantes de la intervención." + "<br/>" + msj);
                        flagMostrarFormulario = false;
                    } else {
...
function st_GenerarHtmlFormulario(tipo, objPlt, intercodiEliminar, nroItem) {
    ...
    var esEditable = nroItem > 0 || intercodiEliminar > 0 || OPCION_ACTUAL != OPCION_VER;
...
async function st_GuardarContenidoPlantilla() {
    st_limpiarMensaje();
    ...
    if (listaMsj.length == 0) {
        ...
        if (intercodiEliminar <= 0 && nroItem <= 0) {
            limpiarMensaje();
        } else {
            var tipoFormulario = $("#tipoFormulario").val();
            if (tipoFormulario == "I") {
                //replicar contenido a las otras intervenciones
                await st_ReplicarSustento(objPlt);

                //actualizar estado de las registros eliminados / nuevos
                st_mostrarTablaFormularioListadoImportacion();

            } else {
                //replicar contenido a las otras intervenciones
                await st_ReplicarSustento(objPlt);

                //actualizar estado de las registros eliminados
...
function st_ObtenerPlantilla(tipo, intercodiEliminar, nroItem) {
    ...
    if (nroItem > 0) {
        ...
        if (intercodiEliminar == 0) {
            ...
            if (TIPO_INCLUSION == tipo) {
                var regSt = PLANTILLA_FORMULARIO_INCLUSION.Sustento;
                objPlt = JSON.parse(JSON.stringify(INTERVENCION_WEB));
                objPlt.Sustento = regSt;
            } else {
                objPlt = PLANTILLA_FORMULARIO_EXCLUSION;
            }
...
async function st_ReplicarSustento(objPlt) {
    //buscar las intervenciones que tengan el mismo código de seguimiento
    var listaInsumo = [];
    if (objPlt.Sustento.FlagTieneExclusion) {
        listaInsumo = LISTA_PLANTILLA_FORMULARIO_EXCLUSION.filter(x => x.Intercodsegempr == objPlt.Intercodsegempr && x.Equicodi == objPlt.Equicodi);
    } else {
        listaInsumo = LISTA_PLANTILLA_FORMULARIO_INCLUSION.filter(x => x.Intercodsegempr == objPlt.Intercodsegempr && x.Equicodi == objPlt.Equicodi);
    }

    if (listaInsumo.length > 1) {
        //copiar objetos texto y archivos
        var listaCDest = [];
        for (var i = 0; i < listaInsumo.length; i++) {
            var objInt = listaInsumo[i];

            var objStClone = JSON.parse(JSON.stringify(objPlt.Sustento));
            objInt.Sustento = objStClone;

            listaCDest.push(objInt.Intercarpetafiles);
        }

        //copiar archivos del FileServer
        await st_ReplicarArchivos(objPlt.Progrcodi, objPlt.Intercarpetafiles, listaCDest.join(','));
    }
}

async function st_ReplicarArchivos(progrcodi, carpetaOrigen, carpetaDestino) {
    return await $.ajax({
        type: 'POST',
        url: controlador + "ReplicarArchivoJustificacion",
        dataType: 'json',
        data: {
            progrcodi: progrcodi,
...
async function st_VerificarListaIntervencionPuedeTenerSustento() {
    ...
    if (LISTA_PLANTILLA_FORMULARIO_EXCLUSION == null
        || LISTA_PLANTILLA_FORMULARIO_EXCLUSION.length == 0
        || LISTA_PLANTILLA_FORMULARIO_INTERCODIS != intercodis) {
        var idTipoprog = parseInt($("#idTipoProgramacion").val()) || 0;
...
function st_FlagTieneListaFormularioCompleto() {
    if (LISTA_PLANTILLA_FORMULARIO_EXCLUSION == null || LISTA_PLANTILLA_FORMULARIO_EXCLUSION.length == 0) {
        ...
        for (var i = 0; i < LISTA_PLANTILLA_FORMULARIO_EXCLUSION.length; i++) {
            ...
            var flagEqOblig = LISTA_FAMCODI_SUSTENTO_OBLIGATORIO.find(x => x == LISTA_PLANTILLA_FORMULARIO_EXCLUSION[i].Famcodi) != null;
            if (flagEqOblig) estaCompleto = estaCompleto && regSust.TienePlantillaCompleta;
        }
...
function st_DibujarTablaListadoHtml(tipo, lista) {
    ...
    <table border="0" class="pretty tabla-icono" id="tbl_lista_sustento_${tipo}" style="overflow: auto; height:auto; width: 2000px !important; white-space: nowrap">
        <thead>
            <tr>
                <th style="text-align:center;width:4%">¿Completo?</th>
    ...
    for (key in lista) {
        ...
        var tdEstado = '';
        if (item.Sustento.TienePlantillaCompleta) tdEstado = IMG_SUSTENTO_LLENO;
        else tdEstado = IMG_SUSTENTO_FALTA;
    ...
    if (listaPlt != null) {
        for (var i = 0; i < listaPlt.length; i++) {
            ...
            var regIn = {
                ...
                Intercarpetafiles: regWeb.Intercarpetafiles
            };
...
function st_mostrarTablaFormularioImportacion(nameFile, sAccion) {
    $("#NombreArchivoFormulario").val(nameFile);
    $("#AccionFormulario").val(sAccion);

    st_mostrarTablaFormularioListadoImportacion();
}

function st_mostrarTablaFormularioListadoImportacion() {
    var nameFile = $("#NombreArchivoFormulario").val();
    var sAccion = $("#AccionFormulario").val();
    ...
    if (!hayExcl && !hayIncl) {
        ...
        if (hayIncl) {
            for (var i = 0; i < LISTA_PLANTILLA_FORMULARIO_INCLUSION.length; i++) {
                ...
                var flagEqOblig = LISTA_FAMCODI_SUSTENTO_OBLIGATORIO.find(x => x == LISTA_PLANTILLA_FORMULARIO_INCLUSION[i].Famcodi) != null;
                if (flagEqOblig) estaCompleto = estaCompleto && regSust.TienePlantillaCompleta;
            }
        }
