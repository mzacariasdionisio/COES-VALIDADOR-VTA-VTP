var controlador = siteRoot + 'RDO/CaudalVolumen/';
$(function () {
    inicializaPagina();
    /// Crea Objeto Pupload
    crearPupload();
...
function inicializaPagina() {
    ...
    $('#txtFecha').Zebra_DatePicker({
        ...
        onSelect: function () {
            ...
            var fechatemporal = $('#txtFecha').val();
            var fch_cmb = fechatemporal.split("/").reverse().join("-");
            var f = new Date(fch_cmb);
            var dd = f.getDate() + 1;
            var mm = f.getMonth() + 1;
            ...
            myDate = new Date();
            hours = myDate.getHours();
            var ddlc = myDate.getDate();
            var mmlc = myDate.getMonth() + 1;
            var yylc = myDate.getFullYear();
            ...
            $("input[name=form_tiempo]").each(function (index) {
                ...
                else if (valor > hours && dd == ddlc && mm == mmlc && yy == yylc) {
                    $("input:radio[value=" + valor + "]").prop('disabled', true);
                }
                else
                    $("input:radio[value=" + valor + "]").prop('disabled', false);
...
function cargarFormatos(idFormato) {
    ...
    $.ajax({
        ...
        dataType: 'json',
        data: {
            idLectura: $("#hfIdLectura").val(), idEmpresa: $('#cbEmpresa').val()
        },
        cache: false,
        success: function (aData) {
        ...
        success: function (evt) {
            ...
            switch (parseInt(idFormato)) {
                ...
                default:
                    ...
                    crearGrillaFormatoTipo1(evt, habilitarEditar, val_envio);
            ...
            UploadComplete: function (up, file) {
                if (!evtHot.Handson.ReadOnly) {
                    ...
                    if (retorno == 1) {
                        ...
                        mostrarError("Error: La hora registrada está fuera de rango de horario.");
                    }
                    else {
