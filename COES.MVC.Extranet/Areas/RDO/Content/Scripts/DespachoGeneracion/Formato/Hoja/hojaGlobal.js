    ...
    for (var col = 0; col < numCol && !fin; col++) {
        for (var row = 0; row < numFil && !fin; row++) {
            ...
            if (!readOnly) {
                ...
                if (row > listaPtos.length && col < numCol && col > 0) {

                    if (listaPtos[0].Famcodi == 37 || listaPtos[0].Famcodi == 39)
                        valor_check = getVariableHot(numHoja).view.instance.getDataAtCell(row, numCol - 2).toString();

                    if (valor_check == "")
                        var regError = obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja, "NS");
                    else
                        var regError = obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja, valor_check);                   
                }
                else {
                    var regError = obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja,'NS');
                }
...
function obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja, valor_check) {
    var errorReg = null;
    ...
    if (value != null) {
        ...
        if (isNaN(value) != true && (value + "") != "" && col <= numFilCabecera) {
            ...
            if (value == "0") {
                ...
                else if (value == "" && col < listaPtos.length && (listaPtos[col - 1].Famcodi == 37 || listaPtos[col - 1].Famcodi == 39)) {
                    errorReg = obtenerError(celda, value, ERROR_BLANCO, numHoja, '');
                }
                else if (value == "" && valor_check == "true") {
                    //var abc = ht.instance.getDataAtCell(row, nColumnas);
                    errorReg = obtenerError(celda, value, ERROR_BLANCO, numHoja, '');
                }
                else if (value == "" && valor_check == "NS") {
                    //var abc = ht.instance.getDataAtCell(row, nColumnas);
                    errorReg = obtenerError(celda, value, ERROR_BLANCO, numHoja, '');
                }
                else {
