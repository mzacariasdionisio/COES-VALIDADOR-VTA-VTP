...
pintarPaginado = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "PaginadoHistoricoSolicitudes",
        data: {
            finicios: finicio,
            ffins: ffin,
...
mostrarListado = function (nroPagina) {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "ListadoHistoricoSolicitudes",
        data: {
            finicios: finicio,
            ffins: ffin,
            tiposolicitud: $('#cbTipoSolicitudes').val(),
...
ExportarExcel = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoReporteHistoricoSolicitudes",
        data: {
            finicios: finicio,
            ffins: ffin,
            tiposolicitud: $('#cbTipoSolicitudes').val(),
        ...
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarExcelHistoricoSolicitudes";
            }
            if (result == -1) {
