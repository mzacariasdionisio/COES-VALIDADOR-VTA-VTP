...
namespace COES.MVC.Extranet.Areas.StockCombustibles.Helper
{
    public class Tools
    {
        ...
        /// <returns></returns>
        public static string[][] ObtenerListaExcelDataPG(StockCombustiblesModel model, List<MeMedicion24DTO> lista24, List<MeCambioenvioDTO> listaCambios, string fecha, int idEnvio)
        {
            int nFil = 27;
        ...
        public static int VerificarIdsFormato(string file, int idEmpresa, int idFormato)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
            {
                ...
                if (ws.Cells[1, ConstantesCombustible.ColEmpresa].Value != null)
                    valorEmp = ws.Cells[1, ConstantesCombustible.ColEmpresa].Value.ToString();
                ...
                if (ws.Cells[1, ConstantesCombustible.ColFormato].Value != null)
                    valorFormato = ws.Cells[1, ConstantesCombustible.ColFormato].Value.ToString();
        ...
        public static string[][] LeerExcelFile(string file, List<MeHojaptomedDTO> listaPto, string fecha)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(fileInfo))
            {
                ...
                for (int i = 0; i < nFil - 3; i++)
                {
                    ...
                    {
                        string valor = (ws.Cells[i + 3 + ConstantesCombustible.FilaExcelData, j + 1 + 1].Value != null) ?
                           ws.Cells[i + 3 + ConstantesCombustible.FilaExcelData, j + 1 + 1].Value.ToString() : string.Empty;
        ...
        public static string GenerarFileExcelConsumo(StockCombustiblesModel model)
        {
            ...
            using (ExcelPackage xlPackage = new ExcelPackage())//, template))
            {
                ...
                row = ConstantesCombustible.FilaExcelData;
        ...
        /// <param name="model"></param>
        /// <param name="ruta"></param>
        public static string GenerarFileExcelPresion(StockCombustiblesModel model)
        {
            string fileExcel = string.Empty;
        ...
        /// <param name="stNumero"></param>
        /// <returns></returns>
        public static string StrNumeroFormatoWithLength(string stNumero, int decimalLength)
        {
            //quitar el caracter final #, el CAMENVDATOS debe terminar en un número
            stNumero = stNumero != null && stNumero.Length > 0 && stNumero[stNumero.Length - 1].ToString() == "#" ? stNumero.Substring(0, stNumero.Length - 1) : stNumero;
            ...
            if (decimal.TryParse(stNumero, out dato))
                numero = dato;
            if (numero != null)
                strNumero = ((decimal)numero).ToString(decimalFormat);
            return strNumero;
        }
