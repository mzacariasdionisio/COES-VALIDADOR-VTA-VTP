...
function crearHandsonTable(evtHot) {
    ...
    function CabColumEVALP(instance, td, row, col, prop, value, cellProperties) {
        Handsontable.renderers.TextRenderer.apply(this, arguments);
        td.style.color = 'MidnightBlue';
        td.style.background = '#3ABE4C';
    }
    ...
    hotOptions = {
        ...
        cells: function (row, col, prop) {
            ...
            if (row < evtHot.FilasCabecera) {
                ...
                if (col > 0) {
                    for (var x = 0; x < evtHot.ListaPtosMME.length; x++) {

                        for (i = 0; i < evtHot.ListaHojaPto.length; i++) {
                            if (evtHot.ListaPtosMME[x].Ptomedicodi == evtHot.ListaHojaPto[i].Ptomedicodi && col == i+1) {
                                render = CabColumEVALP;
                            }
                        }
