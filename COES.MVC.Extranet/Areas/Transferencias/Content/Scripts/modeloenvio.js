    ...
    $.ajax({
        ...
        success: function (result) {
            if (result != -1) {
                ...
                $('#cbModelo').get(0).options.length = 0;
                $('#cbModelo').get(0).options[0] = new Option("--SELECCIONE--", "");
   
                $.each(result.ListaModelos, function (i, item) {
                    $('#cbModelo').get(0).options[$('#cbModelo').get(0).options.length] = new Option(item.TrnModNombre, item.TrnModCodi);
                });               
            }
            else {
                mostrarMensaje('mensaje', 'error', 'Se ha producido un error.');
