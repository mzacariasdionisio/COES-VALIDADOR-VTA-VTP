...
namespace COES.MVC.Extranet.Areas.Transferencias.Controllers
{
   ...
   || x.EMPRCODI == 11772 || x.EMPRCODI == 13).OrderBy(x => x.EMPRNOMB)
      .Select(x => new SiEmpresaDTO() { Emprcodi = x.EMPRCODI, Emprnomb = x.EMPRNOMB }).OrderBy(x => x.Emprnomb).ToList();
        ...
        [HttpPost]
        public JsonResult ObtenerModelo(int empresa)
        {
            ModeloEnvioModel model = new ModeloEnvioModel();
            model.ListaModelos = (new TransferenciasAppServicio()).ListarTrnModeloByEmpresa(empresa);
            return Json(model);
        }
        ...
        public ActionResult Upload(int chunks, int chunk, string name, int empresa, int periodo, int version, int modelo)
        {
            try
            {
        ...
        public ActionResult Cargar1()
        {
            return View();
        }

        public ActionResult Cargar2()
        {
            return View();
        }
