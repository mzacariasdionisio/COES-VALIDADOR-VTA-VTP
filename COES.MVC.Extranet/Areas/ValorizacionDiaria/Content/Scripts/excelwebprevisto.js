...
function inicializaPagina() {
    ...
    $('#btnExpandirRestaurarE').click(function () {
        if ($('#hfExpandirContraerE').val() == "E") {
            expandirExcel();
            calculateSize2(1);
            $('#hfExpandirContraerE').val("C");
...
function dibujarTablaEnviosE(lista) {
    ...
    var cadena = "<div style='clear:both; height:5px'></div> ";
    cadena += "<table id='tablalenvioE' border='1' class='pretty tabla-adicional' cellspacing='0'>";
    cadena += "<thead><tr><th>Id Envío</th><th>Fecha Hora</th><th>Usuario</th></tr></thead>";
    cadena += "<tbody>";
...
function validarConfigPlazoPto() {
    ...
    else if (dataTabActivo == "1") {
        listaPtos = evtHotE.ListaHojaPto;
            ...
            UploadComplete: function (up, file) {
                ...
                if (dataTabActivo == "0" && !evtHot.Handson.ReadOnly) {
                    if (retorno == 1) {
                        ...
                        alert("Por favor presione el botón 'Enviar' para grabar los datos");
...
function getModelFormatoE(accion) {
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            VER_ULTIMO_ENVIO = false;
            VER_ENVIO = false;
            //document.getElementById("mensajeEvento").style.display = "block";
        },
        error: function () {
            alert("Error al cargar Excel Web");
            //evtHot = null;
...
function descargarFormatoE() {
    ...
    $.ajax({
        ...
        data: {
            idEmpresa: $("#cbEmpresaE").val(), //$('#hfIdEmpresa').val(),
            idFormato: iFormatCodi, //ENERGIA PREVISTA A ENTREGAR //$('#hfIdFormato').val(),
            fecha: $('#hfFecha').val(),
            semana: $('#Anho').val() + $('#hfSemana').val(), //$('#hfSemana').val(),
            mes: $('#hfMes').val()
        },
