    ...
    for (var col = 0; col < numCol && !fin; col++) {
        for (var row = 0; row < numFil && !fin; row++) {
            ...
            if (!readOnly) {
                ...
                var regError = obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja);
                if (regError != null) {
                    totalError++;
...
function obtenerErrorGlobal(value, row, col, listaPtos, errores, numFilCabecera, numHoja) {
    var errorReg = null;
    ...
    if (value != null) {
        ...
        if (Number(value)) {
            ...
            if (errores[ERROR_NUMERO_NEGATIVO].validar) {
                if (!document.getElementById("viewCentralPotActiva")) {
                    if (valorNumerico < 0) {
...
//// btnLogHOP
//////////////////////////////////////////////////////////
function exportarLogHOP(numHoja) {
    var data = getData(numHoja);
