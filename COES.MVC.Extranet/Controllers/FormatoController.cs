...
using COES.MVC.Extranet.Areas.IEOD.Helper;
...
namespace COES.MVC.Extranet.Controllers
{
    public abstract class FormatoController : BaseController
    {
        ...
        public JsonResult LeerFileUpExcel(int idEmpresa, string fecha, string semana, string mes, int idFormato, Boolean tieneHojaView = false, int? idHoja = 0)
        {
            ...
            try
            {
                ...
                if (retorno > 0 && idFormato > 0)
                {
                    ...
                    //var listaPtos = servFormato.GetListaPtos(model.Formato.FechaFin, idCfgFormato, idEmpresa, idFormato, cabecera.Cabquery);
                    var listaPtos = new List<MeHojaptomedDTO>();
                    if (idFormato == ConstantesDespacho.FormatoDiarioCodi)
                        listaPtos = servFormato.GetListaPtos(fechaProceso, 0, idEmpresa, idFormato, cabecera.Cabquery);
                    else
                        listaPtos = servFormato.GetByCriteria2MeHojaptomeds(idEmpresa, idFormato, cabecera.Cabquery, fechaProceso, fechaProceso);

                    #region Check para horario seleccionado

                    listaPtos = servFormato.ListaHojaptoCheck(formato.FechaProceso, listaPtos);
        ...
        public void GenerarCabecera(FormatoModel model, List<CabeceraRow> listaCabeceraRow)
        {
            ...
            foreach (var reg in model.ListaHojaPto)
            {
                ...
                for (var w = 0; w < model.FilasCabecera; w++)
                {
                    ...
                    if (reg.Hptoindcheck == "S" && w == 2) //fila unidad
                    {
        ...
        public void BuildHojaExcel(FormatoModel model, int idEmpresa, int idEnvio, string fecha, int idFormato, int verUltimoEnvio)
        {
            ...
            if (model.ValidaHorasOperacion && (ConstantesMedidores.IdFormatoCargaCentralPotActiva != idFormato))
            {
                matrizTipoEstado = ToolsFormato.IncializaMatrizEstado(model.ListaHojaPto, model.FilasCabecera, model.Formato.RowPorDia, -1, model.Formato.Formathorizonte);
            ...
            try
            {
                ...
                switch (formato.Formatresolucion)
                {
                    case ParametrosFormato.ResolucionCuartoHora:
                        ...
                        {
                            //Validación de Horas de Operación vs Medidores
                            if (model.ValidaHorasOperacion && ConstantesMedidores.IdFormatoCargaCentralPotActiva == idFormato)
                            {
                                DateTime fechaConsulta = DateTime.Now;

                                List<LogErrorHOPvsMedidores> listaLog = (new ReporteMedidoresAppServicio()).ListarValidacionHopVsMedidores(formato.FechaProceso, idEmpresa, lista96);

                                if (listaLog.Any())
                                {
                                    List<ReporteHoraoperacionDTO> listaHOP = servHO.ListarReporteHOP(idEmpresa, formato.FechaInicio, formato.FechaFin);

                                    string pathLogo = AppDomain.CurrentDomain.BaseDirectory + RutaDirectorio.PathLogo;
                                    string ruta = (new ReporteMedidoresAppServicio()).GenerarExcelLogErroresHOP(listaLog, base.UserEmail, fechaConsulta, pathLogo, empresa, mes, listaHOP);
                                    string nombreArchivo = "LogErrores_HOP_" + empresa + "_" + mes;

                                    modelResultado.Resultado = -5;
                                    modelResultado.Archivo = nombreArchivo;
                                    modelResultado.DetalleMensaje = ruta;
        ...
        public List<MeMedicion1DTO> ObtenerDatos1(string[][] datos, List<MeHojaptomedDTO> ptos, int checkBlanco, int filCabecera, int lectcodi)
        {
            ...
            for (var i = 1; i < nCol; i++)
            {
                for (var j = filCabecera; j < nFil; j++)
                {
                    ...
                    fecha = DateTime.ParseExact(datos[j][0], datos[j][0].Length > 12 ? Constantes.FormatoFechaHora : Constantes.FormatoFecha, CultureInfo.InvariantCulture);
        ...
        public List<MeMedicion48DTO> ObtenerDatos48(string[][] datos, List<MeHojaptomedDTO> ptos, int checkBlanco, int filCabecera, int lectcodi)
        {
            ...
            bool esFormatoDiario = datos.Length < 96;

            for (var i = 1; i < nCol; i++)
            {
                var regPto = ptos[i - 1];

                //si es formato diario DESPACHO ejecutado, entonce inicializar desde aqui
                if (esFormatoDiario && regPto.Hptoindcheck != "S") 
                ...
                for (var j = filCabecera; j < nFil; j++)
                {
                    ...
                    else
                    {
                        ...
                        if (stValor != null)
                        {
                            valorT = ConstantesFormato.TieneSinCheckExtranet;//unchecked
                            if (stValor == ConstantesFormato.TieneConCheckExtranet) valorT = ConstantesFormato.TieneConCheckExtranet;
                        }
                        reg.GetType().GetProperty("E" + indice.ToString()).SetValue(reg, valorT);
                    }

                    if (indice == 48) //termina de recorrer las primeras 48 filas y guarda el objeto
                    {
                        if (regPto.Hptoindcheck != "S")
                        {
                            lista.Add(reg);
                    ...
                    if (((j - filCabecera) % 48 == 0) && (regPto.Hptoindcheck != "S")) //primera fila
                    {
            ...
            for (var i = 1; i < nCol; i++)
            {
                ...
                for (var j = filCabecera; j < nFil; j++)
                {
                    ...
                    if ((j - filCabecera) % 96 == 0)
                    {
                        ...
                        reg.Lectcodi = lectcodi;
                        reg.Medifecha = new DateTime(fecha.Year, fecha.Month, fecha.Day);
                        reg.Equicodi = ptos[i - 1].Equicodi;
                        reg.Equipadre = ptos[i - 1].Equipadre;
                        reg.Famcodi = ptos[i - 1].Famcodi;
                        reg.Equinomb = ptos[i - 1].Equinomb;
                        reg.Central = ptos[i - 1].Equipopadre;
                    }
                }
            }
            return lista;
            ...
            string areaNombre, DateTime fechaProceso, DateTime fechaEnvio)
        {
            ...
            if (formatoNombre.Equals("Demanda RDO prevista"))
            {
                areaNombre = "Sub Dirección de Coordinación";
                plantilla = servCorreo.GetByIdSiPlantillacorreo(164); //si_plantillacorreo.PLANTCODI de Demanda RDO prevista
            }
            if (plantilla != null)
            {
        ...
        public FormatoResultado GrabarExcelWebGeneracionDespacho(FormatoModel model, int horario)
        {
            ///filtros
            string[][] data = model.Handson.ListaExcelData;
            int idEmpresa = model.IdEmpresa;
            ...
            try
            {
                ...
                listaPto = servFormato.ListaHojaptoCheck(formato.FechaProceso, listaPto);

                //guardar el campo bloquehora cuando la empresa tenga el check para mostrarlo en el reporte de cumplimiento
                int? enviobloquehora = null;
                if (listaPto.Any(x => x.TieneCheckExtranet)) enviobloquehora = horario;
                ...
                string hoy = DateTime.Now.ToString(Constantes.FormatoFecha);
                if (fecha == hoy)
                {
                    ...
                    enPlazo = servFormato.ValidarPlazo(formato);//ValidarFecha(idEmpresa, formato.FechaInicio, idFormato, out mensajePlazo);
                ...
                envio.Fdatcodi = 0;
                envio.Cfgenvcodi = idConfig;
                envio.Enviobloquehora = enviobloquehora;
                int idEnvio = servFormato.SaveMeEnvio(envio);
                modelResultado.IdEnvio = idEnvio;
                ///////////////////////////////////////////////////////
                int horizonte = formato.Formathorizonte;
                ...
                switch (formato.Formatresolucion)
                {
                    case ParametrosFormato.ResolucionMediaHora:
                        ...
                        if (!existePeriodoAnteriorSimilar48)
                        {
                            //grabar
                            servFormato.GrabarValoresCargados48(lista48, User.Identity.Name, idEnvio, idEmpresa, formato);
                            servFormato.GrabarValoresEjecutados48(lista48, User.Identity.Name, idEnvio, idEmpresa, formato);
                        }
                        else
                        {
                            //mandar mensaje
        ...
        public void BuildHojaExcelRDO(FormatoModel model, int idEmpresa, int idEnvio, string fecha, int idFormato, int verUltimoEnvio, string horario, string accion)
        {
            try
            {
                base.ValidarSesionJsonResult();
                ...
                //Mostrar último envio cuando se muestra la interfaz de Carga de datos de un formato
                model.ListaEnvios = this.servFormato.GetByCriteriaMeEnviosxHorario(idEmpresa, idFormato, model.Formato.FechaProceso, horario != null ? Convert.ToInt32(horario) : -1);
                model.ListaEnvios = this.servFormato.GetByCriteriaMeEnviosFormatoEnergPrimaria(model.ListaEnvios, idEmpresa, idFormato, model.IdFormatoNuevo, model.Formato.FechaProceso);

                if (model.ListaEnvios.Count == 0 && idFormato == ConstantesDespacho.FormatoDiarioCodi)
                {
                    for(int hora = Convert.ToInt32(horario) - 3; hora >= 0 && hora < Convert.ToInt32(horario); hora--)
                    {
                        //model.ListaEnvios = this.servFormato.GetByCriteriaMeEnviosUltimoEjecutado(idEmpresa, idFormato, model.Formato.FechaProceso, hora);
                        model.ListaEnvios = this.servFormato.GetByCriteriaMeEnviosxHorario(idEmpresa, idFormato, model.Formato.FechaProceso, hora);
                        if (model.ListaEnvios.Count > 0)
                        {
                            model.ListaEnvios.OrderByDescending(x => x.HorarioCodi).FirstOrDefault();
                            break;
                        }
                ...
                //if (model.ListaEnvios.Count > 0 && idEnvio <= 0)
                if (model.ListaEnvios.Count > 0)
                {
                    model.IdEnvioLast = model.ListaEnvios[model.ListaEnvios.Count - 1].Enviocodi;
                    model.FechaEnvioLast = model.ListaEnvios[model.ListaEnvios.Count - 1].Enviofecha.ToString();
                    if (ConstantesFormato.VerUltimoEnvio == verUltimoEnvio && idEnvio <= 0)
                    {
                        idEnvio = model.IdEnvioLast;
                    }
                    model.IdHorario = model.ListaEnvios[model.ListaEnvios.Count - 1].HorarioCodi;
                }
                ...
                {
                    model.Formato.Emprcodi = idEmpresa;
                    FormatoMedicionAppServicio.GetSizeFormatoRDO(model.Formato);
                    model.EnPlazo = servFormato.ValidarPlazo(model.Formato);
                    model.TipoPlazo = servFormato.EnvioValidarPlazo(model.Formato, idEmpresa);
                ...
                {
                    if (idFormato == ConstantesDespacho.FormatoDiarioCodi && model.Formato.FechaProceso.ToString(Constantes.FormatoFecha) == DateTime.Now.ToString(Constantes.FormatoFecha))
                ...
                model.ListaHojaPto = servFormato.ListaHojaptoCheck(model.Formato.FechaProceso, model.ListaHojaPto);
                ...
                if (idEnvio >= 0) // Es nuevo envio(se consulta el ultimo envio) o solo se consulta envio seleccionado de la BD
                {
                    ...
                    else
                    {
                        ...
                        switch (model.Formato.Formatresolucion)
                        {
                            case ParametrosFormato.ResolucionMediaHora:
                                List<MeMedicion48DTO> lista48 = new List<MeMedicion48DTO>();
                                lista48 = servFormato.GetDataFormato48RDO(idEmpresa, model.Formato, idEnvio, model.IdEnvioLast, horario); //Envío según horario

                                if (lista48.Count == 0)
                                { // Si no existe envío en horario, se muestra el último ejecutado
                                    lista48 = servFormato.GetDataFormato48UltimoEjecutado(idEmpresa, model.Formato, idEnvio, model.IdEnvioLast);
                                }
                                else
                                {
                                    List<MeMedicion48DTO> lista48UltimoEje = servFormato.GetDataFormato48UltimoEjecutado(idEmpresa, model.Formato, idEnvio, model.IdEnvioLast);

                                    foreach (var reg48 in lista48)
                                    {
                                        var reg48Ejec = lista48UltimoEje.Find(x => x.Medifecha == reg48.Medifecha && x.Ptomedicodi == reg48.Ptomedicodi && x.Tipoinfocodi == reg48.Tipoinfocodi);
                                        if (reg48Ejec != null)
                                            for (var i = 1; i <= 48; i++)
                                            {
                                                var valorE =(string) (reg48Ejec.GetType().GetProperty("E" + (i)).GetValue(reg48Ejec, null));
                                                valorE = valorE == "E" ? "E" : "P";
                                                if (valorE.ToString() != "E")
                                                {
                                                    reg48.GetType().GetProperty("E" + (i).ToString()).SetValue(reg48, valorE);
                                                }
