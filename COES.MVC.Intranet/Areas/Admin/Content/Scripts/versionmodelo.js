var controlador = siteRoot + 'admin/versionmodelo/';
var uploaderModelo;
var uploaderManual;
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $('.edit-file').on('click', function () {
                editarFile(id, $(this).attr('data-id'));
            });

            uploaderModelo = configurarUpload(1, 'btnAddModelo', 'containerModelo', 'filelistModelo', 'loadingcargaModelo', true);
            uploaderManual = configurarUpload(2, 'btnAddManual', 'containerManual', 'filelistManual', 'loadingcargaManual', true);
        },
        error: function () {
            mostrarMensaje('mensaje', 'error', 'Se ha producido un error.');
        }
...
editarFile = function (id, indicador) {
    
        $.ajax({
            type: 'POST',
            url: controlador + 'editarchivo',
            data: {
                idVersion: id,
                indicador: indicador
            },
            ...
            success: function (evt) {
                ...
                $('#btnCancelarArchivo').on('click', function () {
...
grabarFile = function (idVersion) {   
    ...
    if ($('#txtNombreArchivo').val() != "") {
        ...
        $.ajax({
            ...
            data: {
                idVersion: idVersion,
                idFile: $('#hfCodigoArchivo').val(),
                nombre: $('#txtNombreArchivo').val(),
                descripcion: $('#txtDescripcionArchivo').val()
            },
            dataType: 'json',
            success: function (result) {
                if (result == 1) {
                    $('#popupFile').bPopup().close();
                    cargarDetalleVersion(idVersion);
                    mostrarMensaje('mensaje', 'exito', 'Operación registrada correctamente.');
                }
                else {
                    mostrarMensaje('mensajeFile', 'error', 'Se ha producido un error.');
                }
            },
            error: function () {
                mostrarMensaje('mensajeFile', 'error', 'Se ha producido un error.');
        ...
        mostrarMensaje('mensajeFile', 'alert', 'Ingrese el nombre de archivo');
    }
};
