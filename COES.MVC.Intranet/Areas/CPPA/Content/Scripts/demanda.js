const controller = siteRoot + "CPPA/Demanda/";
...
$(function () {
    ...
    $('#btnConsultar').click(function () {
        ...
        var cpatdtipo = $('#cbTipoParticipacion').val();
        var cpatdmes = $('#cboMes').val();
        ...
        if (cpatdtipo == '') {
            mostrarAlerta("No selecciono el tipo de participación");
            return;
        }

        if (cpatdmes == 0) {
            mostrarAlerta("No selecciono el mes");
            return;
        }
...
mostrarGrillaExcel = function () {
    ...
    $.ajax({
        ...
        data: {
            ...
            cpatdtipo: $('#cbTipoParticipacion').val(),
            cpatdmes: $('#cboMes').val(),
            cpatdnomtipo: $('#cbTipoParticipacion option:selected').text(),
            cpatdnommes: $('#cboMes option:selected').text()
        },
        dataType: 'json',
...
grabarExcel = function () {
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            IdTipoParticipacion: $('#cbTipoParticipacion').val(),
            Mes: $('#cboMes').val(),
            datos: hot.getData()
        }),
        success: function (result) {
            ...
            else if (result == "ORA-00001: unique constraint (SIC.IDX_CPA_CPATD_UNIQUE) violated") {
                mostrarError('El registro que esta intentando insertar tiene clave duplicada (Revisión-Tipo-Mes). Por favor seleccione una combinacion diferente');
            }
            else {
...
descargarArchivo = function (formato) {
    $.ajax({
        ...
        data: {
            ...
            IdTipoParticipacion: $('#cbTipoParticipacion').val(),
            Mes: $('#cboMes').val(),
            NombTipo: $('#cbTipoParticipacion option:selected').text(),
            NombMes: $('#cboMes option:selected').text(),
...
obtenerDemandaDemandaDet = function (codigo, anio, ajuste, revision, tipo, mes) {
