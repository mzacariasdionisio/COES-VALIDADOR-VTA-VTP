...
var dtEmpresasTransmisoras
var popTransmisor

$(document).ready(function () {
    $('#cboPopEmpresaTransmisora').multipleSelect({
        single: true,
        filter: true
    });

    $('#btnGenNuevoTransmisor').on('click', function () {
        popTransmisor = $('#popupNuevaEmpresaTransmisora').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false
            ...
            function () {
                $.ajax({
                    ...
                    success: function (result) {
                        RefillDropDownListMultiple($('#cboPopEmpresaTransmisora'), result.ListEmpresasTransmision, 'Emprcodi', 'EmprnombConcatenado');
                    },
                    error: function () {
    ...
    $('#btnPopCancelarTransmisora').on('click', function () {
        $("#popupNuevaEmpresaTransmisora").bPopup().close();
    });

    $('#btnPopGuardarTransmisora').on('click', function () {
        registraEmpresaTransmisora();
    });

    $('#btnGenConsultarTransmisor').on('click', function () {
        consultaEmpresasTransmisoras();
    });

    $('#btnGenExportarTransmisor').on('click', function () {
        var myTable = $("#dtListaEmpresasTransmisoras").DataTable();
        ...
        exportarEmpresasTransmisoras(datos);
    });

    consultaEmpresasTransmisoras();
});

function exportarEmpresasTransmisoras(datos) {
...
function consultaEmpresasTransmisoras() {

    const revision = $('#indexIdRevision').val();
    const estado = $('input[name="genEstadoTransmisor"]:checked').val();
    ...
    $.ajax({
        ...
        datatype: 'json',
        success: function (result) {
            dtEmpresasTransmisoras = $('#dtListaEmpresasTransmisoras').DataTable({
...
$(document).off('click', '#dtListaEmpresasTransmisoras tr td .clsEliminar:not(.disabled)')
    .on('click', '#dtListaEmpresasTransmisoras tr td .clsEliminar:not(.disabled)', function (e) {
        e.stopPropagation();

        var data = dtEmpresasTransmisoras.row($(this).parents('tr')).data();
        var confirmacion = confirm('¿Estás seguro de que deseas anular: ' + data.Emprnomb + '?');

        if (confirmacion) {
            anulaEmpresaTransmisora(data.Cpaempcodi, data.Emprcodi, data.Cparcodi);
        }
...
function registraEmpresaTransmisora() {
    const revision = $('#indexIdRevision').val();
    const empresa = $('#cboPopEmpresaTransmisora').val();
...
function anulaEmpresaTransmisora(codigo, empresa, revision) {
