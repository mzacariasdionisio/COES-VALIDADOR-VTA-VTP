...
$(function () {
    ...
    var formularioccgdb = document.getElementById('CCGDB');
    $(formularioccgdb).off('change').on('change', function () {
    ...
    hot3 = new Handsontable(grilla, {
        ...
        nestedHeaders: [
            [
                ...
                { label: 'GENERACIÓN ESTIMADA DE PROYECTO DE <BR> GENERACIÓN DISTRIBUIDA', colspan: 3 }
            ],
            [
                { label: '', colspan: 1 },
                { label: '', colspan: 1 },
...
function grabarCCGDB() {
    if (!validarTablaAntesDeGuardar(hot3)) {
    ...
    if (idProyecto != "0") {
        ...
        param.CCGDBDTOs = getDataCCGDB();
        $.ajax({
            type: 'POST',
            url: controlador + 'GrabarCCGDB',
            data: param,
...
function cargarCCGDB() {
    console.log("Editar");
    if (modoModel == "editar" || modoModel == "consultar") {
        var idProyecto = $("#txtPoyCodi").val();
        $.ajax({
            type: 'POST',
            url: controladorFichas + 'GetCCGDB',
            ...
            success: function (response) {
                ...
                setDataCCGDB(hojaCRes);
                console.log(hojaCRes)
            },
            error: function () {
...
function setDataCCGDB(dataObjects) {
    ...
    for (var r = 0; r < rowCount; r++) {
        ...
        for (var c = 0; c < colCount; c++) {
            row.push((c < 2) ? hot3.getDataAtCell(r, c) : ""); // Mantener Año y Mes, limpiar demás celdas
        }
        emptyData.push(row);
    }
    hot3.loadData(emptyData);
    ...
    dataObjects.forEach(function (obj) {
        var rowIndex = currentData.findIndex(row => row[0] === obj.Anio && row[1] === obj.Mes);
        if (rowIndex !== -1) {
            currentData[rowIndex][2] = formatDecimal(obj.DemandaEnergia);
            currentData[rowIndex][3] = formatDecimal(obj.DemandaHP);
            currentData[rowIndex][4] = formatDecimal(obj.DemandaHFP);
            currentData[rowIndex][5] = formatDecimal(obj.GeneracionEnergia);
            currentData[rowIndex][6] = formatDecimal(obj.GeneracionHP);
            currentData[rowIndex][7] = formatDecimal(obj.GeneracionHFP);
        }
    });
    ...
    // Configuración en modo consulta
    if (modoModel === "consultar") {
        desactivarCamposFormulario('CCGDB');
        $('#btnGrabarCCGDB').hide();
    }
}
...
function exportarTablaExcelCCGDB() {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "CCGDB");

    // Exportar el libro de trabajo a un archivo Excel
    XLSX.writeFile(wb, "GDB_01-EscenarioBase.xlsx");
}


function importarExcelCCGDB() {
...
function updateTableFromExcelBalanceOD(jsonData) {
    ...
    const currentRowCount = hot3.countRows();
    const currentColCount = hot3.countCols();
    ...
    for (let row = 0; row < currentRowCount; row++) {
        for (let col = 0; col < currentColCount; col++) {
            hot3.setCellMeta(row, col, "valid", true);
            hot3.setCellMeta(row, col, "style", {
                backgroundColor: "",
            });
        }
    }
    ...
    const primeraColumna = hot3.getData().map(row => row[0]);
    const segColumna = hot3.getData().map(row => row[1]);


    // Validar y formatear los datos
    const datosValidados = validarYFormatearDatos2(data, configuracionColumnascgdb, mostrarNotificacion, hot3);
    ...
    const stylesBackup = {};
    for (let row = 0; row < hot3.countRows(); row++) {
        for (let col = 0; col < hot3.countCols(); col++) {
            const cellMeta = hot3.getCellMeta(row, col);
    ...
    for (const key in stylesBackup) {
        ...
        hot3.setCellMeta(row, col, "valid", valid);
    }
        ...
        row.forEach((value, colIndex) => {
            ...
            if (config && value !== null && value !== "" && value !== undefined) {
                ...
                if (!esValido) {
                    ...
                    hot3.setCellMeta(rowIndex, colIndex, "valid", false);
                    hot3.setCellMeta(rowIndex, colIndex, "style", {
                        backgroundColor: "#ffcccc", // Rojo claro
                    });
                } else {
                    // Si es válido, limpiar estilo previo
                    hot3.setCellMeta(rowIndex, colIndex, "valid", true);
                    hot3.setCellMeta(rowIndex, colIndex, "style", {
                        backgroundColor: "",
                    });
                }
