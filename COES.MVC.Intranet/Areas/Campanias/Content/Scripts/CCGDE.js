$(function () {
    ...
    var formularioccgde = document.getElementById('CCGDE');
    $(formularioccgde).off('change').on('change', function () {
        cambiosRealizados = true;
    });
});
...
getCCGDE = function () {
    ...
    param.Estudiofactibilidad = parseFloat($("#txtEstudiofactibilidad").val());
    param.Investigacionescampo = parseFloat($("#txtInvestigacionescampo").val()) ;
    param.Gestionesfinancieras = parseFloat($("#txtGestionesfinancieras").val()) ;
    param.Disenospermisos = parseFloat($("#txtDisenospermisos").val());

    param.Obrasciviles = parseFloat($("#txtObrasciviles").val());
    param.Equipamiento = parseFloat($("#txtEquipamiento").val());
    param.Lineatransmision = parseFloat($("#txtLineatransmisio").val());

    param.Administracion = parseFloat($("#txtAdministracion").val());
    param.Aduanas = parseFloat($("#txtAduanas").val());
    param.Supervision = parseFloat($("#txtSupervision").val());
    param.Gastosgestion = parseFloat($("#txtGastosgestion").val());

    param.Imprevistos = parseFloat($("#txtImprevistos").val());
    param.Igv = parseFloat($("#txtIgv").val());
    param.Otrosgastos = parseFloat($("#txtOtrosgastos").val());

    // Obtener los valores calculados de inversi�n total
    param.Inversiontotalsinigv = parseFloat($("#txtInversiontotalsinigv").val());
    param.Inversiontotalconigv = parseFloat($("#txtInversiontotalconigv").val());
    ...
    if (modoModel == "consultar") {
        desactivarCamposFormulario('CCGDE');
        $('#btnGrabarCCGDE').hide();
    }
    return param;
...
function cargarCCGDE() {
    console.log("Editar");
    if (modoModel == "editar" || modoModel == "consultar") {
        var idProyecto = $("#txtPoyCodi").val();
        $.ajax({
            type: 'POST',
            url: controladorFichas + 'GetCCGDE',
            contentType: "application/json; charset=utf-8",
            data: JSON.stringify({ id: idProyecto }),
            
            success: function (response) {
                console.log(response);
                var hojaCCGDE = response.responseResult;
                setCCGDE(hojaCCGDE);
            },
            error: function () {
