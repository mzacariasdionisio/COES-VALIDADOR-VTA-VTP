...
var hotHojacsolar = null;
var valoresListaC = [];
var aniosCabecera = [];
var headerCabe = [];
var catRequisitoCentralSolar = 7;

$(function () {
    cargarCatalogoTablaFichaCSolar(catRequisitoCentralSolar);
   // $('#txtFechaPuertaOperacion').val(obtenerFechaActualMesAnio());
    ...
    $('#btnGrabarSolHojaC').on('click', function () {
        grabarFichaCG4();
    });
    var formularioc = document.getElementById('FichaC');
...
function getDataDetHojaC() {
    var hotInstance = hotHojacsolar.getInstance(); // Obtener la instancia de Handsontable
...
function cargarCatalogoTablaFichaCSolar(id) {
    $.ajax({
        type: 'POST',
        ...
        success: function (eData) {
            valoresListaC = eData;
            tablaHojaCSolar(anioPeriodo, horizonteFin);
        },
        error: function (err) {
...
function tablaHojaCSolar(inicioan, finan) {
    ...
    var grilla = document.getElementById("tablaRequisitoSolar");
    console.log("grilla:", grilla);

    if (typeof hotHojacsolar !== "undefined" && hotHojacsolar !== null) {
        hotHojacsolar.destroy();
    }

    hotHojacsolar = new Handsontable(grilla, {
        data: data,
        // rowHeaders: true,
        colHeaders: true,
    ...
    hotHojacsolar.render();
}

function grabarFichaCG4() {
    var param = {};
    param.TransmisionProyectoDTO = getCabeceraProyecto();
    var idProyecto = $("#txtPoyCodi").val();
    if (idProyecto != "0") {
        param.ProyCodi = idProyecto;
        param.SolHojaCDTO = getDataHojaC();
        param.ListaDetSolHojaCDTO = getDataDetHojaC();
        $.ajax({
            type: 'POST',
            url: controlador + 'GrabarSolHojaC',
            data: param,
...
function cargarDatosC() {
    ...
    if (modoModel == "editar" || modoModel == "consultar") {
        ...
        $.ajax({
            type: 'POST',
            url: controladorFichas + 'GetSolHojaC',
            contentType: "application/json; charset=utf-8",
            ...
            success: function (response) {
                ...
                setDataDetHojaC(hojaCRes.ListaDetSolHojaCDTO);
                cambiosRealizados = false;
            },
            error: function () {
...
setDataHojaC = function (dataResponse) {
    ...
    setFechaPickerGlobal("#txtFechaPuertaOperacion", convertirFechaMesAnio(dataResponse.Fecpuestaope), "mm/aaaa");
    if (modoModel == "consultar") {
        desactivarCamposFormulario('FichaC');
        $("#btnGrabarSolHojaC").hide();
        hotHojacsolar.updateSettings({
