...
function cargarListaTipoProyecto(tipoProyectos) {
    ...
    var a = $('<a>', {
        click: function () {
            ObtenerContenidoFicha('Observaciones');
            highlightTab(this, 'tab-container-1');
        }
    });
...
function aplicarBordes(ws, maxWidth = 50) {
    // Obtener rango de celdas en la hoja de trabajo
    const range = XLSX.utils.decode_range(ws['!ref']);
    // Inicializar anchos de columnas con un valor predeterminado
    const columnWidths = new Array(range.e.c + 1).fill(10);

    // Recorrer cada celda dentro del rango
    for (let R = range.s.r; R <= range.e.r; ++R) {
        for (let C = range.s.c; C <= range.e.c; ++C) {
            // Obtener la dirección de la celda actual
            const cellAddress = XLSX.utils.encode_cell({ r: R, c: C });

            // Si la celda no existe, inicializarla como vacía
            if (!ws[cellAddress]) {
                ws[cellAddress] = { v: "" };
            }

            // Aplicar estilos a la celda: bordes, centrado y ajuste de texto
            ws[cellAddress].s = {
                border: {
                ...
                alignment: {
                    horizontal: "left", // Centrar horizontalmente
                    vertical: "left",   // Centrar verticalmente
                    wrapText: true        // Ajustar texto automáticamente
                }
            };

            // Calcular el ancho necesario para la columna basado en el contenido
            const cellValue = ws[cellAddress].v ? ws[cellAddress].v.toString() : "";
            let calculatedWidth = cellValue.length + 2; // Añadir margen
            columnWidths[C] = Math.min(Math.max(columnWidths[C], calculatedWidth), maxWidth); // Aplicar límite
        }
    }

    // Aplicar anchos calculados a las columnas
...
function exportarTablaExceCheckBox(hotTable, fileName = "HojaC_Datos", sheetName = "HojaC") {
    ...
    aniosCabecera.push("");
    aniosCabecera.push("AÑO");
    aniosCabecera.push("AÑO " + (anioPeriodo - 1) + " o antes");
