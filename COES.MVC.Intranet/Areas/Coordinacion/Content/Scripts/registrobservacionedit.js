...
$(function () {
    ...
    $('#cbTipoObservacion').on('change', function () {
    ...
    $('#btnProcesarSenalesObservadas').on('click', function () {
        procesarSenalesObservadas();
    });

    $('#btnObtenerSenalesObservadas').on('click', function () {
        obtenerSenalesObservadas();
    });
...
seleccionarCanales = function () {
    ...
    var countCanal = $("#hfContador").val();
    canales = $("#hfSeleccion").val();
...
configurarGrilla = function (result) {
    ...
    var widths = [1, 200, 250, 250, 100, 250, 250];
    var columns = [
            { readOnly: true },
            { readOnly: true },
            { readOnly: true },
    ...
    hot.updateSettings({      
        contextMenu: {
            callback: function (key, options) {
                if (key === 'open_busqueda') {
                    openBusquedaCanal();
                    return false;
                }
                else if (key === "history_row") {
                    var selection = this.getSelectedRangeLast();
            ...
            items: {
                "open_busqueda": {
                    name: function () {
                        return " <div class= 'icon-agregar' >Agregar señales</div> ";
                    },
                    disabled: function () {
                        return !flagEdicion;
                    }
                ...
                "remove_row": {
                    name: function () {
                        return " <div class= 'icon-eliminar' >Quitar señal</div> ";
                    },
                    disabled: function () {                      
                        return hot.getSelected()[0] === 0 || !flagEdicion;
                    }
                },
                "history_row": {
                    name: function () {
...
validacion = function () {
    ...
    if ($("#cbTipoObservacion").val() == "") {        
      mensaje = "Seleccione el tipo de observación.";        
    }

    // En caso se seleccione “Observaciones en las señales”, se debe exigir que el usuario ingrese las señales ICCP
    if ($("#cbTipoObservacion").val() == "O")
    {
        if (hot.countRows() == 2) {
            if (hot.getDataAtCell(1, 0) == "")
                mensaje = "Por favor agregue al menos un ICCP.";
        }
    }
...
obtenerSenalesObservadas = function () {
    if ($('#cbEmpresa').val() != "") {        
        ...
        $.ajax({
            type: "POST",
            url: controlador + 'obtenerSenalesObservadas',
            dataType: "json",
            contentType: 'application/json',
            traditional: true,
            data: JSON.stringify({                
...
procesarSenalesObservadas = function () {
    
    $.ajax({
        type: "POST",
        url: controlador + 'procesarSenalesObservadas',
        dataType: "json",
        contentType: 'application/json',
        traditional: true,
        success: function (result) {
