var ht, hc;
var dtPostiva, dtNegativa;

$(document).ready(function () {
    $('#selRabIntervalo').multipleSelect({
        filter: true,
        single: true,
        placeholder: 'HH:mm',  
        onClose: function () {
    ...
    $('#selRabBarra').multipleSelect({
        filter: true,
        single: true,
        placeholder: 'Seleccione barra...',
        onClose: function () {
            obtenerDatos();
        }
    });
    ...
    $('#btnGuardar').click(function () {      
        const _data = ht.getDataAtProp('ajuste');
        registrarDatosABarra(_data);
    });
...
function obtenerDatos() {
    const [_idVersion] = $('#repSelVersion')
        .multipleSelect('getSelects');
    const [_nomVersion] = $('#repSelVersion')
        .multipleSelect('getSelects', 'text');
    const [_idBarra] = $('#selRabBarra')
        .multipleSelect('getSelects');
    const [_nomBarra] = $('#selRabBarra')
        .multipleSelect('getSelects', 'text');
    const [_idIntervalo] = $('#selRabIntervalo')
        .multipleSelect('getSelects');
    ...
    $.ajax({
        type: 'POST',
        url: `${controlador}ObtenerDatosAjusteBarras`,
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            selVersion: (_idVersion) ? _idVersion : -1,
        ...
        success: function (result) {
            ...
            //Crea el highchart
            var hc_title = `Barra corto plazo: ${_nomBarra}`;
...
function registrarDatosABarra(_datos) {
    const [_idVersion] = $('#repSelVersion')
        .multipleSelect('getSelects');
    const [_nomVersion] = $('#repSelVersion')
        .multipleSelect('getSelects', 'text');
    ...
    $.ajax({
        type: 'POST',
        url: `${controlador}RegistrarDatosAjusteBarras`,
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            selVersion: (_idVersion) ? _idVersion : -1,
            selBarra: _idBarra,
...
function obtenerDiferencia() {
    ...
    $.ajax({
        type: 'POST',
        url: `${controlador}ObtenerDifAjusteBarras`,
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            selVersion: (_idVersion) ? _idVersion : -1,
            nomVersion: _nomVersion,
            selFecha: _fecha,
            selIntervalo: _idIntervalo,
            selVersionComp: (_idVersionComp) ? _idVersionComp : -1,
            selFechaComp: _fechaComp,
        }),
        datatype: 'json',
        traditional: true,
        success: function (res) {
            inicialTablaDiffPositiva(res.dataDifPositiva);
            iniciarTablaDiffNegativa(res.dataDifNegativa);
        },
        error: function () {
            alert("Ha ocurrido un problema...");
        }
...
function inicialTablaDiffPositiva(_data) {
    dtPositiva = $('#dtDiffPositiva').DataTable({
        data: _data,
        columns: [
            { title: 'Barra CP', data: 'nombre' },
            { title: 'Dif Positiva', data: 'diferencia' },
        ],        
...
function iniciarTablaDiffNegativa(_data) {
    dtNegativa = $('#dtDiffNegativa').DataTable({
        data: _data,
        columns: [
            { title: 'Barra CP', data: 'nombre' },
            { title: 'Dif Negativa', data: 'diferencia' },
        ],
        createdRow: function (row, data, index) {
...
function formatoDatosAjuste() {
    const datos = ht.getDataAtProp('ajuste');
    let entity = {};
    datos.forEach((item, index) => {
        entity[`H${(index + 1) * 2}`] = item;
    });
    return entity;
}
