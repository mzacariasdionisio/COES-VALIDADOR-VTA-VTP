...
namespace COES.MVC.Intranet.Areas.DemandaCP.Controllers
{
    public class EnvioController : Controller
    {
        ...
        /// Codigo de formato
        /// </summary>
        public int IdFormato = 2;
        public int IdLectura = 47;
        ...
        DemandaPOAppServicio servicioPO = new DemandaPOAppServicio();
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult GenerarFormato(int idEmpresa, int formatoReal, string fecha, string semana, int idVersion)
        {
            int indicador = 0;
            int idEnvio = 0;
            ...
            try
            {
                ruta = AppDomain.CurrentDomain.BaseDirectory + ConstantesDemandaCP.FolderUpload;
                //Assetec - Pronóstico de la demanda Etapa 3
                FormatoModel model = BuildHojaExcel(idEmpresa, formatoReal, IdFormato, idEnvio, fecha, semana, idVersion);
                logic.GenerarFileExcel(model, ruta, ConstantesDemandaCP.FormatoDiarioCodi);
                indicador = 1;
            }
        ...
        public virtual ActionResult DescargarFormato()
        {
            ...
            string ruta = AppDomain.CurrentDomain.BaseDirectory + ConstantesDemandaCP.FolderUpload;
        ...
        public JsonResult LeerFileUpExcel(int idEmpresa, int formatoReal, string fecha, string semana)
        {
            ...
            if (retorno > 0)
            {
                ...
                //
                var listaPtos = this.logic.GetByCriteria2MeHojaptomeds(idEmpresa, IdFormato, cabecera.Cabquery, formato.FechaInicio, formato.FechaFin).Where(x => x.Hojaptoactivo == 1).ToList();
                ...
                Boolean isValido = logic.LeerExcelFile(this.MatrizExcel, this.NombreFile, formato.Formatrows, nBloques, formato.Formatcols, nCol, ConstantesDemandaCP.FormatoDiarioCodi);
            }
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult MostrarHojaExcelWeb(int idEmpresa, int formatoReal, int idEnvio, string fecha, string semana, int idVersion)
        {
            ...
            {
                //Assetec - Pronóstico de la demanda Etapa 3
                FormatoModel jsModel = BuildHojaExcel(idEmpresa, formatoReal, formatoPto, idEnvio, fecha, semana, idVersion);
        ...
        /// <param name="semana"></param>
        /// <param name="mes"></param>
        /// <param name="idVersion">Identificador de la versión de pronóstico</param>
        /// <returns></returns>
        public FormatoModel BuildHojaExcel(int idEmpresa, int idFormato, int formatoPto, int idEnvio, string fecha, string semana, int idVersion)
        {
            ...
            { // los datos para visualizar en el excel web provienen de un archivo excel cargado por el usuario
                ...
                else // los datos vienen de pronostico de demanda
                {
                    //Assetec - Pronóstico de la demanda Etapa 3
                    if (idVersion > 0)
                    {
                        lista = this.servicio.GetDataPronosticoDemandaByVersion(model.Formato.Formatcodi, model.Formato.FechaInicio, model.Formato.FechaFin, idVersion);
                        logic.ObtieneMatrizWebExcel(model, lista, listaCambios, idEnvio);
                    }
                    else
                    {
                        lista = this.logic.GetDataPronosticoDemanda(model.Formato.Formatcodi, model.Formato.FechaInicio, model.Formato.FechaFin);
                        logic.ObtieneMatrizWebExcel(model, lista, listaCambios, idEnvio);
                    }
                }
            }
        ...
        /// <returns></returns>
        public JsonResult ActualizarVersion(string fecha)
        {
            List<PrnVersiongrpDTO> res = this.servicio
                .ListVersionesPronosticoPorFecha(fecha, fecha);
            return Json(res);
        }

        #region Assetec - DemandaPO Iteración 2
        /// <summary>
        ...
        public PartialViewResult Otros()
        {
            FormatoModel model = new FormatoModel();
            ...
            //Assetec - Pronóstico de la demanda Etapa 3
            model.ListaVersionByFecha = this.servicio
                .ListVersionesPronosticoPorFecha(model.Dia,
                model.Dia);
            return PartialView(model);
        }

        public PartialViewResult Reprograma()
        {
            ReprogramaModel model = new ReprogramaModel();
            model.Fecha = DateTime.Now.ToString(Constantes.FormatoFecha);
            string FechaFin = DateTime.ParseExact(model.Fecha,
                Constantes.FormatoFecha, CultureInfo.InvariantCulture)
                .AddDays(1).ToString(Constantes.FormatoFecha);

            model.FechaAnterior = DateTime.Now.AddDays(-1)
                .ToString(Constantes.FormatoFecha);            

            model.ListaVersion = this.servicioPO.VersiongrpListByAreaFecha(
                ConstantesDpo.areaUsuariaDPO, model.Fecha,
                FechaFin);
            model.ListaVersionAnte = this.servicio.ListVersionesPronosticoPorFecha(
                model.FechaAnterior, model.FechaAnterior);
            model.ListaVersionComp = this.servicio.ListVersionesPronosticoPorFecha(
                model.Fecha, model.Fecha);

            return PartialView(model);
        }

        public PartialViewResult Vegetativa()
        {
            ReprogramaModel model = new ReprogramaModel() {
                IdModulo = ConstantesDpo.ReprogramaModVegetativa,
            };
            return PartialView(model);
        }

        public JsonResult ObtenerDatosVeg(int idVersion,
            string nomVersion, string fecha)
        {            
            object res = this.servicioPO.ReprogramaDatosVeg(
                idVersion, nomVersion, fecha);

            return Json(res);
        }

        public JsonResult RegistrarDatosVeg(
            List<DpoMedicion96DTO> datos,
            int version, string fecha)
        {            
            object res = this.servicioPO.ReprogramaRegistrarVeg(
                datos, version, fecha, User.Identity.Name);

            return Json(res);
        }

        public JsonResult ObtenerDatosConf(int idVersion)
        {
            DpoConfiguracionDTO res;
            if (idVersion != -1)
                res = this.servicioPO.GetByVersionDpoConfiguracion(
                    idVersion);
            else
                res = this.servicioPO.GetByIdDpoConfiguracion(
                        ConstantesDpo.DpocngcodiGeneral);

            return Json(res);
        }

        public JsonResult RegistrarConfiguracionVeg(
            DpoConfiguracionDTO datos)
        {
            datos.Dpocngusucreacion = User.Identity.Name;
            datos.Dpocngfeccreacion = DateTime.Now;
            datos.Dpocngusumodificacion = User.Identity.Name;
            datos.Dpocngfecmodificacion = DateTime.Now;
            object res = this.servicioPO.ReprogramaRegistrarConfigVeg(datos);
            return Json(res);
        }

        public JsonResult EjecutarPronosticoVeg(int idVersion, 
            string nomVersion, string fecha, int idVersionAnterior,
            string fechaAnterior, int idVersionComparacion,
            string fechaComparacion)
        {
            object res = this.servicioPO.ReprogramaEjecutarVeg(
                idVersion, nomVersion, fecha, idVersionAnterior,
                idVersionComparacion);
        ...
        public PartialViewResult UsuariosLibres()
        {
            ReprogramaModel model = new ReprogramaModel()
            {
                IdModulo = ConstantesDpo.ReprogramaModUsurioLibre,
            };
            return PartialView(model);
        }

        public JsonResult ObtenerDatosUL(int idVersion,
            string nomVersion, string fecha)
        {
            object res = this.servicioPO.ReprogramaDatosUL(
                idVersion, nomVersion, fecha);

            return Json(res);
        }

        public JsonResult ActualizarDatosUL(int idVersion,
            string nomVersion, string fecha)
        {
            object res = this.servicioPO.ReprogramaActualizarUL(
                idVersion, nomVersion, fecha);

            return Json(res);
        }

        public JsonResult RegistrarDatosUL(
            List<DpoMedicion96DTO> datos,
            int version, string fecha)
        {
            object res = this.servicioPO.ReprogramaRegistrarUL(
                datos, version, fecha,
                User.Identity.Name);
        ...
        public PartialViewResult TotalBarras()
        {
            ReprogramaModel model = new ReprogramaModel()
            {
                IdModulo = ConstantesDpo.ReprogramaModTotalBarra,
            };
            return PartialView(model);
        }

        public JsonResult ObtenerDatosTotalBarras(
            int idVersion, string nomVersion,
            string fecha)
        {
            object res = this.servicioPO.ReprogramaDatosTotalBarra(
                idVersion, nomVersion, fecha);
            return Json(res);
        }

        public PartialViewResult AjusteBarras()
        {
            ReprogramaModel model = new ReprogramaModel();
            model.IdModulo = ConstantesDpo.ReprogramaModAjusteBarra;
            model.ListaIntervalos = UtilDpo.ListaIntervalos(ConstantesDpo.Itv30min);
            model.ListaBarras = this.servicioPO.ListaBarrasFormato();

            return PartialView(model);
        }

        public JsonResult ObtenerDatosAjusteBarras(
            int selVersion, string nomVersion,
            int selBarra, string selFecha, int selVersionComp,
            string selFechaComp)
        {
            object res = this.servicioPO.ReprogramaDatosABarra(
                selVersion, nomVersion, selBarra, selFecha,
                selVersionComp, selFechaComp);
            return Json(res);
        }

        public JsonResult ObtenerDifAjusteBarras(
            int selVersion, string nomVersion, string selFecha,
            int selIntervalo, int selVersionComp,
            string selFechaComp)
        {
            object res = this.servicioPO.ReprogramaDifABarra(
                selVersion, nomVersion, selFecha, selIntervalo,
                selVersionComp, selFechaComp);

            return Json(res);
        }
        public JsonResult RegistrarDatosAjusteBarras(
            int selVersion, int selBarra, 
            string nomBarra, string selFecha,
            decimal[] datos)
        {
            object res = this.servicioPO.ReprogramaRegistrarABarra(
                selVersion, selBarra, nomBarra, selFecha,
                User.Identity.Name, datos);
            return Json(res);
        }

        public PartialViewResult AreasSein()
        {
            ReprogramaModel model = new ReprogramaModel()
            {
                IdModulo = ConstantesDpo.ReprogramaModAjusteArea,
                ListaAreaOperativa = this.servicioPO.ListaAreaOperativa(),
            };
            return PartialView(model);
        }

        public JsonResult ObtenerDatosAreaSein(
            int selArea, int selVersion, string nomVersion,
            string selFecha, int selVersionComp,
            string selFechaComp)
        {
            object res = this.servicioPO.ReprogramaDatosAreaSein(
                selVersion, nomVersion, selArea, selFecha,
                selVersionComp, selFechaComp);
            return Json(res);
        }
        public JsonResult RegistrarDatosAreaSein(
            int selArea, int selVersion,
            string nomVersion, string selFecha,
            decimal[] datos)
        {
            object res = this.servicioPO.ReprogramaRegistrarAreaSein(
                selVersion, nomVersion, selArea, selFecha,
                datos, User.Identity.Name);
            return Json(res);
        }

        public PartialViewResult AjusteSein()
        {
            ReprogramaModel model = new ReprogramaModel();
            model.IdModulo = ConstantesDpo.ReprogramaModAjusteSein;
            model.ListaEscenario = new List<CpTopologiaDTO>();
            return PartialView(model);
        }

        public JsonResult ObtenerEscenarioAjusteSein(string selFecha)
        {
            var res = servicioPO.ObtenerEscenariosYupana(selFecha);
            return Json(res);
        }

        public JsonResult ObtenerDatosAjusteSein(
            int selVersion, int selYupana,
            string selFecha, string selFechaComp)
        {
            object res = this.servicioPO.ReprogramaDatosAjusteSein(
                selVersion, selYupana, selFecha, selFechaComp);
            return Json(res);
        }
        public JsonResult RegistrarDatosAjusteSein(
            int selVersion, string nomVersion,
            string selFecha, decimal[] datos)
        {
            object res = this.servicioPO.ReprogramaRegistrarAjusteSein(
                selVersion, nomVersion, selFecha,
                datos, User.Identity.Name);
            return Json(res);
        }

        public JsonResult CrearVersion(
            int idModulo, string selFecha,
            string selNombre)
        {
            string res = String.Empty;
            res = this.servicioPO.ReprogramaCrearVersion(
                idModulo, selNombre, selFecha,
                User.Identity.Name);

            return Json(res);
        }

        public JsonResult ImportarVersion(int idModulo,
            int idVersionImportar, string selFechaImportar,
            int idVersion, string selFecha)
        {
            object res = this.servicioPO.ReprogramaImportarVersion(
                idModulo, idVersionImportar, selFechaImportar,
                idVersion, selFecha, User.Identity.Name);
            return Json(res);
        }

        public JsonResult ActualizarVersionPO(string fecha)
        {
            List<PrnVersiongrpDTO> res = this.servicioPO
                .VersiongrpListByAreaFecha(ConstantesDpo.areaUsuariaDPO,
                fecha, fecha);
            return Json(res);
        }
        ...
        /// </summary>
        /// <param name="dataExcel"></param>
        /// <param name="dataExcel2"></param>
        /// <param name="fecha"></param>
        /// <param name="nomVersion"></param>
        /// <returns></returns>
        [HttpPost]
        public JsonResult EnviarDatosVeg(string dataExcel, 
            string dataExcel2, string fecha, string nomVersion)
        {
            int idEmpresa = -1;
            int formatoReal = 49;

            //Obtención de número de semana
            DateTime fechaSemana = DateTime.Now;
            int nsemanas = COES.Base.Tools.Util.ObtenerNroSemanasxAnho(fechaSemana, FirstDayOfWeek.Saturday);
            string semana = $"{fechaSemana.Year}{nsemanas}";
            //--------------------------
