var controler = siteRoot + "Despacho/Concepto/";
...
function adjuntarArchivo() {
    ...
    uploaderN = new plupload.Uploader({
        ...
        url: controler + 'UploadConcepto?sFecha=' + comb,
...
function eliminarFile(id) {
    var string = id.split("@");
    var idConcep = string[0];
    var nombreArchivo = string[1];

    uploaderN.removeFile(idConcep);
...
function validarReporte(nameFilem) {
    $.ajax({
        type: 'POST',
        url: controler + "ImportarConceptosExcel",
        datatype: 'json',
        ...
        success: function (evt) {
            if (evt.Resultado != '-1') {
                ...
                if (evt.ListaConceptosErrores.length > 0) {
                    var html = '';
                    html += returnTable();
                    $.each(evt.ListaConceptosErrores, function (i, val) {
                        html += returnBody(val);
                    });
                    html += "</tbody>";
                    html += "</table>";
                    ...
                    if (evt.ListaConceptosCorrectos != 0) {
...
function returnTable() {
    ...
    html += "<th style='text-align:left'>CÓDIGO DE <br/>CONCEPTO</th>";
    ...
    html += "<th style='text-align:left'>CÓDIGO DE CATEGORÍA <br/>DE GRUPO</th>";
    html += "<th style='text-align:left'>NOMBRE DE CATEGORÍA <br/>DE GRUPO</th>";
...
function returnBody(model) {
    var html = '';
    html += '<tr id="number_' + model.Concepcodi + '">';
    html += '<td>' + model.NroItem + '</td>';
    html += '<td>' + model.Observaciones + '</td>';
    html += '<td>' + (model.Concepcodi > 0 ? model.Concepcodi : "") + '</td>';
    html += '<td>' + model.Concepdesc + '</td>';
    html += '<td>' + model.Concepnombficha + '</td>';

    html += '<td>' + model.Concepabrev + '</td>';
    html += '<td>' + model.Concepdefinicion + '</td>';
    html += '<td>' + model.Concepunid + '</td>';
    html += '<td>' + model.Conceptipo + '</td>';
    html += '<td>' + (model.Conceptipolong1 ?? "") + '</td>';

    html += '<td>' + (model.Conceptipolong2 ?? "") + '</td>';
    html += '<td>' + model.Concepfichatec + '</td>';
    html += '<td>' + (model.Catecodi > 0 ? model.Catecodi : "") + '</td>';
    html += '<td>' + model.Catenomb + '</td>';
    html += '</tr>';
