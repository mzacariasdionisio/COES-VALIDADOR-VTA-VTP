...
var IMG_EDITAR = `<img src="${siteRoot}Content/Images/btn-edit.png" title="Editar RSF"/>`;
var IMG_ELIMINAR = `<img src="${siteRoot}Content/Images/Trash.png" title="Eliminar RSF"/>`;
...
function _dibujarTablaListado(model) {
    ...
    var cadena = '';
    cadena += `
    <table class="pretty tabla-icono" id="TablaConsultaRSF" style='table-layout: fixed;' >
        <thead>
            <tr>
                <th>Acciones</th>
    ...
    for (key in lista) {
        ...
        var tdOpciones = _tdAcciones(model, item, sStyle, rowspan, contador);
        ...
        if (contador % 2 == 0) {
            cadena += `
                ${tdOpciones}
                <td rowspan="${rowspan}" style="text-align:center; ${sStyle}">${item.EstadoReserva}</td>
                <td rowspan="${rowspan}" style="text-align:center; ${sStyle}">${item.PrsvfechavigenciaDesc}</td>
                 `;
        }
        else {
            cadena += `                
        ...
        cadena += `
                   <td style="text-align:center; ${sStyle}">${item.TipoReserva}</td>
                   `;

        if (contador % 2 == 0) {
            cadena += `
                <td rowspan="${rowspan}" style="text-align:center; ${sStyle}">${item.Prsvusumodificacion}</td>
                <td rowspan="${rowspan}" style="text-align:center; ${sStyle}">${item.PrsvfecmodificacionDesc}</td>
                 `;
        }
        else {
            cadena += `                
        ...
        for (var key in lstDatos) {
            ...
            cadena += `
            <td rowspan="" style="text-align:right; ${sStyle}">${item2}</td>
            `;
        }
...
function _tdAcciones(Model, item, sStyle, rowspan, contador) {
    var html = '';

    if (contador % 2 == 0) {
        html += `<td rowspan="${rowspan}" style="text-align: left; ${sStyle}">`;
...
function viewEvent() {
    $('.viewEdicion').click(function (event) {
        ...
        mantenerRSF(fechavig, true);
    });
    $('.viewEliminacion').click(function (event) {
        event.preventDefault();
        fechavig = $(this).attr("id").split(",")[1];
        eliminarRSF(fechavig);
    });
};

function mantenerRSF(fechavig, editable) {

    limpiarBarraMensaje("mensaje_rsf");
...
function cargarDatosReserva(fechavig) {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "CargarDatosReserva",
        data: {
        ...
        success: function (evt) {
            ...
            if (evt.Resultado == "1") {
                ...
                mostrarMensaje('mensaje_rsf', 'error', evt.Mensaje);
            }
        },
        error: function (xhr, status) {
            mostrarMensaje('mensaje_rsf', 'error', 'Se ha producido un error.');
        }
    });
...
function cargarDataAHanson(datosEnBd) {
    var tablaHandson;
    var datosSubir, datosBajar = [];
    tablaHandson = tblHndSubir;

    datosSubir = datosEnBd.DatosSubir;
    datosBajar = datosEnBd.DatosBajar;
    ...
    lstData = [
        ...
        { H1: 'RSF UP' },
        {
            H1: datosSubir.H1, H2: datosSubir.H2, H3: datosSubir.H3, H4: datosSubir.H4, H5: datosSubir.H5, H6: datosSubir.H6, H7: datosSubir.H7, H8: datosSubir.H8,
            H9: datosSubir.H9, H10: datosSubir.H10, H11: datosSubir.H11, H12: datosSubir.H12, H13: datosSubir.H13, H14: datosSubir.H14, H15: datosSubir.H15, H16: datosSubir.H16,
            H17: datosSubir.H17, H18: datosSubir.H18, H19: datosSubir.H19, H20: datosSubir.H20, H21: datosSubir.H21, H22: datosSubir.H22, H23: datosSubir.H23, H24: datosSubir.H24,
            H25: datosSubir.H25, H26: datosSubir.H26, H27: datosSubir.H27, H28: datosSubir.H28, H29: datosSubir.H29, H30: datosSubir.H30, H31: datosSubir.H31, H32: datosSubir.H32,
            H33: datosSubir.H33, H34: datosSubir.H34, H35: datosSubir.H35, H36: datosSubir.H36, H37: datosSubir.H37, H38: datosSubir.H38, H39: datosSubir.H39, H40: datosSubir.H40,
            H41: datosSubir.H41, H42: datosSubir.H42, H43: datosSubir.H43, H44: datosSubir.H44, H45: datosSubir.H45, H46: datosSubir.H46, H47: datosSubir.H47, H48: datosSubir.H48
        },
        { H1: 'RSF DOWN' },
        {
            H1: datosBajar.H1, H2: datosBajar.H2, H3: datosBajar.H3, H4: datosBajar.H4, H5: datosBajar.H5, H6: datosBajar.H6, H7: datosBajar.H7, H8: datosBajar.H8,
            H9: datosBajar.H9, H10: datosBajar.H10, H11: datosBajar.H11, H12: datosBajar.H12, H13: datosBajar.H13, H14: datosBajar.H14, H15: datosBajar.H15, H16: datosBajar.H16,
            H17: datosBajar.H17, H18: datosBajar.H18, H19: datosBajar.H19, H20: datosBajar.H20, H21: datosBajar.H21, H22: datosBajar.H22, H23: datosBajar.H23, H24: datosBajar.H24,
            H25: datosBajar.H25, H26: datosBajar.H26, H27: datosBajar.H27, H28: datosBajar.H28, H29: datosBajar.H29, H30: datosBajar.H30, H31: datosBajar.H31, H32: datosBajar.H32,
            H33: datosBajar.H33, H34: datosBajar.H34, H35: datosBajar.H35, H36: datosBajar.H36, H37: datosBajar.H37, H38: datosBajar.H38, H39: datosBajar.H39, H40: datosBajar.H40,
            H41: datosBajar.H41, H42: datosBajar.H42, H43: datosBajar.H43, H44: datosBajar.H44, H45: datosBajar.H45, H46: datosBajar.H46, H47: datosBajar.H47, H48: datosBajar.H48
        }
    ];
...
function iniciarHandsontables() {
    ...
    // Inicio datos
    Handsontable.renderers.registerRenderer('datosIngresadosRsfRenderer', datosIngresadosRsfRenderer);

    // #region Handsontable
    containerTablaSubir = document.getElementById('tblRSF');
    ...
    tblHndSubir = new Handsontable(containerTablaSubir, {
        ...
        mergeCells: [
            { row: 1, col: 0, rowspan: 1, colspan: 48 },
            { row: 3, col: 0, rowspan: 1, colspan: 48 }
        ],
        colWidths: 55,
        height: '160px',
        width: '100%',
    ...
    tblHndSubir.addHook('afterChange', function (changes, [source]) {
        ...
        listaErroresGeneral = listaErroresGeneral.concat(listaErroresBajar.concat(listaErroresSubir));
    });

    //valido existe de dato en RSF subir/ bajar
    function validaRangosRsf() {
        datosGeneral = datosRsfBajar.concat(datosRsfSubir);
        agregarValErrorSinDatos();
    }

    function agregarValErrorSinDatos() {
        if (datosGeneral.length != 96) {
            agregarError("Subir / Bajar", "", "", "Se debe ingresar valor numérico para todas las celdas.");//
        }
    }
        ...
        cells(row, col, prop) {
            ...
            switch (row) {
                ...
                case 4:
                    ...
                    cellProperties.renderer = "datosIngresadosRsfRenderer"; // uses lookup map
                    break;
                default:
            }
...
function datosIngresadosRsfRenderer(instance, td, row, col, prop, value, cellProperties) {
...
function datosIngresadosRenderer(instance, td, row, col, prop, value, cellProperties) {
    ...
    var tipoInfo = "";
    if (row == 2) tipoInfo = "RSF subir";
    if (row == 4) tipoInfo = "RSF bajar";
    ...
    if (value != null) {
        ...
        if (isNaN(value)) {
            ...
            agregarError(tipoInfo, celda, value, "Solo se permiten valores numéricos (con punto como separador decimal) mayores a cero.");
            if (row == 2) {
                datosRsfSubir.push(value);
            }
            if (row == 4) {
                datosRsfBajar.push(value);
            }
        } else {
            ...
            if (value < 0) {
                ...
                if (numPE > 3 || numPD > 3) {
                    td.className = 'celda_error_roja htCenter';
                    agregarError(tipoInfo, celda, value, "Solo está permitido el ingreso de valores con máximo 3 cifras enteras y/o como máximo 3 cifras decimales.");
                    if (row == 2) {
                        datosRsfSubir.push(value);
                    }
                    if (row == 4) {
...
function agregarError(tipoInfo, celda, valor, mensajeError) {
    //Agrega al array de tipo de informacion
    if (validarError(tipoInfo, celda, valor, mensajeError)) {
        var regError = {
            TipoInfo: tipoInfo,
            Celda: celda,
            Valor: valor,
        ...
        if (tipoInfo === "RSF subir")
            listaErroresSubir.push(regError);
        else {
            if (tipoInfo === "RSF bajar")
                listaErroresBajar.push(regError);
            else {
                listaErroresGeneral.push(regError);
            }
...
function validarError(tipoInfo, celda, valor, mensajeError) {
    var arrayData = [];
    arrayData = listaErroresSubir.slice();

    for (var j in arrayData) {
        if (arrayData[j]['TipoInfo'] == tipoInfo && arrayData[j]['Celda'] == celda && arrayData[j]['Valor'] == valor && arrayData[j]['Mensaje'] == mensajeError) {
            return false;
        }
    }
    return true;
...
function dibujarTablaErrores() {
    ...
    //ordeno el listado
    listaErroresGeneral.sort(function (a, b) {
        return b.TipoInfo.localeCompare(a.TipoInfo);
    });
...
function guardarReservas() {
    ...
    var dataHandsonSubir, dataValidoSubir = [];
    ...
    $.ajax({
        url: controlador + "GuardarReserva",
        type: 'POST',
        ...
        success: function (result) {
            ...
            if (result.Resultado == "1") {
                ...
                if (result.Resultado == "2")
                    ...
                    mostrarMensaje('mensaje_rsf', 'error', result.Mensaje);
            }
        },
        error: function (xhr, status) {
            mostrarMensaje('mensaje_rsf', 'error', 'Se ha producido un error.');
...
function eliminarRSF(fechavig) {
    if (confirm("Desea eliminar el registro?")) {
        $.ajax({
            type: 'POST',
            dataType: 'json',
            traditional: true,
            url: controlador + 'EliminarRSF',
            data: {
                fechaVigencia: fechavig
            },
            cache: false,
...
function obtenerDataGuardar() {
    ...
    datosSubir = ObtenerInformacionDeHandson(2);
    datosBajar = ObtenerInformacionDeHandson(4);

    var dataGuardar = {
        DatosSubir: datosSubir,
        DatosBajar: datosBajar
    };
