var controler = siteRoot + "Equipamiento/EquipoPropiedad/";
...
function validarReporte(nameFilem) {
    $.ajax({
        type: 'POST',
        url: controler + "ImportarPropiedadesMasivoExcel",
        datatype: 'json',
        ...
        success: function (evt) {
            if (evt.Resultado != '-1') {
                ...
                if (evt.ListaPropEquiErrores.length > 0) {
                    var html = '';
                    html += returnTable();
                    $.each(evt.ListaPropEquiErrores, function (i, val) {
                        html += returnBody(val);
                    });
                    html += "</tbody>";
                    html += "</table>";
                    ...
                    if (evt.ListaPropEquiCorrectos != 0) {
...
function returnTable() {
    ...
    html += "<th style='text-align:left'>PROPIEDAD</th>";
    html += "<th style='text-align:left'>FECHA DE VIGENCIA</th>";
    html += "<th style='text-align:left'>VALOR</th>";
...
function returnBody(model) {
    ...
    html += '<td>' + model.Propcodi + '</td>';
    html += '<td>' + model.FechapropequiDesc + '</td>';
    html += '<td>' + model.Valor + '</td>';
    html += '<td>' + (model.PropequicheckceroDesc ?? "") + '</td>';
    html += '<td>' + model.Propequicomentario + '</td>';
    html += '<td>' + model.Propequisustento + '</td>';
    html += '</tr>';
