...
function validarReporte(nameFilem) {
    $.ajax({
        type: 'POST',
        url: controler + "ImportarEquiposExcel",
        datatype: 'json',
        ...
        success: function (evt) {
            if (evt.Resultado != '-1') {
                ...
                if (evt.ListaEquiposErrores.length > 0) {
                    var html = '';
                    html += returnTable();
                    $.each(evt.ListaEquiposErrores, function (i, val) {
                        html += returnBody(val);
                    });
                    html += "</tbody>";
                    html += "</table>";
                    ...
                    if (evt.ListaEquiposCorrectos != 0) {
...
function returnTable() {
    ...
    html += "<th style='text-align:left'>CÓDIGO DE <br/>EMPRESA</th>";
    html += "<th style='text-align:left'>CÓDIGO DE <br/>OPERADOR</th>";
    html += "<th style='text-align:left'>CÓDIGO DE <br/>TIPO DE EQUIPO</th>";

    html += "<th style='text-align:left'>CÓDIGO DE <br/>UBICACIÓN</th>";
    html += "<th style='text-align:left'>NOMBRE DE <br/>EQUIPO</th>";
    html += "<th style='text-align:left'>ABREVIATURA</th>";
    html += "<th style='text-align:left'>ABREVIATURA 2</th>";
    html += "<th style='text-align:left'>ESTADO</th>";
    html += "<th style='text-align:left'>Tensión</th>";
    html += "<th style='text-align:left'>CÓDIGO DE <br/>SUBESTACIÓN</th>";
...
function returnBody(model) {
    ...
    html += '<td>' + (model.Emprcodi > 0 ? model.Emprcodi : "") + '</td>';
    html += '<td>' + (model.Operadoremprcodi > 0 ? model.Operadoremprcodi : "") + '</td>';
    html += '<td>' + (model.Famcodi > 0 ? model.Famcodi : "") + '</td>';
    html += '<td>' + (model.Areacodi > 0 ? model.Areacodi : "") + '</td>';

    html += '<td>' + (model.Equinomb ?? "") + '</td>';
    html += '<td>' + (model.Equiabrev ?? "") + '</td>';
    html += '<td>' + (model.Equiabrev2 ?? "") + '</td>';
    html += '<td>' + (model.Equiestado ?? "") + '</td>';

    html += '<td>' + (model.Equitension > 0 ? model.Equitension : "") + '</td>';
    html += '<td>' + (model.Equipadre > 0 ? model.Equipadre : "") + '</td>';
    html += '</tr>';
