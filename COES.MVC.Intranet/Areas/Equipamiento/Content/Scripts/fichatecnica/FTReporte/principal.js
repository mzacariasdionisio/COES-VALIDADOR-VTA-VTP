...
var controladorReporte = siteRoot + 'Equipamiento/FTReporte/';
...
function dibujarTablaReporte(lista, estado, interfaz) {
    ...
    <table border="0" class="pretty tabla-icono" id="tablaEnvios" style="width:100%;">
       <thead>
           <tr>
               <th style='width:40px;white-space: inherit;'>Carpeta</th>
    ...
    cadena += `
               <th style="width:60px;white-space: inherit;">Descargar</th>
        `;
    ...
    for (var key in lista) {
        ...
        cadena += $("#cbEstado option:selected").text();
        ...
        if (interfaz == INTRANET) {
            ...
            cadena += `
                        <td style="width:60px;white-space: inherit;">
                            <a title="Habilitar plazo de revisión" href="JavaScript:Descargar(${item.Ftenvcodi}, ${item.Ftetcodi}, ${item.Ftenvtipoformato});">${IMG_DESCARGAR_EXCEL}</a>
                        </td>
                     `;
        }
...
function Descargar(codigoEnvio, idEtapa, tipoFormato) {

    switch (idEtapa) {
        case ETAPA_CONEXION:
        case ETAPA_INTEGRACION:
            DescargarModificacion(codigoEnvio);
            break;
        case ETAPA_OPERACION_COMERCIAL:
            DescargarRquisito(codigoEnvio);
            break;
...
function DescargarModificacion(codigoEnvio) {

    $.ajax({
        type: 'POST',
        url: controladorReporte + "GenerarFormatoConexIntegModifHistorico",
        data: {
...
function DescargarRquisito(codigoEnvio) {

    $.ajax({
        type: 'POST',
        url: controladorReporte + "GenerarFormatoRevisionContenidoHistorico",
        data: {
            codigoEnvio: codigoEnvio
        },
        success: function (evt) {
