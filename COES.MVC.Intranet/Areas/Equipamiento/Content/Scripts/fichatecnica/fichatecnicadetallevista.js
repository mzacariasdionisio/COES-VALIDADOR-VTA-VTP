...
var OPCION_ACTUAL = 0;
//var OPCION_VISTAPREVIA = 1;
//var OPCION_VISTADATOS = 2;
var TIPO_EQ_FICHA_SELECCIONADO = -2;
var DETALLE_FT_GLOBAL = null;
var IMG_INFO_COMENTARIO = '<img src="' + siteRoot + 'Content/Images/ico-info.gif" title="" width="16" height="16" style ="POSITION: ABSOLUTE;">';
var IMG_OCULTO_ = `<img src="${siteRoot}Content/Images/ContextMenu/menuhidden.png" title="Oculto para el Portal"/>`;
var IMG_OCULTO_EXTRANET_ = `<img src="${siteRoot}Content/Images/ContextMenu/menuhidden.png" title="Oculto para Extranet"/>`;
        ...
        if (hijo.Famcodi != null) {
            ...
            if (listaEquipo_.length > 0) {
                ...
                htmlHijo += `<div class="tabla_hijo titulo">${nombreListaEquipo}</div>
                        ...
                        ${htmlVermasmenos}
                             <thead>
                                ...
                                <th style="width:60px;">Oculto Extranet</th>
                                <th style="width:60px;">Oculto Portal</th>
                ...
                for (var j = 0; j < listaEquipo_.length; j++) {
                    ...
                    var htmlTdOcultoEqPortal = `<td class=""><span id="oculto_${equipo.Equicodi} "></span></td>`;
                    if (equipo.EstadoDesc == "Activo") {

                        if (equipo.FtverocultoPortal == "S") {
                            htmlTdOcultoEqPortal = `<td class=""><span id="oculto_${equipo.Equicodi}"> ${IMG_OCULTO_}</span></td>`;
                        }
                        claseFila = "context-menu-fila-activo";
                    }
                    else {
                        htmlTdOcultoEqPortal = `<td class=""><span id="oculto_${equipo.Equicodi} ">Oculto</span></td>`;
                        claseFila = "context-menu-extranet";
                    }

                    var htmlTdOcultoEqExtranet = `<td class=""><span id="ocultoExt_${equipo.Equicodi} "></span></td>`;
                    if (equipo.FtverocultoExtranet == "S") {
                        htmlTdOcultoEqExtranet = `<td class=""><span id="ocultoExt_${equipo.Equicodi}"> ${IMG_OCULTO_EXTRANET_}</span></td>`;
                    }
                    ...
                    ${htmlTdOcultoEqExtranet}
                    ${htmlTdOcultoEqPortal}
                    <td>
            ...
            if (listaGrupo_.length > 0) {
                ...
                for (var k = 0; k < listaGrupo_.length; k++) {
                    ...
                    var htmlTdOcultogrPortal = `<td class=""><span id="oculto_${grupo.Grupocodi} "></span></td>`;
                    if (grupo.GrupoEstadoDesc == "Activo") {
                        if (grupo.FtverocultoPortal == "S") {
                            htmlTdOcultogrPortal = `<td class=""><span id="oculto_${grupo.Grupocodi}"> ${IMG_OCULTO_}</span></td>`;
                        }
                        claseFilaGr = "context-menu-fila-activo";
                    }
                    else {
                        htmlTdOcultogrPortal = `<td class=""><span id="oculto_${grupo.Grupocodi} ">Oculto</span></td>`;
                        claseFilaGr = "context-menu-extranet";
                    }

                    var htmlTdOcultogrExtranet = `<td class=""><span id="ocultoExt_${grupo.Grupocodi} "></span></td>`;
                    if (grupo.FtverocultoExtranet == "S") {
                        htmlTdOcultogrExtranet = `<td class=""><span id="ocultoExt_${grupo.Grupocodi}"> ${IMG_OCULTO_EXTRANET_}</span></td>`;
                    }
                    ...
                    ${htmlTdOcultogrExtranet}
                    ${htmlTdOcultogrPortal}
                    <td>
...
function cargarEventoHijos() {
    ...
    eventoContextMenuHijosNoActivo();
};
...
function eventoContextMenuHijosActivo() {
    ...
    $('.tboculto').contextMenu({
        ...
        callback: function (key, options) {
            ...
            if (key == "ocultarPort" && equipo.FtverocultoPortal != "S") {
                modificarVisibilidadEqModos(idElement, "S", idElementFicha, 1)
            }
            if (key == "visualizarPort" && equipo.FtverocultoPortal != "N") {
                modificarVisibilidadEqModos(idElement, "N", idElementFicha, 1)
            }
            if (key == "ocultarExt" && equipo.FtverocultoExtranet != "S") {
                modificarVisibilidadEqModos(idElement, "S", idElementFicha, 2)
            }
            if (key == "visualizarExt" && equipo.FtverocultoExtranet != "N") {
                modificarVisibilidadEqModos(idElement, "N", idElementFicha, 2)
            }
        },
        items: objItems
    });
    ...
    $('.tboculto2').contextMenu({
        ...
        callback: function (key, options) {
            ...
            //Actualiza los equipos ocultos
            if (key == "ocultarPort" && grupo.FtverocultoPortal != "S") {
                modificarVisibilidadEqModos(idElement, "S", idElementFicha, 1)
            }
            if (key == "visualizarPort" && grupo.FtverocultoPortal != "N") {
                modificarVisibilidadEqModos(idElement, "N", idElementFicha, 1)
            }
            if (key == "ocultarExt" && grupo.FtverocultoExtranet != "S") {
                modificarVisibilidadEqModos(idElement, "S", idElementFicha, 2)
            }
            if (key == "visualizarExt" && grupo.FtverocultoExtranet != "N") {
                modificarVisibilidadEqModos(idElement, "N", idElementFicha, 2)
            }
        },
        items: objItems2
...
function eventoContextMenuHijosNoActivo() {
    var objItems = {};
    ...
    $('.tboculto').contextMenu({
        ...
        callback: function (key, options) {
            ...
            idElementFicha = $(this).attr("id").split("_")[2];

            //Actualiza los equipos ocultos
            if (key == "ocultarExt") {
                modificarVisibilidadEqModos(idElement, "S", idElementFicha, 2)
            }
            if (key == "visualizarExt") {
                modificarVisibilidadEqModos(idElement, "N", idElementFicha, 2)
            }
        },
        items: objItems
...
function modificarVisibilidadEqModos(idElement, opcion, idElementFicha, tipoOculto) {
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $("#detalle_ficha_tecnica").html('');
            GenerarDetalleFT(tipo, idTipo, idElemento, idFicha, anchoPortal, 1);
        },
        error: function (err) {
