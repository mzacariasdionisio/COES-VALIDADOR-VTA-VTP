...
$(function () {
    ...
    $('#idSubestacion1').multipleSelect({
        width: '100%',
        filter: true,
        single: true
    });

    $('#idSubestacion2').multipleSelect({
        width: '100%',
        filter: true,
        single: true
    });

    $('#idEmpresa').multipleSelect({
        width: '100%',
        filter: true,
        single: true
    });

    $('#idSubestacion1').multipleSelect('setSelects', ['']);
    $('#idSubestacion2').multipleSelect('setSelects', ['']);
    $('#idEmpresa').multipleSelect('setSelects', ['']);
...
let consultar = function () {
    ...
    let idEmpresa = obtenerValorMultiselect('idEmpresa');
    let idSubestacion1 = obtenerValorMultiselect('idSubestacion1');
    let idSubestacion2 = obtenerValorMultiselect('idSubestacion2');

    let idArea = $("#idArea").val();
    let fTension = $("#fTension").val();
    let estado = $("#estado").val();  
    let incluirCalcular = 1;
    setTimeout(function () {
        $.ajax({
            type: 'POST',
            url: controlador + 'ListaLinea',
            data: {
                equicodi: fCodigoId,
                codigo: fCodigo,
                emprcodi: idEmpresa,
                equiestado: estado,
                idsuestacion1: idSubestacion1,
                idsuestacion2: idSubestacion2,
                idarea: idArea,
                tension: fTension,
                incluirCalcular: incluirCalcular
            },
            success: function (evt) {
                ...
                $('#tablaListado').dataTable({
                    "iDisplayLength": 25,
                    columnDefs: [
                        { type: 'date-dd-mm-yyyy', targets: 16 }
                    ],
                    language: {
...
function consultaLinea(e) {

    window.location.href = controlador + "IncluirModificarConsulta?idLinea=" + e ;
}
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $("#btnSeleccionar").click(function () {
                ...
                if (idProyecto == '') {
                    alert("Seleccione una proyecto")
                    return;
                }
                window.location.href = controlador + "IncluirModificar?idLinea=" + e + "&idProyecto=" + idProyecto + "&idEquipo=" + e + "&accion=Modificar";
...
function consultarEquipo(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexConsultarEquipo?codigoId=" + codigoId + "&pathReturn=Evaluacion-Linea-Index";
};

function historialCambio(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexHistorialCambio?codigoId=" + codigoId + "&pathReturn=Evaluacion-Linea-Index";
};
        ...
        success: function (respuesta) {
            ...
            if (!respuesta.Exito) {
                ...
                if (respuesta.RegistrosObservados > 0) {
                    ...
                    setTimeout(function () {
                        $('#idTerrores').html(dibujarTablaError(respuesta.ListaErrores));
...
function excluirEquipo(id) {
    ...
    $.ajax({
        ...
        success: function (resultado) {
            ...
            $("#popupExcluirEquipo").bPopup().close();
            alert("La Línea fue excluida correctamente.");
            consultar();
