...
let consultar = function () {
    ...
    setTimeout(function () {
        ...
        $.ajax({
            type: 'POST',
            url: controlador + 'ListaReactor?codigoId=' + codigoId + "&codigo=" + codigo + "&ubicacion=" + ubicacion + "&empresa=" + empresa + "&area=" + area + "&estado=" + estado + "&incluirCalcular=" + incluirCalcular,
            success: function (evt) {
                $('#lista').html(evt);
                $('#tablaListado').dataTable({
                    "iDisplayLength": 25,
                    columnDefs: [
                        { type: 'date-dd-mm-yyyy', targets: 11 }
                    ],
                    language: {
...
function consultarEquipo(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexConsultarEquipo?codigoId=" + codigoId + "&pathReturn=Evaluacion-Reactor-Index";
};

function historialCambio(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexHistorialCambio?codigoId=" + codigoId + "&pathReturn=Evaluacion-Reactor-Index";
};
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $("#btnSeleccionar").click(function () {
                ...
                window.location.href = controlador + "IncluirModificar?idReactor=" + e + "&idProyecto=" + idProyecto + "&idEquipo=" + e + "&accion=Modificar";
...
function excluirEquipo(id) {
    ...
    $.ajax({
        ...
        success: function (resultado) {
            $("#popupExcluirEquipo").bPopup().close();
            alert("El Reactor fue excluido correctamente");
            consultar();
        },
        error: function () {
            mostrarError();
