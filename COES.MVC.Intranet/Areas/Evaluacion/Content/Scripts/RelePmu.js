...
let consultar = function () {
    ...
    setTimeout(function () {
        $.ajax({
            type: 'POST',
            url: controlador + 'ListaRelePmu?codigoId=' + codigoId + "&codigo=" + codigo + "&subestacion=" + subestacion + "&celda=" + celda + "&empresa=" + empresa + "&area=" + area + "&estado=" + estado,
            success: function (evt) {
                $('#lista').html(evt);
                $('#tablaListado').dataTable({
                    "iDisplayLength": 25,
                    columnDefs: [
                        { type: 'date-dd-mm-yyyy', targets: 8 }
                    ],
                    language: {
...
function consultarEquipo(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexConsultarEquipo?codigoId=" + codigoId + "&pathReturn=Evaluacion-Rele-IndexRelePmu";
};

function historialCambio(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexHistorialCambio?codigoId=" + codigoId + "&pathReturn=Evaluacion-Rele-IndexRelePmu";
};
...
let exportar = function () {
    ...
    setTimeout(function () {
        $.ajax({
            type: 'POST',
            url: controlador + 'ExportarRelePmu?codigoId=' + codigoId + "&codigo=" + codigo + "&subestacion=" + subestacion + "&celda=" + celda + "&empresa=" + empresa + "&area=" + area + "&estado=" + estado,
            success: function (evt) {
                if (evt.Resultado != "-1") {
