...
let consultar = function () {
    ...
    let tipo = $("#fTipo").val();
    let subestacion = $("#fUbicacion").val();
    ...
    setTimeout(function () {
        $.ajax({
            type: 'POST',
            url: controlador + 'ListaTransformador?codigoId=' + codigoId + "&codigo=" + codigo + "&tipo=" + tipo + "&subestacion=" + subestacion + "&empresa=" + empresa + "&area=" + area + "&estado=" + estado + "&tension=" + tension + "&incluirCalcular=" + incluirCalcular,
            success: function (evt) {
                ...
                $('#tablaListado').dataTable({
                    "iDisplayLength": 25,
                    columnDefs: [
                        { type: 'date-dd-mm-yyyy', targets: 12 }
                    ],
                    language: {
...
function consultarEquipo(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexConsultarEquipo?codigoId=" + codigoId + "&pathReturn=Evaluacion-Transformador-Index";
};

function historialCambio(codigoId) {
    window.location.href = siteRoot + "Evaluacion/Transversal/IndexHistorialCambio?codigoId=" + codigoId + "&pathReturn=Evaluacion-Transformador-Index";
};
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $("#btnSeleccionar").click(function () {
                ...
                window.location.href = controlador + "IncluirModificar?idTransformador=" + e + "&idProyecto=" + idProyecto + "&idEquipo=" + e + "&accion=Modificar";
        ...
        success: function (respuesta) {
            ...
            if (!respuesta.Exito) {
                ...
                if (respuesta.RegistrosObservados > 0) {
                    ...
                    setTimeout(function () {
                        $('#idTerrores').html(dibujarTablaError(respuesta.ListaErroresTransformadores));
...
let exportarDatos = function () {   
    ...
    let subestacion = obtenerValorMultiselect('fUbicacion');
    let idEmpresa = obtenerValorMultiselect('fEmpresa');

    let idArea = $("#fArea").val();
    let estado = $("#fEstado").val();
    let fTension = $("#fNivelTension").val();
...
function excluirEquipo(id) {
    ...
    $.ajax({
        ...
        success: function (resultado) {
            $("#popupExcluirEquipo").bPopup().close();
            alert("El Transformador fue excluido correctamente.");
            consultar();
        },
        error: function () {
            mostrarError();
