...
function nuevaRevision(ideo, id, TipoConfig) {
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: controlador + "VerificarRevisionesEstudioEO",
        data: {
            eocodi: ideo
        },
        success: function (evt) {
            if (evt == "" || id != 0) {
                window.location = controlador + "NuevaRevision?eocodi=" + ideo + "&id=" + id + "&TipoConfig=" + TipoConfig;
            }
            else {
                mostrarMensajePopup(evt, "");
            }
        },
        error: function () {
            mostrarError();
...
/*Fin Mejoras EO - EPO*/
function ConfirmarEliminar(id, eocodi) {
    mostrarConfirmacion("¿Desea eliminar el registro seleccionado?", EliminarRevision, id, eocodi);
}
...
function EliminarRevision(id) {
    ...
    $.ajax({
        type: 'POST',
        dataType: 'json',
        url: controlador + "EliminarRevisionEO",
        data: {
            revcodi: id[0]
        },
        success: function (evt) {
            if (evt == "1"){
                ...
                mostrarMensajePopup("Se elimino el registro seleccionado", "");
                window.location = controlador + "revision/" + id[1];
            }
        },
        error: function () {
            mostrarError();
...
function crearHandsonTable(evtHot, flag) {
    ...
    function diasLaborables(d, days) {
        var now = new Date(d);
    ...
    var ampliaciontercer = 0;
    var ampliaciontercerter = 0;
    hotOptions = {
        ...
        beforeChange: function (changes, source) {
            ...
            for (var i = changes.length - 1; i >= 0; i--) {
                ...
                if ($("#hfTipoConfig").val() == 1) {
                    if (celda == "B19") {
                        ampliaciontercer = changes[i][3];
                    }

                    if (celda == "B26") {
                        ampliaciontercerter = changes[i][3];
                    }

                    if (source == "edit") {

                        if (celda == "B6") {
                            evtHot.Handson.ListaExcelData[8][1] = changes[i][3];
                            evtHot.Handson.ListaExcelData[21][1] = changes[i][3];
                        }

                        if (celda == "B18") {
                            var pattern = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;
                            //var arrayDate = changes[i][3].match(pattern);
                            //var dt = arrayDate[2] + "/" + arrayDate[1] + "/" + arrayDate[3];
                            //evtHot.Handson.ListaExcelData[17][1] = diasLaborables(dt, Number(ampliaciontercer)).toLocaleDateString();
                            //var dt = new Date();

                            //alert(diasLaborables(dt, ampliaciontercer).toLocaleDateString());
                            //evtHot.Handson.ListaExcelData[19][1] = diasLaborables(dt, ampliaciontercer);
                    ...
                    if (source == "edit") {
                        ...
                        if (celda == "B6") {
                            evtHot.Handson.ListaExcelData[8][1] = changes[i][3];
                            evtHot.Handson.ListaExcelData[20][1] = changes[i][3];
                        }
                    }
                }
        ...
        afterChange: function (changes, source) {
            ...
            if (arguments[1] == "edit") {
                for (var i = 0; i < arguments[0].length; i++) {
                    if (arguments[0][i][0] == 17 || arguments[0][i][0] == 18) {
                        if (arguments[0][i][0] == 18) {
                            ampliaciontercer = arguments[0][i][3];
                        }
                        var pattern = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;
                        var arrayDate = evtHot.Handson.ListaExcelData[17][1].match(pattern);
                    ...
                    if (arguments[0][i][0] == 24 || arguments[0][i][0] == 25) {
                        if (arguments[0][i][0] == 25) {
                            ampliaciontercerter = arguments[0][i][3];
                        }
                        var pattern = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;
                        var arrayDate = evtHot.Handson.ListaExcelData[24][1].match(pattern);
                        console.log(arrayDate);
                        if (arrayDate == null) return;

                        var dt = arrayDate[2] + "/" + arrayDate[1] + "/" + arrayDate[3];
                        //evtHot.Handson.ListaExcelData[24][1] = diasLaborables(dt, Number(ampliaciontercerter)).toLocaleDateString();
                    }
                }
            }
        },
        cells: function (row, col, prop) {
            ...
            if (evtHot.Handson.ListaExcelData.length > 0) {
                if (evtHot.Handson.ListaExcelData[row][1] == null) {
            ...
            if (evtHot.Handson.ListaExcelData[row][0] == "Fecha Inicio") {
                tipo = 'date';
                formato = 'dd/MM/yyyy';
            }

            if (evtHot.Handson.ListaExcelData[row][0] == "Fecha Fin") {
                tipo = 'date';
                formato = 'dd/MM/yyyy';
            }
...
function crearGrillaFormatoTipo1(modelo) {
    ...
    hotOptions = {
        ...
        cells: function (row, col, prop) {
            ...
            if (modelo.FilasCabecera[row][1] == null) {
