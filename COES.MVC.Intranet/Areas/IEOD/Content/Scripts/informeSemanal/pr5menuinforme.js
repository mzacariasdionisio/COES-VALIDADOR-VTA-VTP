var controlador = siteRoot + 'IEOD/InformeSemanal/';
var SEM_INI = 'Ini';
var SEM_FIN = 'Fin';
...
$(function () {
    ...
    $('#btnPopupNuevaVersion').click(function () {
        if ($("#AnhoIniNuevo").val() == $("#AnhoIni").val())
            $("#cboSemanaIniNuevo").val($("#cboSemanaIni").val());
        $('#txtMotivo').val('');
        setTimeout(function () {
    ...
    $('#Anho' + SEM_INI + 'Nuevo').Zebra_DatePicker({
        format: 'Y',
        onSelect: function () {
            cargarSemanaAnho(SEM_INI + 'Nuevo');
        }
    });
    versionListado();

    //filtros
    $('#Anho' + SEM_INI).Zebra_DatePicker({
        format: 'Y',
        onSelect: function () {
            cargarSemanaAnho(SEM_INI);
        }
    });
    $('#cboSemana' + SEM_INI).change(function () {
        versionListado();
    });

    $('#btnExportar').click(function () {
        if (window.confirm("El proceso de exportación a Excel del Informe Semanal puede tardar algunos minutos. ¿Desea continuar?")) {
            exportarExcel();
        }
    });
...
function cargarSemanaAnho(tipoSem) {
    $('#cboSemana' + tipoSem).unbind();

    $.ajax({
        type: 'POST',
        url: controlador + 'CargarSemana',
        data: {
            idAnho: getAnio(tipoSem),
            tipoSem: tipoSem
        },
        success: function (aData) {
            ...
            if (SEM_INI == tipoSem) {
                $('#cboSemana' + tipoSem).change(function () {
                    versionListado();
                });
                versionListado();
            }
...
function getFecha(tipoSem) {
    var semana = $('#cboSemana' + tipoSem).val();
    return semana;
}
...
function versionGuardar() {
    ...
    //datos
    var fechaPeriodo = $('#cboSemanaIniNuevo').val();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.Resultado == "-1") {
                    alert("Ha ocurrido un error durante la generación de la nueva versión.");
                    console.log(result.Mensaje);
                } else {
                    if ($("#AnhoIniNuevo").val() == $("#AnhoIni").val())
                        $("#cboSemanaIni").val($("#cboSemanaIniNuevo").val());
...
function versionListado() {
    ...
    var fechaPeriodo = $('#cboSemanaIni').val();
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Resultado != "-1") {
                console.log("Informe semanal " + fechaPeriodo);
    ...
    for (key in lista) {
        ...
        cadena += `
            <tr>
                ...
                <td style="height: 24px">
                    <a title="Descargar Excel" href="JavaScript:descargarExcel(${item.Verscodi}, 'E');">${IMG_DESCARGA_EXCEL} </a>
                ...
                <td style="height: 24px">
                    <a title="Descargar log" href="JavaScript:descargarExcel(${item.Verscodi}, 'L');">${IMG_DESCARGA_LOG} </a>
                </td>
...
//por defecto redireccionar a 1.1. Ingreso en Operación Comercial al SEIN
//TODO cambiar a IndexResumenRelevante 
function verNumeral(verscodi) {
    window.location = controlador + 'IndexResumenRelevante?codigoVersion=' + verscodi;
}
...
function descargarExcel(idVersion, tipo) {
    location.href = controlador + 'DescargarArchivoExcel?idVersion=' + idVersion + '&tipo=' + tipo;
}
