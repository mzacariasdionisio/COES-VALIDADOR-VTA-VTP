...
function cargarHansonWeb(irptcodi) {
    ...
    $.ajax({
        type: 'POST',
        url: CONTROLADOR_URL + "HandsonCargaHistoricoTermico",
...
function exportarCargaHistorica() {
    ...
    $.ajax({
        type: 'POST',
        url: CONTROLADOR_URL + "ExportarCargaHistoricoTermico",
...
function GuardarCargaHistorica() {
    ...
    $.ajax({
        type: 'POST',
        url: CONTROLADOR_URL + "GuardarCargaHistoricoTermico",
        data: { stringJson: JSON.stringify(dataArray) },
        datatype: 'json',
        success: function (evt) {
            if (evt.Resultado != "-1") {
...
function inicializarHansonTable(hot) {
    ...
    tblCargarHistorico.addHook('afterValidate', function (isValid, value, row, prop, source) {
        ...
        if (!isValid) {
            ...
            var error = [];
            var propiedad = prop.split(".");
            var periodo = moment(propiedad[0], "YYYYMM").format("MMM YYYY");

            error = {
                address: `[<b>${periodo} - ${propiedad[1]}</b>] [<b>${row + 1}</b>]`,
                valor: value,
        ...
        Object.entries(items).forEach(item => {
            ...
            if (item[1]) { //Valor propiedad
                ...
                if (Number.isInteger(column)) {//Periodo
                    ...
                    var periodo = items[`${column}`];

                    indreporte.Periodo = column;
                    indreporte.Tothorasf = periodo.Fortuito;
                    indreporte.Tothorasp = periodo.Program;
...
function getSchema(lstColumn) {
    var schema = {};
    lstColumn.forEach(function (item) {
        schema[item.data] = null;
    });
    return schema;
}
...
function importarCargaHistorica() {
    ...
    var uploader = new plupload.Uploader({
        ...
        url: CONTROLADOR_URL + "UploadCargaHistoricoTermo",
...
function guardarFechaMaxCargaHistorica() {
    $.ajax({
        type: 'POST',
        url: CONTROLADOR_URL + "GuardarFechaMaxTermo",
        data: {
            mes: $('#fechaMaxMes').val()
        },
        datatype: 'json',
