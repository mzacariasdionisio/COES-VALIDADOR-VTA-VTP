...
$(function () {
    ...
    //if (cuadro != CUADRO2)
    //    $("#div_cambios_cuadro1").show();

    $('#tab-container').bind('easytabs:after', function () {
        refrehDatatable();
    });

    $('#tab-container').easytabs('select', '#tabFort');
    ...
    cargarHansonWeb();
    if (cuadro != CUADRO2)
        inicializarFormulariosCambiosCuadro1();
    else {
        $(".class_fila_parcial").show();
        inicializarFormulariosCambiosCuadro2();
    }
});
...
function cargarHansonWeb() {
    ...
    var container1 = document.getElementById('detalleFort');
    var container2 = document.getElementById('detalleProg');
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "ViewCargarExcelWeb",
        data: {
            irptcodi: $("#hfReporteVersion").val(),
            centralIntegrante: $('#cbCentralIntegrante').val(),
            empresa: getEmpresa(),
        ...
        success: function (evt) {
            if (evt.Resultado != "-1") {
                for (var tab = 0; tab < 2; tab++) {
                    var handson = (tab == 0) ? evt.HandsonFort : evt.HandsonProg;
                    var container = (tab == 0) ? container1 : container2;

                    if (cuadro == CUADRO2)
                        crearGrillaExcelCuadro2(tab, container, handson, ALTURA_HANDSON);
                    else
                        crearGrillaExcel(tab, container, handson, ALTURA_HANDSON);
                }
...
function refrehDatatable() {
    ...
    $('#tabla_reporte_consolidado' + cuadro).dataTable({
        "sPaginationType": "full_numbers",
        "destroy": "true",
        "ordering": false,
        "searching": true,
