...
$(function () {
    ...
    $('#tab-container').easytabs('select', '#tabReporte');
    ...
    cargarHansonWeb();
    inicializarFormulariosCambiosCuadro5();
});
...
function cargarHansonWeb() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "ViewCargarExcelWebCuadro5",
        data: {
            irptcodi: $("#hfReporteVersion").val(),
            centralIntegrante: $('#cbCentralIntegrante').val(),
            empresa: getEmpresa(),
        ...
        success: function (evt) {
            if (evt.Resultado != "-1") {
                crearGrillaExcelCuadro5(0, container1, evt.HandsonDisp, ALTURA_HANDSON);
                if (evt.ListaNota != null && evt.ListaNota.length > 0) {
                    var str = '';
                    for (var i = 0; i < evt.ListaNota.length; i++)
                        str += (evt.ListaNota[i] + " <br/>")
                    $("#nota_cuadro5").html("<strong>Nota (*): </strong>" + str);
                }
