...
function mostrarListado() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "reportes/ListaCapImpor",
        data: {
...
function generarGrafico() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "reportes/GeneraGraficoCapacImport",
        data: {
            idVersion: $('#hfVersion').val(),
            idHorizonte: $('#hfHorizonte').val(),
            fechaInicial: $('#FechaDesde').val(),
...
graficoRepImpoExpSt = function (result) {
    var nseries = [];
    for (var z in result.Grafico.SerieDataS) {
        nseries[z] = [];
        for (k = 0; k < result.Grafico.SerieDataS[z].length; k++) {
            var seriePoint = [];
            var now = parseJsonDate(result.Grafico.SerieDataS[z][k].X);
            ...
            seriePoint.push(result.Grafico.SerieDataS[z][k].Y);
            nseries[z].push(seriePoint);
        }
    }
    var opcion = {
        rangeSelector: {
    ...
    for (var i in result.Grafico.SerieDataS) {
        opcion.series.push({
...
function exportarExcel() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "reportes/GenerarArchivoCapacInter",
        data: {
            idVersion: $('#hfVersion').val(),
            idHorizonte: $('#hfHorizonte').val(),
            fechaInicial: $('#FechaDesde').val(),
