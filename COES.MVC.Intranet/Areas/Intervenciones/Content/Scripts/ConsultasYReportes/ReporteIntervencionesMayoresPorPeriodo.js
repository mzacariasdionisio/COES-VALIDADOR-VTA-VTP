...
function mostrarLista(nroPagina) {
    var paginaActual = $('#hfNroPagina').val(nroPagina);
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "RptIntervencionesMayoresPorPeriodoListado",
        data: {
...
function generarExcel() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + 'GenerarExcelRptIntervencionesMayoresPorPeriodo',
        data: {
            tipoProCodi: tipoProCodi, emprCodi: emprCodi,
            fechaInicio: toDate(fechaInicio).toISOString(), fechaFin: toDate(fechaFin).toISOString()
        },
