...
var PLANTILLA_FORMULARIO_INCLUSION = null;
var PLANTILLA_FORMULARIO_EXCLUSION = null;
...
function cargarEventosJs() {
    ...
    $("#finhora").on("change paste keyup", function () {
        if ($(this).val() == "24" && $("#finminuto").val() == "00") {
    ...
    $("#justificacion").on('change', function () {
        $("#justificacion").val(validarTextoSinSaltoLinea($("#justificacion").val()));
    });

    //Nota
    $("#idNota").unbind();
    $('#idNota').keypress(function (e) {
        if (e.which == 13) {
            e.preventDefault();
            return false;
        }
    });
    $("#idNota").on('change', function () {
        $("#idNota").val(validarTextoSinSaltoLinea($("#idNota").val()));
    });

    //Ver modificación de intervención
    $("#btnVerModificacion").unbind();
    $("#btnVerModificacion").click(function () {
        var intercodi = parseInt($('#InterCodi').val()) || 0;
...
function generarHtmlFormularioIntervencionFlotante(model) {
    ...
    if (model.AccionNuevo) {
        ...
        htmlTitulo += `
        Editar Intervención <span style="color: black; font-size: 10px;">ID ${model.IdIntervencion}</span>
        `;
    }
    else {
        htmlTitulo += `
        Ver Intervención <span style="color: black; font-size: 10px;">ID ${model.IdIntervencion}</span>
        `;
    }

    var htmlBotonAccion = "";
    if (!model.EsDesabilitado) {
        htmlBotonAccion += `   <input type="button" value="" id="btnActualizar" style="float: right;">
                <input type="button" value="Confirmar" id="confirmarEnvio" style="float: right; display: none">
   `;
    }
    var htmlBotonInfoModif = "";
    if (!model.AccionNuevo)
        htmlBotonInfoModif += `<input type="button" value="Info. modificaciones" id="btnVerModificacion">`;

    var htmlAlerta = "";
    if (!model.TienePermisoSPR) {
        htmlAlerta += `
            <div class="action-alert">Usted solo tiene permiso para visualizar la intervención. Si desea crear o editar la intervención debe ingresar nuevamente a través de la opción del menu.</div>
            `;
    }
                ...
                <tr class="fila_val_intervenciones" style="display: none;">
                    <td colspan="2">
                        <table style="margin-top: 0px; margin-bottom: 0px;">
                            <tr>
                                <td>
                                    <div id="div_msj_val_intervenciones" style="text-indent: 0px; margin-bottom: 0; margin-top: 0;"></div>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                    <i>Presione el botón <b>Confirmar</b> para guardar la intervención, esta acción enviará un correo electrónico al Administrador de Horas de Operación.</i>
                                </td>
                            </tr>
                        </table>
                    </td>
...
<input type="hidden" id="hfConfirmarValInterv" name="FlagConfirmarValInter" value="0" />
...
<input type="hidden" id="EsRevertido" value="${model.EsRevertido}" />
...
<table id="tablaDatos" cellpadding="5" style='width: 100%;'>
    <thead>
        ...
        <tr class="fila_form_reg">
            <td style='width: 105px;'>Tipo Intervención:</td>
            <td>
            ...
            <td style="width: 114px;">Clase Programación:</td>
            <td>
        ...
        <tr class="fila_form_reg">
            ...
            <td rowspan="2" colspan="6">
                ...
                <div>
                    <input type="checkbox" value="" id="ChkIntermantrelev"> <span>Mantenimiento Relevante</span>

                    <input type="checkbox" value="" id="ChkIntersistemaaislado" style='margin-left: 17px;'> <span>Sistema Aislado</span>
                </div>
            </td>
        </tr>
        <tr class="fila_form_reg">
            ...
            <td class="editor-label" id="fechaFin" style="width: 100px; background-color:white;padding-top: 0px;">
                ...
                <table style="width:30px;float: right;">
                    <tr>
                        <td style="padding-top: 0px">
                            <input type="text" id="finhora" value="" style="background-color: white; width: 30px" min="0" max="24" maxlength="2" autocomplete="off" />
                        </td>
                        <td style="padding-top: 0px"><strong>:</strong></td>
                        <td style="padding-top: 0px">
...
<!--Segunda seccion -->
        ...
        <tr class="fila_form_reg" id="trNota" hidden>
            <td>Nota:</td>
            <td colspan="5">
                <textarea id="idNota" name="Internota" value="" style="background-color: white; width:100%; height:44px"></textarea>
            </td>
        </tr>
        <tr class="fila_form_reg" id="trSustento" hidden>
            <td></td>
            <td colspan="5">
                <input type="button" value="Ver Sustento de Inclusión" id="btnSustentoInclPopup" onclick="st_CargarFormularioSustento('${TIPO_INCLUSION}', 0)">
                <input type="button" value="Ver Sustento de Exclusión" id="btnSustentoExclPopup" onclick="st_CargarFormularioSustento('${TIPO_EXCLUSION}', 0)">
            </td>
        </tr>
    </thead>
...
function inicializarVistaFormulario() {
    ...
    switch (OPCION_ACTUAL) {
        case OPCION_VER:
            ...
            document.getElementById('nomOperador').style.backgroundColor = '';
            ...
            $("#trNota").show();
            $("#idNota").prop('disabled', 'disabled');
            document.getElementById('idNota').style.backgroundColor = '';
            ...
            $("#idNota").val(INTERVENCION_WEB.Internota);
        ...
        case OPCION_NUEVO:
            ...
            if (!INTERVENCION_GLOBAL.EsCruzadas) {
                $("#cboTipoProgramacionPopup").prop('disabled', 'disabled');
                $("#cboProgramacionPopup").prop('disabled', 'disabled');
            }
            _setearFechasDescProgramacion(INTERVENCION_GLOBAL);
...
function inicializaDesplegables() {
    ...
    if (INTERVENCION_GLOBAL.EsCruzadas && OPCION_ACTUAL == OPCION_NUEVO) {
        $('#cboTipoProgramacionPopup').empty();
        var optionTipoPrograma = '';
        $.each(INTERVENCION_GLOBAL.ListaTiposProgramacion, function (k, v) {
            optionTipoPrograma += '<option value =' + v.Evenclasecodi + '>' + v.Evenclasedesc + '</option>';
        })
        $('#cboTipoProgramacionPopup').append(optionTipoPrograma);

        $('#cboProgramacionPopup').empty();
        var optionProgramacion = '';
        $.each(INTERVENCION_GLOBAL.ListaProgramaciones, function (k, v) {
            optionProgramacion += '<option value =' + v.Progrcodi + '>' + v.ProgrnombYPlazoCruzado + '</option>';
        })
...
function _guardarIntervencionBD() {
    ...
    var envioconfirmado = $("#hfConfirmarValInterv").val();
    $.ajax({
        type: 'POST',
        dataType: 'json',
        traditional: true,
        ...
        success: function (result) {
            if (result.Resultado != '-1' && result.IdIntervencion > 0) {
                ...
                if (INTERVENCION_GLOBAL.EsCruzadas)
                    mostrarGrillaExcel(INTERVENCION_GLOBAL.TieneCeldaSelec); //actualizar fila modificada del Excel web de intervenciones cruzadas
                else {
                ...
                if (result.Resultado == "2") {
                    $("#confirmarEnvio").show();
                    interv_mostrarAdvertenciaHoraOperacion(result.Entidad.Interindispo, result.ListaValidacionHorasOperacion);
                } else {
...
function ActualizarIntervencionJson() {
    ...
    if (OPCION_ACTUAL == OPCION_NUEVO) {
        ...
        if (INTERVENCION_GLOBAL.EsCruzadas) {
            var programacion = $('#cboProgramacionPopup').val();
            $("#ProgrcodiForm").val(programacion);
            INTERVENCION_WEB.Progrcodi = $("#ProgrcodiForm").val();
        }
        ...
        INTERVENCION_WEB.Internota = $("#idNota").val();

        INTERVENCION_WEB.Estadocodi = 1;// Estadocodi por defecto
        INTERVENCION_WEB.Interprocesado = 0;// Interprocesado por defecto
        ...
        if ($("#EsRevertido").val() == 'value') {
            INTERVENCION_WEB.Estadocodi = 2;
            INTERVENCION_WEB.Interprocesado = 2; // revertido
        }
    }
    if (OPCION_ACTUAL == OPCION_EDITAR) {
        INTERVENCION_WEB.Tipoevencodi = $("#cboTipoEventoPopup").val();
        ...
        //INTERVENCION_WEB.Estadocodi = $("#estado").val();
    }
...
function validarIntervencionFormulario(obj) {
    ...
    obj.Progrcodi = parseInt(obj.Progrcodi) || 0;
    if (obj.Progrcodi <= 0) {
    ...
    if (parseInt(obj.FinHora) <= 24 && parseInt(obj.IniHora) <= 23 && parseInt(obj.FinMinuto) <= 59 && parseInt(obj.IniMinuto) <= 59) {
        ...
        if (obj.FinFecha == null || obj.FinFecha == '') {
            ...
            if (formatFecha(obj.FinFecha, obj.FinHora, obj.FinMinuto) > fechaFinProg) {
                ...
                //NO SE VALIDA EJECUTADOS EN INTRANET
            }
        }
        //............................FECHA INICIO.........................
    ...
    //............................Nota.............................
    if (obj.Internota.length > 1000) {
        validacion = validacion + "<li>Nota: El texto supera los 1000 caracteres.</li>";//supera 500 caracteres
        flag = false;
    }
...
function obtenerFechaOperacionPopup() {
    var programacion = parseInt($('#cboProgramacionPopup').val()) || 0;
    $("#ProgrcodiForm").val(programacion);
    ...
    if (programacion > 0) {
        $.ajax({
            ...
            success: function (model) {
                _setearFechasDescProgramacion(model);
            },
            error: function () {
...
function listarProgramacionesXHorizonte() {
    var progrcodi = parseInt($('#cboTipoProgramacionPopup').val()) || 0;
    ...
    if (progrcodi != "") {
        $.ajax({
            ...
            datatype: "json",
            contentType: 'application/json',
            data: JSON.stringify({ idTipoProgramacion: progrcodi }),
            ...
            success: function (evt) {
                ...
                $('#cboProgramacionPopup').append(option);

                obtenerFechaOperacionPopup();
            },
            error: function () {
                alert("Lo sentimos, se ha producido un error");
            }
        ...
        $("#cboProgramacionPopup").empty();
        var option = '<option value="">----- Seleccione un tipo de programación ----- </option>';
        $('#cboProgramacionPopup').append(option);
    }
}
//..................................................................................................................
...
///////////////////////////////////////////////////////////////////////////////////////////////////
function interv_mostrarAdvertenciaHoraOperacion(tipoDisp, listaVal) {
    if (listaVal.length > 0) {
        ...
        if (tipoDisp == "F") {
            ...
            //Intervenciones
            for (var j = 0; j < listaVal.length; j++) {
                var regInterv = listaVal[j];
                ...
                var subcausa = regInterv.Subcausadesc;
