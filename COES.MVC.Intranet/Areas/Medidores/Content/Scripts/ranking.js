    ...
    if ($('#FechaConsulta').val() != "") {
        $.ajax({
            ...
            success: function (evt) {
                ...
                evolucion($('#hfTipoEmpresa').val(), $('#hfEmpresa').val(), $('#hfTipoGeneracion').val(), $('#cbCentral').val());
...
ordenamiento = function(fecha)
{
    $.ajax({
        ...
        success: function (result) {                  
            plotear(result.ListaDemandaDiaGeneracion);
...
plotear = function (result)
{
    ...
    for (var i in json) {
        ...
        jsondata.push([variable, parseFloat(json[i].ValorGeneracion)]);
        count++;
    }
