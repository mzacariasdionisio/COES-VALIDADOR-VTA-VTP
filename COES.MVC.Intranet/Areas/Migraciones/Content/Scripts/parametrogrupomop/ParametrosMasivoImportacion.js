var controler = siteRoot + "Migraciones/Parametro/";
...
$(document).ready(function ($) {
    ...
    $('#btnRegresar').click(function () {
        var repcodi = parseInt($("#hdRepcodi").val()) || 0;
        window.location.href = controler + "Index" + (repcodi > 0 ? "?repcodi=" + repcodi : "");
    });
...
function adjuntarArchivo() {
    ...
    uploaderN = new plupload.Uploader({
        ...
        url: controler + 'UploadParametro?sFecha=' + comb,
...
function validarReporte(nameFilem) {
    $.ajax({
        type: 'POST',
        url: controler + "ImportarParametrosMasivoExcel",
        datatype: 'json',
        data: JSON.stringify({ filename: nameFilem }),
        contentType: "application/json",
        success: function (evt) {
            if (evt.Resultado != '-1') {
                $('#Message').html("<div class='action-exito mensajes'>" + evt.Mensaje + "</div>");
                $("#listado").html("");
            } else {
                alert(evt.Mensaje);
                ...
                if (evt.ListaGrupoDatErrores.length > 0) {
                    var html = '';
                    html += returnTable();
                    $.each(evt.ListaGrupoDatErrores, function (i, val) {
                        html += returnBody(val);
                    });
                    html += "</tbody>";
                    html += "</table>";
                    ...
                    if (evt.ListaGrupoDatCorrectos != 0) {
                        $('#Message').html("<div class='action-message mensajes'>" + evt.Mensaje + "</div>");
                    } else {
                        $('#Message').html("<div class='action-error mensajes'>" + evt.Mensaje + "</div>");
                    }
                }
...
function returnTable() {
    ...
    html += "<th style='text-align:left'>FECHA DE VIGENCIA</th>";
    html += "<th style='text-align:left'>FORMULA</th>";
    html += "<th style='text-align:left'>VALOR CERO(0) CORRECTO</th>";
    html += "<th style='text-align:left'>COMENTARIO</th>";
    html += "<th style='text-align:left'>SUSTENTO</th>";
...
function returnBody(model) {
    ...
    html += '<td>' + model.FechadatDesc + '</td>';
    html += '<td>' + model.Formuladat + '</td>';
    html += '<td>' + (model.GdatcheckceroDesc ?? "") + '</td>';
    html += '<td>' + model.Gdatcomentario + '</td>';
    html += '<td>' + model.Gdatsustento + '</td>';
    html += '</tr>';
