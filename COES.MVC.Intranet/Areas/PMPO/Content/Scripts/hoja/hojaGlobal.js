...
function generarListaErroresFromDataEstandar(numHoja) {
    ...
    var numColCabecera = evt.Handson.ColCabecera;
    var listErrores = [];
    var totalError = 0;
    var errores = getErrores(numHoja);
    ...
    for (var col = 0; col < numCol && !fin; col++) {
        for (var row = 0; row < numFil && !fin; row++) {
            ...
            if (row >= numFilCabecera && row <= numFil && col >= numColCabecera && col <= numCol) {
            ...
            if (!readOnly) {
                ...
                var regError = obtenerErrorGlobal(value, row, col, listaPtos, errores, numColCabecera, numHoja);
                if (regError != null) {
                    totalError++;
...
function obtenerErrorGlobal(value, row, col, listaPtos, errores, numColFijo, numHoja) {
    var errorReg = null;
    var limiteInf = listaPtos[col - numColFijo].Hojaptoliminf;
    var limiteSup = listaPtos[col - numColFijo].Hojaptolimsup;
    var tipoCentral = listaPtos[col - numColFijo].Famcodi;
...
//// btnVerFeriados
//////////////////////////////////////////////////////////
function popupFeriados(numHoja) {
    $('#idTablaFeriados').html(dibujarTablaFeriado(numHoja));
    setTimeout(function () {
        $('#popupVerFeriados').bPopup({
            easing: 'easeOutBack',
            speed: 450,
            transition: 'slideDown',
            modalClose: false
...
function dibujarTablaFeriado(numHoja) {
    var htmlTabla = '';

    if (!getTieneHojaView(numHoja)) {
        //htmlTabla = dibujarTablaFeriadosEstandar(numHoja);
    } else {
        htmlTabla = dibujarTablaFeriadoHojaFormato(numHoja);
    }

    return htmlTabla;
}

function dibujarTablaFeriadoHojaFormato(numHojaPrincipal) {
    var cadena = "";
    cadena += `
    <div style='clear:both; height:5px'></div> 
    <table id='tablaFeriados' border='1' class='pretty tabla-adicional' cellspacing='0'>
        <thead>
            <tr> 
                <th>Hoja</th> 
                <th>Fecha</th>
    ...
    for (var nh = 0; nh < lista.length; nh++) {
        ...
        var listaDataXHoja = getListaFeriados(numHoja);

        var len = listaDataXHoja.length;
        for (var i = 0; i < len; i++) {
            cadena += `
            <tr>
                <td>${nombreHoja}</td>
                <td style="">${listaDataXHoja[i].PmfrdofechaDesc}</td>
                <td style='text-align: left;'>${listaDataXHoja[i].Pmfrdodescripcion}</td>
            </tr>
            `;
        }
    }
...
function getListaFeriados(numHoja) {
    var listaErroresApp = [];
    var evtHot = getVariableEvt(numHoja);
    var listaFeriado = evtHot.ListaFeriado;
...
//
function listarPtomedinombFromHojaptomed(listaPtos) {
    var listaFinal = [];
    for (var i = 0; i < listaPtos.length; i++) {
        var find = buscarPtomedinombFromHojaptomed(listaFinal, listaPtos[i].PtoMediEleNomb);
        if (find == -1) {
            var pto = {
                Ptomedinomb: listaPtos[i].PtoMediEleNomb,
            }
            listaFinal.push(pto);
        }
    }
...
function buscarPtomedinombFromHojaptomed(lista, ptomedinomb) {
    for (var i = 0; i < lista.length; i++) {
        if (lista[i].Ptomedinomb == ptomedinomb) {
            return i;
        }
    }
    return -1;
...
//
function buscarPtoyHptoFromHojaptomed(lista, ptomedinomb, tpotmedicodi) {
    for (var i = 0; i < lista.length; i++) {
        if (lista[i].PtoMediEleNomb == ptomedinomb && lista[i].Tptomedicodi == tpotmedicodi) {
            return i;
        }
    }
    return -1;
...
//
function listarTptoFromHojaptomed(listaPtos, ptomedinomb) {
    var listaFinal = [];
    for (var i = 0; i < listaPtos.length; i++) {
        if (listaPtos[i].PtoMediEleNomb == ptomedinomb) {
            var find = buscarTptoFromLista(listaFinal, listaPtos[i].Tptomedicodi);
            if (find == -1) {
                var tpto = {
                    Tptomedicodi: listaPtos[i].Tptomedicodi,
                    Tipoptomedinomb: listaPtos[i].Tipoptomedinomb,
                }
                listaFinal.push(tpto);
            }
        }
    }
...
function buscarTptoFromLista(lista, tpotmedicodi) {
    for (var i = 0; i < lista.length; i++) {
        if (lista[i].Tptomedicodi == tpotmedicodi) {
            return i;
        }
    }
    return -1;
...
function getUnidadEnvio(numHoja) {
    var valor = $(getIdElemento(numHoja, "#cbUnidadEnvio")).val();
    $(getIdElemento(numHoja, "#hfUnidadEnvio")).val(valor);
    valor = $(getIdElemento(numHoja, "#hfUnidadEnvio")).val();
...
function setVerUltimoEnvioGlobal(valor) {
    //LISTA_OBJETO_HOJA.forEach(function (hoja, index) {
    //    hoja.verUltimoEnvio = valor;
    //});

    for (var hoja in LISTA_OBJETO_HOJA) {
        LISTA_OBJETO_HOJA[hoja].verUltimoEnvio = valor;
    }
}
function getVerUltimoEnvio(hoja) {
    var configHoja = getConfigHoja(hoja);

    var opcionActual = NO_VER_ULTIMO_ENVIO;
    if (LISTA_OBJETO_HOJA[getNumeroHoja(hoja)].verUltimoEnvio)
        opcionActual = VER_ULTIMO_ENVIO;
...
function getEsHojaPadre(hoja) {
    var hoja = LISTA_OBJETO_HOJA[getNumeroHoja(hoja)];

    if (hoja !== undefined && hoja != null)
        return hoja.esHojaPadre;
    return false;
}
...
function mostrarMensajeInformativo(numHoja, mensaje) {
    ...
    //mensajes pendientes
    var numMsj = getVariableEvt(numHoja).NumMsjPendiente;
    $(getIdElemento(numHoja, '#btnVerMensajesPMPO')).attr('data-content', numMsj);
}
...
function obtenerMensajeEnvio(numHoja, idEnvio) {
    var tipoIndex = $("#hdTipoIndex").val();
    var bMostrarEnPlazo = (tipoIndex == TIPO_INDEX_REPORTE);
    ...
    if (envio > 0) {
        ...
        if (bMostrarEnPlazo)
            mensaje += ", <strong>Enviado </strong>" + plazo;
        return mensaje;
    }
    else {
