...
$(document).ready(function () {
    ...
    $('#idCentral').change(function () {
        const unidad = $('#idUnidad').val();
        val = (unidad) ? unidad : -1;
        if(val != -1)
        listaVariablesCentrales();
    });

    $('#idUnidad').change(function () {
        const asociacion = $('#idCentral').val();
        val = (asociacion) ? asociacion : -1;
        console.log(asociacion, 'asociacionN');
        console.log(val, 'valN');
        if (val != -1)
        listaVariablesCentrales();
    });

    $('#btnAgruparBarras').on('click', function () {
        objPopup = $('#popup-barras').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false,
    ...
    $('#btnPopNuevo').on('click', function () {
        ...
        if (seleccionados.length == 0) {
            SetMessage('#pop-alert',
                'Debe seleccionar al menos una barra ',
                'warning', true);
            return false;
        }
...
function listaVariablesCentrales() {
    const unidad = $('#idUnidad').val();
    const asociacion = $('#idCentral').val();

    $.ajax({
        type: 'POST',
        url: controller + 'ListaVariablesCentrales',
        ...
        success: function (result) {
            RefillDropDownList($('#idVariable'), result, 'Prnvarcodi', 'Prnvarnom');
        },
        error: function () {
...
function Exportar(datos, headers) {
    ...
    if (val == true) {
        SetMessage('#message', 'Debe seleccionar una agrupacion o barra...', 'error', true);
    } else {  
        $.ajax({
        type: 'POST',
...
function iniciarTablaSeleccionados() {
    dtSeleccionados = $('#dtSeleccionados').DataTable({
        ...
        columns: [
            ...
            { title: 'Barras Seleccionadas', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaDisponibles(modelo) {
    dtDisponibles = $('#dtDisponibles').DataTable({
        ...
        columns: [
            ...
            { title: 'Barras Disponibles', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
