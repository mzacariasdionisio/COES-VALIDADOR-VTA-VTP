...
$(document).ready(function () {
    ...
    $('#btnAsociarGeneradores').on('click', function () {        
        objPopup = $('#popup-generadores').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false,
...
function Exportar(datos, headers) {
    ...
    if (val == true) {
        SetMessage('#message', 'Debe seleccionar una central o unidad...', 'error', true);
    } else { 
        $.ajax({
        type: 'POST',
...
function registrarAsociacion(datos) {
    $.ajax({
        ...
        success: function (result) {
            ...
            SetMessage('#message',
                'Las centrales se registraron exitosamente!',
                'success', true);
        },
        error: function () {
...
function obtenerMediciones() {
    ...
    console.log($('#main').data('data-mdl'), '_modulo');
...
function iniciarTablaGrupos(modelo) {
    dtGrupos = $('#dtGrupos').DataTable({        
        ...
        columns: [
            ...
            { title: 'Centrales', data: 'Asocianom' }
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaSeleccionados() {
    dtSeleccionados = $('#dtSeleccionados').DataTable({
        ...
        columns: [
            ...
            { title: 'Unidades Seleccionadas', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaDisponibles(modelo) {
    dtDisponibles = $('#dtDisponibles').DataTable({
        ...
        columns: [
            ...
            { title: 'Unidades Disponibles', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
