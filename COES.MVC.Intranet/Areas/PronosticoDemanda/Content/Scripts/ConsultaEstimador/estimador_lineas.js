...
$(document).ready(function () {
    ...
    $('#btnAgruparLineas').on('click', function () {
        objPopup = $('#popup-lineas').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false,
...
function Exportar(datos, headers) {
    ...
    if (val == true) {
        SetMessage('#message', 'Debe seleccionar un enlace o linea...', 'error', true);
    } else { 
        $.ajax({
        type: 'POST',
...
function iniciarTablaSeleccionados() {
    dtSeleccionados = $('#dtSeleccionados').DataTable({
        ...
        columns: [
            ...
            { title: 'Líneas Seleccionadas', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaDisponibles(modelo) {
    dtDisponibles = $('#dtDisponibles').DataTable({
        ...
        columns: [
            ...
            { title: 'Líneas Disponibles', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
