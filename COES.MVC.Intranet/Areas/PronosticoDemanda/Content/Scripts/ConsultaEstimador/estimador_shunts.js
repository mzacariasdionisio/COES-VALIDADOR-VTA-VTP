...
$(document).ready(function () {
    ...
    $('#btnAgruparShunts').on('click', function () {
        objPopup = $('#popup-shunts').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false,
...
function Exportar(datos, headers) {
    ...
    if (val == true) {
        SetMessage('#message', 'Debe seleccionar una agrupacion o shunt...', 'error', true);
    } else { 
        $.ajax({
        type: 'POST',
...
function iniciarTablaSeleccionados() {
    dtSeleccionados = $('#dtSeleccionados').DataTable({
        ...
        columns: [
            ...
            { title: 'Shunts Seleccionados', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaDisponibles(modelo) {
    dtDisponibles = $('#dtDisponibles').DataTable({
        ...
        columns: [
            ...
            { title: 'Shunts Disponibles', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
