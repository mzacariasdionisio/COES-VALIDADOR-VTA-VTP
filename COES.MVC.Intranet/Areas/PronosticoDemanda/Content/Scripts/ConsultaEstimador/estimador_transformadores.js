...
$(document).ready(function () {
    ...
    $('#btnAgruparTransformadores').on('click', function () {
        objPopup = $('#popup-transformadores').bPopup({
            easing: 'easeOutBack',
            speed: 350,
            transition: 'fadeIn',
            modalClose: false,
    ...
    $('#btnPopNuevo').on('click', function () {
        ...
        if (seleccionados.length == 0) {
            SetMessage('#pop-alert',
                'Debe seleccionar al menos una transformador ',
                'warning', true);
            return false;
        }
...
function Exportar(datos, headers) {
    ...
    if (val == true) {
        SetMessage('#message', 'Debe seleccionar un enlace o transformador...', 'error', true);
    } else { 
        $.ajax({
        type: 'POST',
...
function registrarAsociacion(datos) {
    $.ajax({
        ...
        success: function (result) {
            ...
            SetMessage('#message',
                'Los enlaces se registraron exitosamente!',
                'success', true);
        },
        error: function () {
...
function iniciarTablaSeleccionados() {
    dtSeleccionados = $('#dtSeleccionados').DataTable({
        ...
        columns: [
            ...
            { title: 'Transformadores Seleccionados', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
...
function iniciarTablaDisponibles(modelo) {
    dtDisponibles = $('#dtDisponibles').DataTable({
        ...
        columns: [
            ...
            { title: 'Transformadores Disponibles', data: 'Ptomedidesc' },
        ],
        columnDefs: [
            {
                targets: 0,
