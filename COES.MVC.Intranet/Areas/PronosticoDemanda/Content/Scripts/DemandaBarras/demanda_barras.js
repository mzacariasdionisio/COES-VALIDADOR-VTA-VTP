...
$(document).ready(function () {
    ...
    $('#btn-guardar').on('click', function () {
        ...
        var dataBarra = FormatMedicion(ht.getDataAtProp('barra'));
        //Función para el registro
        Save(dataAjuste, dataBarra);
    });
...
function LoadData() {
    ...
    $.ajax({
        type: 'POST',
        url: controller + 'DemandaBarrasDatos',
...
function Save(dataAjuste, dataBarra) {
    $.ajax({
        type: 'POST',
        url: controller + 'DemandaBarrasSave',
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            idArea: $('#id-area').val(),
            regFecha: $('#id-fecha').val(),
    ...
    ht = new Handsontable(container, {
        ...
        afterChange: function (changes, source) {
            if (ValidarEventosHandsontable(source)) {
                htBaseIndex = ht.propToCol('barra');//Setea la columna base (Handsontable)
...
function actualizarMedicion(medicion) {
    ...
    $.ajax({
        type: 'POST',
        url: controller + 'ActualizarMedicionDemandaBarra',
        contentType: 'application/json; charset=utf-8',
        data: JSON.stringify({
            idArea: $('#id-area').val(),
            idVersion: $('#id-version').val(),
