...
$(document).ready(function () {
    ...
    dt = $('#dt2').DataTable({
        ...
        columns: [
            { title: 'Pronóstico', data: 'Ptogrppronostico' },
            { title: 'Agrupación', data: 'Ptomedicodi' },
            { title: 'Energía (MW)', data: 'Meditotal' },
            { title: 'Depurado', data: null },
            { title: 'Área operativa', data: 'Areanomb' },
            { title: 'Empresa', data: 'Emprnomb' },
            { title: 'Puntos de medición', data: null },
            { title: '', data: null }
        ],
        columnDefs: [
            {
                //Contenido de la columna "Pronóstico" segun criterio
                targets: [0],
                createdCell: function (td, cellData, rowData, row, col) {
                    if (rowData.Ptogrppronostico == 1) {
                        $(td).html('Si');
            ...
            {
                //Contenido de la Estado
                targets: [3],
                createdCell: function (td, cellData, rowData, row, col) {
                    if (rowData.Prnm48tipo != 1) {
                        $(td).html('Manual');
                        $(td).css('font-weight', 'bold');
                    }
                    else {
            ...
            },
            {
                ...
                createdCell: function (td, cellData, rowData, row, col) {
                    tb = $('<table></table>');
                    $.each(rowData.ListaDetalle, function (i , item) {
                        r = $('<tr></tr>');
                        r.html(item.name + ' [ ' + item.sum + ' MW ]');

                        switch (item.state) {
                            case 0: r.css('background', '#FF9595'); break; //No reportado
                            default: r.css('background', '#95D7FF'); break; //Reportado 
                        }
            ...
            },
            {
                ...
                createdCell: function (td, cellData, rowData, row, col) {
                    ...
                    switch (col) {
                        case 1:
                            //Punto de medición
                            str = rowData.Ptomedidesc + ' (' + rowData.Ptomedicodi + ')';
                            $(td).html(str);
                            break;
                    }
                }
            },
            {
                //Botones
                targets: -1,
                defaultContent: '<input type="button" value="Ver"/>'
            }
        ],
...
$(document).on('click', '#dt2 tr td input[type="button"]', function () {
    ...
    var title = r.Ptomedidesc + ' (' + r.Ptomedicodi + ')';
    ...
    //Carga los datos
    LoadData(s_id, s_type_dem, s_date, title);
});


//Carga los datos del modulo
function LoadData(s_id, s_type_dem, s_date, title) {
    $.ajax({
        type: 'POST',
        ...
        success: function (result) {
            if (Reload(result.valid)) {
                ...
                SetMessage('#message',
                    'No existe información reportada para la agrupación seleccionada...',
                    'warning', true);
                return false;
            }
        },
...
function GetFiltersValues() {
    ...
    model['SelPronostico'] = $('#id-pronostico').val();
    return model;
}
...
function QuickLoadData(name) {
    ...
    var s_date = $('#id-fecha').val();
    var s_type_dem = $('#id-tpdemanda').val();
    LoadData(tempData.id, s_type_dem, s_date, tempData.title);
}
