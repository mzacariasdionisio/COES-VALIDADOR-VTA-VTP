...
namespace COES.MVC.Intranet.Areas.PronosticoDemanda.Helper
{
    /// <summary>
    /// Metodo de ayuda para el comparativo RPF y Ejecutado
    /// </summary>
    public class DespachoEjecutadoHelper
    {
        /// <summary>
        ...
        /// <returns></returns>
        public static List<ComparativoItemModel> ObtenerComparativo(DateTime fecha, List<ServicioCloud.Medicion> listRPF, MeMedicion48DTO despacho, PrnMedicioneqDTO datosPronostico)
        {
            ...
            if (listRPF.Count == 48 && despacho != null)
            {
                for (int i = 1; i <= 48; i++)
                {
                    ...
                    entity.Pronostico = Convert.ToDecimal(datosPronostico.GetType().GetProperty(Constantes.CaracterH + i).GetValue(datosPronostico, null));
                    entitys.Add(entity);
                }
            }
