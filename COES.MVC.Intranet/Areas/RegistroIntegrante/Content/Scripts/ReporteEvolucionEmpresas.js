...
pintarPaginado = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "paginadoEvolucionEmpresa",
        data: {
            finicios: finicio,
            ffins: ffin,
...
mostrarListado = function (nroPagina) {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "listadoEvolucionEmpresas",
        data: {
            finicios: finicio,
            ffins: ffin,
            tipoempresa: $('#cbTipoEmpresa').val(),
...
ExportarExcelEvolucionEmpresa = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoReporteEvolucionEmpresa",
        data: {
            finicios: finicio,
            ffins: ffin,
            tipoempresa: $('#cbTipoEmpresa').val(),
        ...
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarExcelEvolucionEmpresa";
            }
            if (result == -1) {
