...
ExportarWordRepresentanteLegal = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoWordReporteRepresentantesLegales",
        data: {
            tipoempresa: $('#cbTipoEmpresa').val(),
            nombre: $('#txtNombreFiltro').val(),
            tiporepresentante: $('#cbTipoRepresentante').val(),
        ...
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarWordRepresentanteLegal";
            }
            if (result == -1) {
                alert("Error al imprimir.");
            }
...
ExportarTextoRepresentanteLegal = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoTextoReporteRepresentantesLegales",
        data: {
            tipoempresa: $('#cbTipoEmpresa').val(),
            nombre: $('#txtNombreFiltro').val(),
            tiporepresentante: $('#cbTipoRepresentante').val(),
        ...
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarTextoRepresentanteLegal";
            }
            if (result == -1) {
                alert("Error al imprimir.");
            }
