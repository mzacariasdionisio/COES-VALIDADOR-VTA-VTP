...
pintarPaginado = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "PaginadoTiemposProceso",
        data: {
            finicios: finicio,
            ffins: ffin,
...
mostrarListado = function (nroPagina) {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "ListadoTiemposProceso",
        data: {
            finicios: finicio,
            ffins: ffin,
            tipoempresa: $('#cbTipoEmpresa').val(),
...
ExportarExcelEvolucionEmpresa = function () {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoReporteTiemposProceso",
        data: {
            finicios: finicio,
            ffins: ffin,
            tipoempresa: $('#cbTipoEmpresa').val(),
        ...
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarExcelTiemposProceso";
            }
            if (result == -1) {
