...
cargarRepresentantes = function () {    
    $.ajax({
        ...
        success: function (evt) {
            ...
            $('#tablaRepresentanteLegal').dataTable({
                "scrollY": 430,
                "scrollX": true,
                "sDom": 't',
                "ordering": false,
...
EditarRepresentanteLegal = function (idRepresentante) {
    ...
    $.ajax({
        ...
        success: function (evt) {
            ...
            $('#txtFechaVigenciaPoder').Zebra_DatePicker({});

            $('#tiporepresentante').val($('#hfTipoRepresentante').val());
...
DarBajaRepresentanteLegal = function (idRepresentante) {
    ...
    if (confirm('¿Está seguro de realizar esta operación?')) {
        $.ajax({
            ...
            dataType:'json',
            url: controlador + 'darbajarepresentante',
            success: function (result) {
                if (result == 1) {
                    $('#mensajeList').removeClass();
                    $('#mensajeList').html("La operación se realizó correctamente.");
                    $('#mensajeList').addClass('action-exito');
                    cargarRepresentantes();
                }
                else {
                    $('#mensajeList').removeClass();
                    $('#mensajeList').html("Se ha producido un error. Intente nuevamente.");
                    $('#mensajeList').addClass('action-error');
                }
            }
        });
    }
...
grabar = function (idRepresentante) {
    ...
    if (validacion == "") {
        ...
        $.ajax({
            ...
            data: {
                idRepresentante: idRepresentante,
                tipoRepresentante: $('#tiporepresentante').val(),
                dni: $('#txtDNIRL').val(),
                nombre: $('#txtNombresRL').val(),
                apellido: $('#txtApellidosRL').val(),                
                cargo: $('#txtCargoEmpresaRL').val(),
                telefono: $('#txtTelefonoRL').val(),
                telefonoMovil: $('#txtMovilRL').val(),
                fechaVigenciaPoder: $('#txtFechaVigenciaPoder').val(),
                correo: $('#txtCorreoRL').val()
            },
            dataType: 'json',
            success: function (result) {
                if (result == 1) {
