...
namespace COES.MVC.Intranet.Areas.ServicioRPF.Helper
{
    ...
    public class ExcelDocument
    {
        ...
        public static void GenerarReporteRangoHoras(List<ServicioRpfDTO> listConfiguration, List<RegistroRPF> listDatos)
        {
            try
            {
                string ruta = ConfigurationManager.AppSettings[RutaDirectorio.ReporteServicioRPF].ToString();
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(newFile))
                {
                    ...
                    if (ws != null)
                    {
                        ...
                        foreach (ServicioRpfDTO item in listConfiguration)
                        {
                            ...
                            List<RegistroRPF> datos = listDatos.Where(x => x.PTOMEDICODI == item.PTOMEDICODI).
                                OrderBy(y => y.FECHAHORA).ToList();

                            index = index + 4;
                            int contador = 0;
                            foreach (RegistroRPF dato in datos)
                            {
