...
namespace COES.MVC.Intranet.Areas.ServicioRPFNuevo.Controllers
{
    public class AnalisisFallaController : Controller
    {
        ...
        public List<RegistrorpfDTO> ListaDatos
        {
            get
            {
                return (Session[DatosSesion.ListaDatosFalla] != null) ?
                    (List<RegistrorpfDTO>)Session[DatosSesion.ListaDatosFalla] : new List<RegistrorpfDTO>();
            }
        ...
        public PartialViewResult Validacion(string fecha, string hora, string unidades, decimal reserva)
        {
            ...
            List<RegistrorpfDTO> list = this.logic.ObtenerDatosFallas(fechaProceso);
            ...
            if (list.Count > 0)
            {
                ...
                {
                    List<RegistrorpfDTO> datos = list.Where(x => x.PTOMEDICODI == punto).ToList();
                    if (datos.Count > 0)
                    {
        ...
        public JsonResult Grafico(int ptoMediCodi)
        {
            ...
            try
            {
                ...
                {
                    List<RegistrorpfDTO> list = this.ListaDatos.Where(x => x.PTOMEDICODI == ptoMediCodi).ToList();
                    if (list != null)
                    {
                        if (list.Count >= 60)
                        {
        ...
        public PartialViewResult Reporte()
        {
            ...
            List<RegistrorpfDTO> listDatos = this.ListaDatos;
