...
namespace COES.MVC.Intranet.Areas.ServicioRPFNuevo.Helper
{
    ...
    public class ComparativoHelper
    {
        ...
        /// <returns></returns>
        public static List<ComparativoItemModelrpf> ObtenerComparativo(DateTime fecha, List<MeMedicion60DTO> listRPF, MeMedicion48DTO despacho)
        {
        ...
        public static void ExportarComparativo(List<ComparativoItemModelrpf> list, string path, string filename, string fecha)
        {
            try
            {
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(newFile))
                {
                    ...
                    if (ws != null)
                    {
                        ...
                        index = 6;
                        foreach (ComparativoItemModelrpf item in list)
                        {
        ...
        public static void ExportarDatosRPF(List<MeMedicion60DTO> list, List<MePtorelacionDTO> relacion, string fecha, string path, string filename)
        {
            try
            {
                string file = path + filename;
                ...
                using (ExcelPackage xlPackage = new ExcelPackage(newFile))
                {
                    ...
                    if (ws != null)
                    {
                        ...
                        for (int i = 1; i <= 48; i++)
                        {
                            ws.Cells[index, 2].Value = fechaConsulta.AddMinutes(i * 15).ToString(Constantes.FormatoOnlyHora);
                        ...
                        foreach (var central in centrales)
                        {
                            ...
                            List<MeMedicion60DTO> datos = list.Where(x => puntos.Any(y => x.Ptomedicodi == y)).ToList();
