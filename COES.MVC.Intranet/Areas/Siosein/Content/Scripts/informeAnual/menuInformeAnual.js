var controlador = siteRoot + 'Siosein/InformesAnual/';
...
$(function () {
    ...
    //    if (window.confirm("El proceso de exportación a Excel del Informe Anual puede tardar algunos minutos. ¿Desea continuar?")) {
    //        exportarExcelTodo();
    //    }
    //});
    $('#btnPopupNuevaVersion').click(function () {
        $("#AnhoIniNuevo").val($("#dtpAnio").val());
        $('#txtMotivo').val('');
...
function versionGuardar() {
    ...
    //datos
    var fechaPeriodo = $('#AnhoIniNuevo').val();
...
function versionListado() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.Resultado != "-1") {
                console.log("Informe anual " + fechaPeriodo);
...
function verNumeral(verscodi) {
    window.location = controlador + 'IndexAnualProdEnergia?codigoVersion=' + verscodi;    
}
    ...
    $.ajax({
        ...
        success: function (model) {
            if (model.Resultado != "") {
                ...
                var rutaInicial = rutaCompleta.split("Siosein"); //Obtener url de intranet
