var controlador = siteRoot + 'StockCombustibles/Ampliacion/';
var listFormatCodi = [];
...
function exportarExcel() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + 'ampliacion/GenerarReporteCumplimiento',
        data: {
            sEmpresas: $('#hfEmpresa').val(),
            idFormato: formato,
            fIni: $('#FechaDesde').val(),
        ...
        success: function (result) {
            if (result == 1)
                window.location = controlador + "ampliacion/ExportarReporte";
            if (result == -1)
