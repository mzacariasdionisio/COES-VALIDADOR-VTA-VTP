...
function mostrarListado() {
    ...
    var yacimiento = $('#cbYacimiento').multipleSelect('getSelects');
    if (yacimiento == "[object Object]") yacimiento = -1;
    if (yacimiento == "") yacimiento = "-1";


    $('#hfTipoAgente').val(tipoAgente);
    /* $('#hfCentralInt').val(centralInt);*/
    $('#hfYacimiento').val(yacimiento);
    $('#hfAgente').val(agente);
    ...
    $.ajax({
        ...
        data: {
            ...
            idsYacimientos: $('#hfYacimiento').val(),
...
function exportarExcelReporte() {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + 'GenerarReporteXLSDisponibilidad',
        data: {
            idsTipoAgente: $('#hfTipoAgente').val(),
            idsCentralInt: $('#hfCentralInt').val(),
            idsYacimientos: $('#hfYacimiento').val(),
        ...
        success: function (result) {
            if (result == 1) {//
                window.location = controlador + "ExportarReporte?tipo=7";
            }
            if (result == -1) {
