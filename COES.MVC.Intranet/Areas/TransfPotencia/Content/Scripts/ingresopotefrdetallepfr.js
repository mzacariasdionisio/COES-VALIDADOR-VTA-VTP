var controler = siteRoot + "transfpotencia/ingresopotefrdetallepfr/";
var hot;
var data;
var error = [];
var listaObjEmpresas;
var listaObjCentrales;
var listaObjUnidades;
$(function () {
    mostrarGrillaExcel();
...
mostrarGrillaExcel = function () {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            var listaUnidadGeneracion = result.ListaUnidadesGeneracion;

            listaObjEmpresas = result.EmpresaList;
            listaObjCentrales = result.CentralList;
            listaObjUnidades = result.UnidadList;
            ...
            hot = new Handsontable(container, {
                ...
                beforeChange: function (changes, source) {
                    for (var i = changes.length - 1; i >= 0; i--) {
                        ...
                        if (numColumna == '0') {
                            ...
                            lista = listaEmpresas;
                            //var empresa = listaObjEmpresas.find((m) => m.Emprnomb === sNuevoValor);
                            //var centralesLista = listaObjCentrales.filter(number => number.Emprcodi === empresa.Emprcodi);

                            //listaCentralGeneracion = [];
                            //for (var k = 0; k < centralesLista.length; k++) {
                            //    var objcentralNomb = centralesLista[k].Central;
                            //    listaCentralGeneracion.push(objcentralNomb);
                            //}

                            //var cellProperties = hot.getData()[numFila];
                            //cellProperties['dropdown'].data = listaCentralGeneracion;
                            ...
                            //Valida si el nuevo valor de la celda se encuentra en listaCentralGeneracion 
                            lista = listaUnidadGeneracion;
                            bEsNumero = null;
                            header = "Unidad Generación";
                            sMensaje = "Nombre de la Unidad de Generación es incorrecto";
                        }
                ...
                cells: function (row, col, prop) {
                    ...
                    //if (col == 0) {
                    //    cellProperties.renderer = comboRendererCentral;
                    //    cellProperties.source = listaCentralGeneracion;
                    //}
...
//comboRendererCentral = function (instance, td, row, col, prop, value, cellProperties) {
//    Handsontable.renderers.TextRenderer.apply(this, arguments);

//    listaCentralGeneracion = listaObjCentralesTemp;
//}
                ...
                beforeChange: function (changes, source) {
                    ...
                    for (var i = changes.length - 1; i >= 0; i--) {
                        ...
                        if (numColumna == '0') {
                            ...
                            //Valida si el nuevo valor de la celda se encuentra en listaUnidadGeneracion
                            lista = listaUnidadGeneracion;
                            bEsNumero = null;
                            header = "Unidad de Generación";
                            sMensaje = "Nombre de la Unidad de Generación es incorrecto";
                        }
                        else if (numColumna == '3') {
                            //Valida si el nuevo valor de la celda se encuentra en Potencia Efectiva
...
firstRowRendererCeleste = function (instance, td, row, col, prop, value, cellProperties) {
    ...
    if (cellProperties.valid === true && arguments[5] != null) {
        switch (col) {
            case 0: //EMPRESA
                var empresa = listaObjEmpresas.find((m) => m.Emprnomb === arguments[5]);
                var centralesLista = listaObjCentrales.filter(number => number.Emprcodi === empresa.Emprcodi);

                var listaCentralFiltro = [];
                for (var k = 0; k < centralesLista.length; k++) {
                    var objcentralNomb = centralesLista[k].Central;
                    listaCentralFiltro.push(objcentralNomb);
                }
                instance.getCellMeta(row, col + 1).source = listaCentralFiltro;
                //hot.getData()[row][col + 1] = "";

                break;
            case 1: //CENTRAL

                var central = listaObjCentrales.find((m) => m.Central === arguments[5]);
                var unidadesLista = listaObjUnidades.filter(number => number.Equipadre === central.Equipadre);

                var listaUnnidadFiltro = [];
                for (var k = 0; k < unidadesLista.length; k++) {
                    var objunidadNomb = unidadesLista[k].Pfrtotunidadnomb;
                    listaUnnidadFiltro.push(objunidadNomb);
                }
                instance.getCellMeta(row, col + 1).source = listaUnnidadFiltro;
                break;
        }


        //instance.getCellMeta(row, col + 1).source = listaObjEmpresasTemp;
    }
}
