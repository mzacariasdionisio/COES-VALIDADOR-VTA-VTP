var controler = siteRoot + "transfpotencia/mejorinformacion/";
        ...
        success: function (result) {
            ...
            hot = new Handsontable(container, {
                ...
                cells: function (row, col, prop) {
                    ...
                    else if ((col == 0 || col == 1 || col == 2 || col == 3 || col == 13) && row > 0) {
                        cellProperties.renderer = firstRowRendererCeleste;
                    }
                    else if ((col == 10 || col == 11 || col == 12) && row > 0) {
...
procesarArchivo = function (sFile) {
    ...
    $.ajax({
        ...
        success: function (result) {
                ...
                beforeChange: function (changes, source) {
                    ...
                    for (var i = changes.length - 1; i >= 0; i--) {
                        ...
                        if (numColumna == '2') {
                            ...
                            if (sNuevoValor != null) sNuevoValor = sNuevoValor.toString();
                            bEsNumero = sRegExVal.test(sNuevoValor);
                            header = "Potencia Coincidente";
                            sMensaje = "Valor de la Potencia Coincidente no es válido";
                            if (bEsNumero) {   //Validamos la condición: la potencia egreso no sean superiores a 400,000.
                                if (potenciaMaxima < sNuevoValor) {
                                    //console.log(sNuevoValor);
                                    //console.log(potenciaMaxima);
                                    bEsNumero = false;
                                    sMensaje = "El valor " + sNuevoValor + " de la Potencia Coincidente no debe ser superior a 400,000";
                                }
                            }
                            if (sValorAnterior !== null) {
                                //Se fuerza que se elimine el valor anterior por si fue ingresado potenciaMaxima < sNuevoValor
                            ...
                            if (sNuevoValor != null) sNuevoValor = sNuevoValor.toString();
                            bEsNumero = sRegExVal.test(sNuevoValor);
                            header = "Factor Pérdida";
                            sMensaje = "Valor del Factor Pérdida no válido";
                        }
                        else if (numColumna == '11') {
                            //Valida si el nuevo valor de la celda se encuentra en Calidad
                            lista = listaCalidad;
            ...
            $('#divAcciones').css('display', 'block');
            if (iRegError > 0) {
                mostrarAlerta("Lo sentimos, <b>" + iRegError + "</b> registro(s) no ha(n) sido leido(s) por presentar <b>errores</b>" + sMensajeError);
            }
            else {
                var iNumRegistros = hot.countRows();
                if (iNumRegistros >= 2) iNumRegistros = iNumRegistros - 3;
        ...
        error: function () {
            mostrarAlerta('Lo sentimos no se puede mostrar la hoja de cálculo')
        }
    });
}

firstRowRendererHeader = function (instance, td, row, col, prop, value, cellProperties) {
    Handsontable.renderers.TextRenderer.apply(this, arguments);

    if (col == 6 || col == 7) {
        td.style.color = 'white';
        td.style.background = '#8BAF5D';
        ...
        success: function (result) {
            ...
            hotConsulta = new Handsontable(container, {
                ...
                cells: function (row, col, prop) {
                    ...
                    var cellProperties = {};
                    if (row == 0) {
                        cellProperties.renderer = firstRowRendererHeader;
                    }
                    else {
                        if (col == 0 || col == 1 || col == 2 || col == 3 || col == 10 || col == 11) {
                            cellProperties.renderer = firstRowRendererCeleste;
                        }
                    }
                    if (row == result.Data.length - 1) {
...
mostrarGrillaConsulta = function () {
    var container = document.getElementById('grillaExcelConsulta');
    var pericodi = $('#pericodiConsulta').val();
    var recpotcodi = $('#recpotcodiConsulta').val();
    var pegrmitipousuario = $('#pegrmitipousuario').val();
...
mostrarGrillaConsultaFormatoNuevo = function () {
    var container = document.getElementById('grillaExcelConsulta');
    var pericodi = $('#pericodiConsulta').val();
    var recpotcodi = $('#recpotcodiConsulta').val();
    var pegrmitipousuario = $('#pegrmitipousuario').val();
            ...
            hotConsulta = new Handsontable(container, {
                ...
                cells: function (row, col, prop) {
                    ...
                    if (row == 0) {
                     ...
                     //   if (col == 0 || col == 1 || col == 2 || col == 3 || col == 4 || col == 5 || col == 10 || col == 11) {
                       if (col == 0 || col == 1 || col == 2 || col == 3  || col == 10 || col == 11) {
                            cellProperties.renderer = firstRowRendererCeleste;
                        }
                    }
