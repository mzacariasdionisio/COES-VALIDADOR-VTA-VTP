...
using PtoMedida = COES.MVC.Intranet.Areas.ValorizacionDiaria.Models.PtoMedida;

namespace COES.MVC.Intranet.Areas.ValorizacionDiaria.Helper
{
    public class ValorizacionDiariaHelper
    {

        /// <summary>
        /// Obtiene Lista de entidades de ValorizacionDiaria
        /// </summary>
        /// <returns></returns>
        public static List<PtoMedida> ObtenerListaMedida()
        {
            ...
            lista.Add(elemento);
            elemento = new PtoMedida() { IdMedida = 4, NombreMedida = "Estación Valorización Diaria" };
            lista.Add(elemento);
        ...
        public static int EnviarCorreo(string lectura, string formato, bool enPlazo, string empresa, DateTime fechaInicio, DateTime fechaFin, string areaCoes, string usuario, DateTime fechaEnvio, int idEnvio)
        {
            ...
            //string toEmail = ConfigurationManager.AppSettings[ConstantesValorizacionDiaria.ToMail];
            //string ccEmail = string.Empty;
            //if (usuario.IndexOf("@") != -1)
            //    ccEmail = usuario;
            //appMail.EnviaCorreoFull(ConstantesValorizacionDiaria.FromEmail, ConstantesValorizacionDiaria.FromName, toEmail,
            //    ccEmail, "Notificacion de Envío de Información Valorización Diaria", msg, true, "", null);
            return resultado;
        }
        /// <summary>
        ...
        public static void GenerarFormato(ValorizacionDiariaModel model, int idEmpresa, string empresa, DateTime fecha, string semana)
        {
            string ruta = ConfigurationManager.AppSettings[RutaDirectorio.ReporteValorizacionDiaria].ToString();
            string fileTemplate = NombreArchivoValorizacionDiaria.PlantillaFormatoProgDiario;
            ...
            using (ExcelPackage xlPackage = new ExcelPackage(newFile, template))
            {
                ...
                /////////////////////// Imprimir los titulos agrupadados por pto de Medicion //////////////////////////////////
                //row = ParamFormato.RowDatos - 2;
                //column = ParamFormato.ColDatos;
                //foreach (var reg in model.ListaHeadColumn)
                //{
                //    if (column != ParamFormato.ColDatos)
                //    {
                //        ws.Cells[row, column].Value = "";
                //        ws.Cells[row, column].StyleID = ws.Cells[row, ParamFormato.ColDatos].StyleID;
                //    }
                //    ws.Cells[row, column].Value = reg.Headnombre;
                //    for (int i = 1; i < reg.Headlen; i++)
                //    {
                //        ws.Cells[row, column + i].Value = "";
                //        ws.Cells[row, column + i].StyleID = ws.Cells[row, column].StyleID;
                //    }
                //    ws.Cells[row, column, row, column + (int)reg.Headlen - 1].Merge = true;
                //    column = column + (int)reg.Headlen;
                //}

                /////////////////////// Imprimir Fechas y Cabeceras de los nombres de los ptos de medicion/////////////////////
        ...
        public static void GenerarFileExcel(FormatoModel model, string ruta)
        {
            ...
            //FileInfo template = new FileInfo(ruta + fileTemplate);
            //FileInfo newFile = new FileInfo(ruta + NombreArchivo.FormatoProgDiario);
            string rutaLogo = ruta + ConstantesValorizacionDiarias.NombreArchivoLogo;
            FileInfo newFile = new FileInfo(ruta + model.Formato.Formatnombre + ".xlsx");
            if (newFile.Exists)
            {
                newFile.Delete();
        ...
        public static string ObtenerCeldaFecha(int periodo, int resolucion, int tipoLectura, int horizonte, int indice, DateTime fechaInicio)
        {
            ...
            switch (periodo)
            {
                case ParametrosFormato.PeriodoMensual:
                    ...
                    {
                        case ParametrosFormato.ResolucionMes:
                            if (tipoLectura == ParametrosLecturaValorizacionDiaria.Ejecutado)
            ...
            for (var i = 0; i < nCol; i++)
            {
                for (var j = 0; j < nFil; j++)
                {
                    ...
                    if ((j % 96) == 0)
                    {
                        ...
                        reg.Ptomedicodi = ptos[i].Ptomedicodi;
                        reg.Lectcodi = idLectura;
                        reg.Meditotal = 0;
                        reg.Tipoinfocodi = (int)ptos[i].Tipoinfocodi;
        ...
        public static void LoadMatrizExcel2(string[][] data, List<MeMedicionxintervaloDTO> medicion, int col)
        {
            ...
            foreach (var reg in medicion)
            {
                ...
                data[i][5] = string.IsNullOrEmpty(reg.Medintdescrip) ? "" : reg.Medintdescrip;
                i++;
            }
        ...
        public static void GetFechaActualEnvio(int periodo, int tipo, ref string mes, ref string fecha, ref int semana, ref int anho)
        {
            ...
            {
                case ParametrosFormato.PeriodoDiario:
                    if (tipo == ParametrosLecturaValorizacionDiaria.Ejecutado)
                    {
                        fecha = fechaActual.AddDays(-1).ToString(Constantes.FormatoFecha);
                    }
                    if (tipo == ParametrosLecturaValorizacionDiaria.Programado)
                    {
                        fecha = fechaActual.ToString(Constantes.FormatoFecha);
                    }
                    if (tipo == ParametrosLecturaValorizacionDiaria.TiempoReal)
                    {
                        fecha = fechaActual.ToString(Constantes.FormatoFecha);
                    }
                    break;
        ...
        public static string GenerarBodyMail(string lectura, string formato, bool enPlazo, string empresa, DateTime fechaInicio, DateTime fechaFin, string areaCoes, string usuario, DateTime fechaEnvio, int idEnvio)
        {
            ...
            strHtml.Append("        <P>Por medio del presente, se le comunica que se registró " + stCumplimientoMensaje + " en el portal extranet la información de Valorización Diaria");
            strHtml.Append(" de su representada en atención a lo  dispuesto en el <strong>Procedimiento Técnico N° 41 - </strong> “ Información Valorización Diaria para la Operación del SEIN");
