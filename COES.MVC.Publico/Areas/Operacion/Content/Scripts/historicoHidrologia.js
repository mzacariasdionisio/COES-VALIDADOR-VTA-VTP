var controlador = siteRoot + 'operacion/HistoricoHidrologia/';
...
$(document).ready(function () {
    ...
    let select = $("#cbTipoInformacion");
    let optionExists = select.find("option[value='75']").length > 0;

    if (optionExists) {
        select.val("75").trigger("change"); // Asigna la opción con ID 75 y dispara el evento de cambio
    }
...
function cambiarFormatoFecha(tipo) {
    switch (tipo) {
        ...
        default:
            ...
            $('#FechaHasta').val(stFecha);
            setTimeout(() => {
                // Forzar re-render del input con el ícono de calendario
                $('#FechaDesde, #FechaHasta').css("display", "none").offset();
                $('#FechaDesde, #FechaHasta').css("display", "block");
            }, 50);
            if (flag2 == 1) {
...
function generarGrafico(nroPagina) {
    ...
    $.ajax({
        type: 'POST',
        url: controlador + "graficoreporte",
        data: {
...
function exportarExcel() {
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (result == 1) {//Horas
                window.location = controlador + "ExportarReporte?tipoReporte=10";
            }
            if (result == 2) {//PROGRAMADO SEMANAL
                window.location = controlador + "ExportarReporte?tipoReporte=4";
            }
            if (result == 3) {//Mensual, Semanal-diario
                window.location = controlador + "ExportarReporte?tipoReporte=0";
            }
            if (result == 4) {//Semanal - Dia.
                window.location = controlador + "ExportarReporte?tipoReporte=6";
            }
            if (result == -1) {
...
function exportarGrafDiarioM1() {
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoGrafM1",
        data: {
            fechaInicial: $('#hfFechaDesde').val(), fechaFinal: $('#hfFechaHasta').val()
        },
        dataType: 'json',
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarReporte?tipoReporte=8";
            }
            if (result == -1) {
                mostrarError();
            }
...
function exportarGrafDiarioM24() {
    $.ajax({
        type: 'POST',
        url: controlador + "GenerarArchivoGrafM24",
        data: {
            fechaInicial: $('#hfFechaDesde').val(), fechaFinal: $('#hfFechaHasta').val()
        },
        dataType: 'json',
        success: function (result) {
            if (result == 1) {
                window.location = controlador + "ExportarReporte?tipoReporte=3";
            }
            if (result == -1) {
                mostrarError();
            }
