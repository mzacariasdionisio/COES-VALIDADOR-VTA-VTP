...
namespace COES.MVC.Publico.Areas.Operacion.Controllers
{
    public class CostosMarginalesController : Controller
    {
        ...
        public JsonResult ListaProceso(string fecha)
        {
            ...
            List<CmCostomarginalDTO> list = this.servicio.ObtenerResultadoCostosMarginalesWeb(fechaConsulta, 1);

            foreach (CmCostomarginalDTO item in list)
            {
                item.FechaProceso = item.Cmgnfecha.ToString(Constantes.FormatoHoraMinuto);
