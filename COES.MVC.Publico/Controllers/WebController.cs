...
namespace COES.Storage.Controllers
{
    public class WebController : Controller
    {
        ...
        {            
            ...
            WbBlobDTO blob = this.servicio.ObtenerBlobPorUrl(libreria);
        ...
        public PartialViewResult VistaDatos(string baseDirectory, string url, string indicador, string initialLink, string orderFolder)
        {
            ...
            if (!indicadorTree)
            {
                ...
                model.Formulario = COES.MVC.Publico.Helper.Helper.ObtenerFormulario(busqueda);
        ...
        public PartialViewResult VistaDatosArbol(string baseDirectory, string url, string indicador, string initialLink)
        {
            ...
            List<FileData> listPrincipal = this.fileManager.HabilitarVistaDatosWeb(url, out columnas, string.Empty, out ordenamiento, out issuuind,
                out issuulink, out issuupos, out issuulenx, out issuuleny, indicador, out indHeader, out indicadorTree);
            
            int[] idsGeneral = { 82127, 82150, 82149, 92101, 94852 };          
            List<FileData> listGeneral = listPrincipal.Where(x => !idsGeneral.Contains(x.Blobcodi)).ToList();

            //82126: 82127
            //82130: 82150, 82149, 92101

            List<FileData> list = new List<FileData>();
            foreach (FileData item in listGeneral)
            {
                if (item.Blobcodi == 82130)
                {
                    item.IndMain = Constantes.SI;
                }

                item.Padrecodi = -1;
                list.Add(item);

                if (item.Blobcodi == 82126)
                {
                    List<FileData> subList = listPrincipal.Where(x => x.Blobcodi == 82127).ToList();
                    foreach (FileData itemSub in subList)
                    {
                        itemSub.IndMain = Constantes.NO;
                        itemSub.Padrecodi = -1;
                ...
                if (item.Blobcodi == 82130)
                {
                    List<FileData> subList = listPrincipal.Where(x => x.Blobcodi == 82150 || x.Blobcodi == 82149 || x.Blobcodi == 92101 || x.Blobcodi == 94852).ToList();
                    foreach (FileData itemSub in subList)
                    {
                        itemSub.IndMain = Constantes.NO;
                        itemSub.Padrecodi = item.Blobcodi;
        ...
        /// <returns></returns>
        [HttpPost]
        public PartialViewResult Busqueda(string url, string ids, string valores, int nroPagina)
        {
            BrowserModel model = new BrowserModel();
            ...
            if (!string.IsNullOrEmpty(ids) && !string.IsNullOrEmpty(valores))
            {
                ...
                List<WbBlobcolumnDTO> columnas = new List<WbBlobcolumnDTO>();
                List<FileData> list = this.fileManager.BuscarArchivos(url, out columnas, datos, nroPagina, Constantes.PageSize);
                model.ListaColumnas = columnas;
                model.DocumentList = list;
            }
        ...
        public PartialViewResult Paginado(string url, string ids, string valores)
        {
            ...
            if (!string.IsNullOrEmpty(ids) && !string.IsNullOrEmpty(valores))
            {
                ...
                List<WbBlobcolumnDTO> columnas = new List<WbBlobcolumnDTO>();
                int nroRegistros = this.fileManager.ObtenerNroRegistrosBusqueda(url, datos);
