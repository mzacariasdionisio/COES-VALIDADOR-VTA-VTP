...
namespace COES.Servicios.Aplicacion.CortoPlazo
{
    ...
    /// </summary>
    public class CostoMarginalAppServicio
    {
        /// <summary>
        ...
        /// <summary>
        /// Permite ejecutar el proceso de cálculo de costos marginales
        /// </summary>
        /// <param name="fechaProceso">Fecha para el cual se está ejecutando el proceso.</param>
        /// <param name="indicadorPSSE">Indica si se utilizará un archivo automáticamente o se especificará uno</param>
        /// <param name="reproceso">Indica si es un proceso en tiempo real o es un reproceso</param>
        /// <param name="indicadorNCP">Indica si se tomará un archivo NCP por defecto o uno manual</param>
        /// <param name="flagWeb">Indica si los resultados se mostrarán en el Portal Web</param>
        /// <param name="rutaNCP">Indica la ruta de donde se colocarán los archivos NCP para el caso en que sea manual</param>
        /// <param name="flagMD">Indica si se utilizará la fuente NCP o YUPANA</param>
        /// <param name="idEscenario">Código de escenario a utilizar en caso se especifuqe escenario manualmente</param>
        public void Procesar(DateTime fechaProceso, int indicadorPSSE, bool reproceso, bool indicadorNCP, bool flagWeb, string rutaNCP, bool flagMD, int idEscenario, string usuario, int tipo)
        {
            ...
            this.ProcesarGeneral(fechaProceso, indicadorPSSE, reproceso, indicadorNCP, false, out indicadorProceso, flagWeb, rutaNCP, flagMD, idEscenario, usuario, tipo);
        }
            ...
            try
            {
                ...
                for (int i = 0; i <= nroDias; i++)
                {
                    foreach (string hora in horas)
                    {
                        ...
                        bool indicadorProceso = false;
                        this.ProcesarGeneral(fechaProceso, 0, true, false, true, out indicadorProceso, false, string.Empty, flagMD, 0, usuario, 1);
        ...
        private void ProcesarGeneral(DateTime fechaProceso, int indicadorPSSE, bool reproceso, bool indicadorNCP, bool indicadorMasivo, out bool indicadorProceso, 
            bool flagWeb, string rutaNCP, bool flagMD, int idEscenario, string usuario, int tipo)
        {
            ...
            try
            {
                ...
                int periodo = UtilCortoPlazo.CalcularPeriodo(fechaProceso);
                ...
                //- Debemos realizar la obtención del programa o reprograma para NCP y MDCOES
                FileHelper.VerificarExistenciaReprograma(fechaProceso, pathRaiz, indicadorNCP, out reprograma, rutaNCP, flagMD, out topologiaFinal, idEscenario, periodo);
                                
                //- Carpeta con los datos de la corrida
                bool flagPrograma = false;
                string pathTrabajo = FileHelper.EstablecerCarpetaTrabajo(fechaProceso, pathRaiz, correlativo, out flagPrograma, flagMD, topologiaFinal);
                ...
                if (flagPrograma)
                {
                    ...
                    string filePsse = ConstantesCortoPlazo.ArchivoPSSE + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionRaw);
                    bool indicadorPsse = FileHelper.ObtenerArchivoRaw(filePsse, pathTrabajo, pathRaiz, fechaProceso, indicadorPSSE);
                    ...
                    if (indicadorPsse)
                    {
                        ...
                        List<NombreCodigoLinea> relacionLinea = FileHelper.ObtenerLineas(pathTrabajo + filePsse, pathRaiz, relacionBarra);

                        //- Obtenemos la relacion de los trafos 2D y 3D
                        List<TrafoEms> relacionTrafo = FileHelper.ObtenerListadoTrafo(pathTrabajo + filePsse, pathRaiz, relacionBarra);
                        ...
                        string fileResultado = ConstantesCortoPlazo.ArchivoProprocesador + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionGen);
                        FileHelper.GenerarArchivoResultado(resultadoGeneracion, resultadoCongestion, resultadoCongestionGrupo, resultadoForzada,
                            pathRaiz, pathTrabajo + fileResultado);

                        //- Generamos el archivo Log de datos
                        string fileLog = ConstantesCortoPlazo.ArchivoLOG + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        FileHelper.GenerarArchivoLog(pathTrabajo + fileLog, pathRaiz, log);

                        //- Generamos el archivo DAT que será entrada al GAMS

                        //- Creamos los objetos: contienen sus propiedades calculadas
                        List<LineaEms> objetoLinea = UtilCortoPlazo.ObtenerObjetoLinea(relacionLinea);

                        //- Obtenemos las cargas
                        List<Carga> relacionCarga = UtilCortoPlazo.ObtenerCargas(pathTrabajo + filePsse, pathRaiz, relacionBarra);
                        
                        //- Obtener los shunts
                        List<Shunt> relacionShunt = UtilCortoPlazo.ObtenerShunts(pathTrabajo + filePsse, pathRaiz);

                        //- Obtener datos de generadores
                        List<Generator> relacionGeneracion = UtilCortoPlazo.ObtenerGeneracion(datosEPPS);

                        //- Lectura de Switch dinámicos
                        List<SwitchedShunt> relacionShuntDinamicos = UtilCortoPlazo.ObtenerShuntsDinamicos(pathTrabajo + filePsse, pathRaiz);
                        ...
                        string entradaGams = UtilCortoPlazo.GenerarEntradaGams(relacionCarga, relacionBarra, relacionShunt, objetoLinea, relacionTrafo, resultadoGeneracion,
                            resultadoForzada, resultadoCongestion, resultadoCongestionGrupo, relacionBarra);

                        ////- Escribimos en una ruta el archivo entrada
                        string fileEntradaGams = ConstantesCortoPlazo.ArchivoEntradaGams + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionDat);
                        bool flagEntradaGams = FileHelper.GenerarArchivoEntradaGams(pathTrabajo + fileEntradaGams, pathRaiz, entradaGams);                                               

                        //- Ejecucion gams
                        string fileSalidaGams = ConstantesCortoPlazo.ArchivoResultadoGams + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        string fileSalidaGamsAnalisis = ConstantesCortoPlazo.ArchivoResultadoGams2 + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        //string fileSalidaGamsAlternativo = ConstantesCortoPlazo.ArchivoResultadoAlternativoGams + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        //string fileSalidaGamsAlternativoAnalisis = ConstantesCortoPlazo.ArchivoResultadoAlternativoGamsAnalisis + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        ...
                        try
                        {
                            ...
                            int resultadoGams = EjecucionGams.Ejecutar(fechaProceso, pathRaiz, pathTrabajo + fileEntradaGams, pathTrabajo + fileSalidaGams, pathTrabajo + fileSalidaGamsAnalisis, null);
                            #endregion

                            listaResultado = FileHelper.ObtenerResultadoGams(pathTrabajo + fileSalidaGams, pathRaiz);
                        ...
                        {                        
                            ...
                            string entradaGamsAC = UtilCortoPlazo.GenerarEntradaGamsAC(relacionCargaAC, relacionBarraAC, relacionShuntAC, objetoLineaAC, relacionTrafoAC, resultadoGeneracionAC,
                            ...
                            string fileEntradaGamsAC = ConstantesCortoPlazo.ArchivoEntradaGamsAC + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionDat);
                            bool flagEntradaGamsAC = FileHelper.GenerarArchivoEntradaGams(pathTrabajo + fileEntradaGamsAC, pathRaiz, entradaGamsAC);

                            string fileSalidaCmgAC = ConstantesCortoPlazo.ArchivoResultadoAC + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                            string fileSalidaFlujoAC = ConstantesCortoPlazo.ArchivoFlujoAC + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                            string fileSalidaAddAC = ConstantesCortoPlazo.ArchivoAddAC + UtilCortoPlazo.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        ...
                        (new CortoPlazoAppServicio()).AlmacenarCostosMarginales(listaResultado, fechaProceso, correlativo,
                              relacionBarra, true, false, out flagValidacionDatos, 0, usuario, flagMD);
                        ...
                        try
                        {
                            ...
                            //    ConstantesCortoPlazo.EstimadorPSS);
                        }
                        catch (Exception ex)
                        {
                        ...
                        try
                        {
                            ...
                            versionPrograma.Cmveprtipoestimador = ConstantesCortoPlazo.EstimadorPSS;
                        ...
                        {
                            Logger.Error("Error grabar datos adicionales (1): " + fechaProceso, ex);
                        }
                    }
                    else
                    {
            ...
            if (flagInconsistencias && flagModosOperacion && flagOperacionEMS)
            {
                ...
                string mensajeInconsistencia = UtilCortoPlazo.ObtenerMensajeInconsistencia(logInconsistencias);
                string mensajeModoOperacion = UtilCortoPlazo.ObtenerMensajeModoOperacion(logModosOperacion);
                string mensajeOperacionEMS = UtilCortoPlazo.ObtenerMensajeOperacionEMS(logOperacionEMS);
            ...
            else if (flagInconsistencias && !flagModosOperacion && !flagOperacionEMS)
            {
                ...
                mensaje = UtilCortoPlazo.ObtenerMensajeInconsistencia(logInconsistencias);
            }
            else if (!flagInconsistencias && flagModosOperacion && !flagOperacionEMS)
            {
                indicadorOperacion = ConstantesCortoPlazo.ErrorModosOperacion;
                mensaje = UtilCortoPlazo.ObtenerMensajeModoOperacion(logModosOperacion);
            }
            else if (!flagInconsistencias && !flagModosOperacion && flagOperacionEMS)
            {
                indicadorOperacion = ConstantesCortoPlazo.ErrorOperacionEMS;
                mensaje = UtilCortoPlazo.ObtenerMensajeOperacionEMS(logOperacionEMS);
            }
            else if (flagInconsistencias && flagModosOperacion && !flagOperacionEMS)
            {
            ...
            string mensajeEmail = UtilCortoPlazo.ObtenerMensajeCorreo(indicadorOperacion, mensaje, fechaProceso, flagValidacionDatos, flagWeb, flagValoresNegativos);

            try
            {
                if (!indicadorMasivo)
                {
                    //UtilCortoPlazo.EnviarCorreo(fechaProceso, mensajeEmail, reproceso);
                }
            }
            catch (Exception ex)
            {
        ...
        public string ValidacionProceso(DateTime fechaProceso)
        {
            ...
            int validacionPSSE = FileHelper.VerificarExistenciaRaw(fechaProceso, out pathpsse, out filepsse);

            //- Verificamos Horas de Operación
            //- Obtenemos la configuracion de los nombres y códigos de las barras
            List<NombreCodigoBarra> relacionBarra = FileHelper.ObtenerBarras(filepsse, pathpsse);
            ...
            if (!string.IsNullOrEmpty(validacion))
            {
                ...
                string mensaje = UtilCortoPlazo.ObtenerCuerpoCorreoValidacion(validacion, fechaProceso);
        ...
        public ResultadoValidacion ObtenerAlertasCostosMarginales(DateTime fechaProceso)
        {
            ...
            if (validacionPSSE == 1)
            {
                ...
                int validacionPSSEAnterior = FileHelper.VerificarExistenciaRaw(fechaProcesoAnterior, out pathpsseAnterior, out filepsseAnterior);

                if (validacionPSSEAnterior == 1)
                {
                    List<NombreCodigoBarra> relacionBarraAnterior = FileHelper.ObtenerBarras(filepsseAnterior, pathpsseAnterior);
                    List<string[]> datosEPPSAnterior = FileHelper.ObtenerDatosEPPS(filepsseAnterior, pathpsseAnterior);
            ...
            //-Preparamos el correo a enviar
            ResultadoValidacion validacion = UtilCortoPlazo.ObtenerDetalleValidacion(validacionPSSE, validacionNCP, horasOperacion, listadoRsf,
            ...
            bool flagMD, List<CpRecursoDTO> listaBarraCentral)
        {
            ...
            //- Linea agregada para la verificacion de alarmas de CM 05/03/2020
            List<EqRelacionDTO> modosOperacionLimTrans = FactorySic.GetEqRelacionRepository().ObtenerModosOperacionLimiteTransmision(fechaProceso);
            ...
            NombreCodigoBarra adicional = null;
            genFenixG11.Codbarra = UtilCortoPlazo.ObtenerCodigoBarra(genFenixG11.Nombarra, list, out tension1, out adicional);
            genFenixG12.Codbarra = UtilCortoPlazo.ObtenerCodigoBarra(genFenixG12.Nombarra, list, out tension1, out adicional);
            genFenixTV.Codbarra = UtilCortoPlazo.ObtenerCodigoBarra(genFenixTV.Nombarra, list, out tension1, out adicional);

            string[] datosFenixG11 = UtilCortoPlazo.ObtenerValorPSSE(genFenixG11.Codbarra, genFenixG11.Idgener, datosPSSE);
            string[] datosFenixG12 = UtilCortoPlazo.ObtenerValorPSSE(genFenixG12.Codbarra, genFenixG12.Idgener, datosPSSE);
            string[] datosFenixTV = UtilCortoPlazo.ObtenerValorPSSE(genFenixTV.Codbarra, genFenixTV.Idgener, datosPSSE);

            if (datosFenixG11.Length == 2)
            {
            ...
            foreach (EqRelacionDTO entity in relacion)
            {
                ...
                string tension = string.Empty;
                NombreCodigoBarra barraAdicional = null;
                entity.Codbarra = UtilCortoPlazo.ObtenerCodigoBarra(entity.Nombarra, list, out tension, out barraAdicional);
                ...
                if (barraAdicional!= null)
                {
                    entity.Codbarra1 = barraAdicional.CodBarra;
                    entity.Tension1 = barraAdicional.Tension;
                }
                ...
                if (entity.IndTipo == ConstantesCortoPlazo.TipoTermica)
                {
                    ...
                    if (entity.Indcoes == ConstantesAppServicio.SI)
                    {
                        ...
                        else
                        {
                            ...
                            //- Determinación del limite de transmision 05/03/2020

                            EqRelacionDTO indLimTran = modosOperacionLimTrans.Where(x => x.Grupocodi == idModoOperacion).FirstOrDefault();

                            if (indLimTran != null)
                            {
                                if (indLimTran.LimiteTransmision == ConstantesAppServicio.SI)
                                {
                                    entity.IndLimTran = true;
                                }
                            }
                ...
                else if (entity.IndTipo == ConstantesCortoPlazo.TipoHidraulica)
                {
                    if (entity.Indcoes == ConstantesAppServicio.SI)
                    {
                        ...
                        //- Región modificada por cambio de NCP a MDCOES
                ...
                if (rsfDetalle != null)
                {
                    ...
                    entity.ValorRsf = (decimal)rsfDetalle.Rsfdetvalaut;
            ...
            log += "Cod. Barra" + ConstantesCortoPlazo.CaracterSeparacionCSV +
                ...
                "Limite Transmión" + ConstantesCortoPlazo.CaracterSeparacionCSV +
            ...
            foreach (EqRelacionDTO entity in relacion)
            {
                log += (entity.Codbarra == null ? "" : entity.Codbarra) + ConstantesCortoPlazo.CaracterSeparacionCSV +
                    ...
                    entity.IdModoOperacion + ConstantesCortoPlazo.CaracterSeparacionCSV + 
                    (entity.CapacidadRegulacion != null ? entity.CapacidadRegulacion : "") + ConstantesCortoPlazo.CaracterSeparacionCSV + 
                    entity.LimiteTransmision + ConstantesCortoPlazo.CaracterSeparacionCSV ;
                        ...
                        if (item.CapacidadRegulacion == ConstantesCortoPlazo.CapacidadRegulacionDiaria)
                        {
                            ...
                            else
                            {
                                ...
                                foreach (string[] itemPhidro in datosPotHidraulico)
                                {
                                    ...
                                    if (item.Nombrencp.Contains("ñ")) item.Nombrencp = item.Nombrencp.Replace("ñ", "�");
            ...
            List<NombreCodigoLinea> relacionLinea, List<TrafoEms> relacionTrafo)
        {
            try
            {
                ...
                foreach (EqCongestionConfigDTO entity in list)
                {
                    ...
                    {
                        LineaEms lineaEms = ObtenerLinea(out flujo, entity.Nodobarra1, entity.Nodobarra2, relacionBarra, entity.Idems, relacionLinea, out flujo1, out flujo2);
            ...
            List<TrafoEms> relacionTrafo, ref string log, out List<PrCongestionDTO> listCongestionConjunto)
        {
            ...
            foreach (PrCongestionDTO entity in list)
            {
                ...
                if (entity.Famcodi == ConstantesCortoPlazo.IdLineaTransmision)
                {
                    ...
                    LineaEms lineaEms = ObtenerLinea(out flujo, entity.Nodobarra1, entity.Nodobarra2, relacionBarra, entity.NombLinea, relacionLinea, out flujo1, out flujo2);
                ...
                else if (entity.Famcodi == ConstantesCortoPlazo.IdTrafo2D)
                {
                    ...
                    if (barra1 != null && barra2 != null)
                    {
                        TrafoEms trafo = relacionTrafo.Where(x =>
                            x.IdBarra1 == barra1.CodBarra &&
                            x.IdBarra2 == barra2.CodBarra &&
                            //x.Barra3.NombBarra == entity.Nodobarra3 && 
                            x.Orden == entity.NombLinea).FirstOrDefault();

                        if (trafo != null)
                        {
                            //entity.Flujo = Math.Max(Math.Max(trafo.Ip.Magnitude, trafo.Is0.Magnitude), trafo.It.Magnitude);
                            entity.Flujo = Math.Max(Math.Max(trafo.Pp, trafo.Ps), trafo.Pt);
                            result.Add(entity);
                        }
                ...
                {
                    //- Logica para considerar los trafos como suma de flujos
                    NombreCodigoBarra barra1 = relacionBarra.Where(x => x.NombBarra == entity.Nodobarra1 && entity.Nodobarra1 != null).FirstOrDefault();
                    NombreCodigoBarra barra2 = relacionBarra.Where(x => x.NombBarra == entity.Nodobarra2 && entity.Nodobarra2 != null).FirstOrDefault();
                    NombreCodigoBarra barra3 = relacionBarra.Where(x => x.NombBarra == entity.Nodobarra3 && entity.Nodobarra3 != null).FirstOrDefault();

                    if (barra1 != null && barra2 != null && barra3 != null)
                    {
                        TrafoEms trafo = relacionTrafo.Where(x =>
                           x.IdBarra1 == barra1.CodBarra &&
                           x.IdBarra2 == barra2.CodBarra &&
                           x.IdBarra3 == barra3.CodBarra &&
                           x.Orden == entity.NombLinea).FirstOrDefault();

                        double congestion = Math.Abs(trafo.Pp);

                        //- Agregando la congestión como conjunto de líneas

                        PrCongestionDTO congestionTrafo = new PrCongestionDTO();
                        congestionTrafo.ListaItems = new List<PrCongestionitemDTO>();
                        congestionTrafo.Flujo = congestion;

                        congestionTrafo.ListaItems.Add(
                            new PrCongestionitemDTO
                            {
                                Nombarra1 = entity.Nodobarra1,
                                Nombarra2 = entity.Nodobarra2,
                                NombLinea = entity.NombLinea
                            });
                        ...
                        congestionTrafo.ListaItems.Add(
                            ...
                            {
                                Nombarra1 = entity.Nodobarra1,
                                Nombarra2 = entity.Nodobarra3,
                                NombLinea = entity.NombLinea
                            });

                        resultadoTrafo.Add(congestionTrafo);
                    }
                }
            }
        ...
        /// <param name="relacionLinea"></param>
        /// <returns></returns>
        private LineaEms ObtenerLinea(out double flujo, string nodobarra1, string nodobarra2, List<NombreCodigoBarra> relacionBarra, string nombreLinea,
            List<NombreCodigoLinea> relacionLinea, out double flujo1, out double flujo2)
        {
            ...
            NombreCodigoBarra barra1 = relacionBarra.Where(x => x.NombBarra == nodobarra1 && nodobarra1 != null).FirstOrDefault();
            NombreCodigoBarra barra2 = relacionBarra.Where(x => x.NombBarra == nodobarra2 && nodobarra2 != null).FirstOrDefault();

            //linea asociada
            if (barra1 != null && barra2 != null)
            {
                NombreCodigoLinea linea = relacionLinea.Where(x =>
                    ((x.CodBarra1 == barra1.CodBarra && x.CodBarra2 == barra2.CodBarra) ||
                    (x.CodBarra1 == barra2.CodBarra && x.CodBarra2 == barra1.CodBarra)) && x.NombLinea == nombreLinea
                    ).FirstOrDefault();
                ...
                if (linea != null)
                {
                    lineaEms = new LineaEms(linea.CodBarra1, linea.CodBarra2, 0, 0, 1, linea.NombLinea, linea.Rps, linea.Xps, linea.Bsh, linea.GshP, linea.BshP,
                        linea.GshS, linea.BshS, linea.BitEstado, linea.VoltajePU1, linea.Angulo1, linea.VoltajePU2, linea.Angulo2, linea.Pot, 0, 0, string.Empty);

                    //flujo = Math.Max(lineaEms.IpP.Magnitude, lineaEms.IsP.Magnitude);
            ...
            foreach (int id in grupos)
            {
                ...
                foreach (PrCongestionDTO congestion in result)
                {
                    if (congestion.Grulincodi == id)
                    {
                        ...
                        double flujo1 = 0;
                        double flujo2 = 0;
                        LineaEms lineaEms = ObtenerLinea(out flujo, congestion.Nodobarra1, congestion.Nodobarra2, relacionBarra,
            ...
            List<NombreCodigoBarra> lista, List<string[]> datosEPPS, List<EqRelacionDTO> listaForzada, ref string log, out List<PrGenforzadaDTO> forzadaInicial)
        {
            ...
            //- INICIO MODIFICACION TEMPORAL PARA EL CASO DE RUBI

            //bool flagRubi = false;
            //List<NombreCodigoBarra> listBarraRubi = lista.Where(x => x.NombBarra.Trim() == "RUBI33").ToList();
            //if (listBarraRubi.Count == 2) flagRubi = true;
            //string codbarrag1 = string.Empty;
            //string codbarrag2 = string.Empty;
            //string tensiong1 = string.Empty;
            //string tensiong2 = string.Empty;

            //if (flagRubi)
            //{
            //    PrGenforzadaDTO genRubig1 = entitys.Where(x => x.Equicodi == 18082).First();
            //    PrGenforzadaDTO genRubig2 = entitys.Where(x => x.Equicodi == 18084).First();
            //    string[] datos01 = UtilCortoPlazo.ObtenerValorPSSE(listBarraRubi[0].CodBarra, genRubig1.Idgener, datosEPPS);
            //    string[] datos11 = UtilCortoPlazo.ObtenerValorPSSE(listBarraRubi[1].CodBarra, genRubig1.Idgener, datosEPPS);
            //    string[] datos02 = UtilCortoPlazo.ObtenerValorPSSE(listBarraRubi[0].CodBarra, genRubig2.Idgener, datosEPPS);
            //    string[] datos12 = UtilCortoPlazo.ObtenerValorPSSE(listBarraRubi[1].CodBarra, genRubig2.Idgener, datosEPPS);

            //    if (datos01[0] != null && datos01[1] != null)
            //    {
            //        codbarrag1 = listBarraRubi[0].CodBarra;
            //        tensiong1 = listBarraRubi[0].Tension;
            //    }
            //    else if (datos11[0] != null && datos11[1] != null)
            //    {
            //        codbarrag1 = listBarraRubi[1].CodBarra;
            //        tensiong1 = listBarraRubi[1].Tension;
            //    }

            //    if (datos02[0] != null && datos02[1] != null)
            //    {
            //        codbarrag2 = listBarraRubi[0].CodBarra;
            //        tensiong2 = listBarraRubi[0].Tension;
            //    }
            //    else if (datos12[0] != null && datos12[1] != null)
            //    {
            //        codbarrag2 = listBarraRubi[1].CodBarra;
            //        tensiong2 = listBarraRubi[1].Tension;
            //    }
            //}

            //- FIN MODIFICACION TEMPORAL CASO DE RUBI


            foreach (PrGenforzadaDTO entity in entitys)
            {
                string tension = string.Empty;
                entity.Codbarra = this.ObtenerCodigoBarra(entity.Nombarra, lista, out tension);
                entity.Tension = tension;

                entity.Valor = 0;


                //- MODIFICADO POR CASO RUBI
                //if (entity.Equicodi == 18082)
                //{
                //    entity.Codbarra = codbarrag1;
                //    tension = tensiong1;
                //}
                //if (entity.Equicodi == 18084)
                //{
                //    entity.Codbarra = codbarrag2;
                //    tension = tensiong2;
                //}
                //- FIN MODIFICADO POR CASO RUBI


                log += "Cod Barra:" + (entity.Codbarra == null ? "" : entity.Codbarra) +
                    " Tension:" + (entity.Tension == null ? "" : entity.Tension);

                string[] datos = UtilCortoPlazo.ObtenerValorPSSE(entity.Codbarra, entity.Idgener, datosEPPS);
                decimal valor;
                if (datos != null)
                {
                    if (datos.Length > 0)
            ...
            foreach (EqRelacionDTO itemRel in listaForzada)
            {
                ...
                {
                    if (itemRel.Forzada && listExcluir.Where(x => x.Nombarra == itemRel.Nombarra && x.Idgener == itemRel.Idgener).Count() == 0)
                    {
                        item = new PrGenforzadaDTO();
                        item.ListaItems.Add(new PrgenforzadaitemDTO
                        {
            ...
            bool flagMD, List<CpMedicion48DTO> lstCmgBarras)
        {
                ...
                if (!item.IndEspecial)
                {
                    ...
                    if (item.IndTipo == ConstantesCortoPlazo.TipoTermica)
                    {
                        ...
                        if (item.Indcc == 0)
                        {
                            ...
                            entity.IndLimTrans = result.IndLimTrans;
                            entity.CostoIncremental = result.CostoIncremental;
                            entity.Cod = contador;
                            contador++;
                    ...
                    if (item.Indnoforzada == ConstantesAppServicio.SI)
                    {
            ...
            foreach (int idCiclo in idCcombinados)
            {
                ...
                foreach (RegistroGenerado itemTG in tgcc)
                {
                    if (flagTV && itemTG.IndOpe == 1)
                    {
                        ...
                        else
                        {
                            ...
                            itemTG.Cod = contador;
                            itemTG.IndLimTrans = result.IndLimTrans;
                            itemTG.CostoIncremental = result.CostoIncremental;
                ...
                tvcc.IndLimTrans = resultTV.IndLimTrans;
                tvcc.CostoIncremental = resultTV.CostoIncremental;
                tvcc.Cod = contador;
        ...
        public RegistroGenerado ObtenerRegistroTermico(EqRelacionDTO entity, RegistroGenerado registroPadre)
        {
            ...
            registro.IndLimTrans = entity.IndLimTran;
            ...
            if (coordenadas.Count > 1)
            {
                ...
                for (int i = 1; i < coordenadas.Count; i++)
                {
                    ...
                    //- Realizamos el ajuste para determinar el costo incremental 05032020

                    if ((decimal)entity.PotGenerada >= coordenadas[i - 1].Potencia && (decimal)entity.PotGenerada <= coordenadas[i].Potencia)
                    {
                        registro.CostoIncremental = (double)ci;
                    }

                    //- Fin ajuste de calculo de costo incremental
