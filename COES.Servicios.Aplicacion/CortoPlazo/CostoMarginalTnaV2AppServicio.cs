...
{
    public class CostoMarginalTnaV2AppServicio
    {
        /// <summary>
        ...
        public void ProcesarMasivo(DateTime fechaInicio, DateTime fechaFin, List<string> horas, bool flagMD, string usuario)
        {
            try
            {
                ...
                UtilCortoPlazoTnaV2.EnviarCorreoEjecucionReprocesoMasivo(entitys, fechaInicio, fechaFin);
            }
            catch (Exception ex)
            {
        ...
        public void ProcesarMasivoModificado(string[][] datos, string usuario)
        {
            try
            {
                ...
                UtilCortoPlazoTnaV2.EnviarCorreoEjecucionReprocesoMasivoModificado(entitys);
            }
            catch (Exception ex)
            {
                Logger.Error(ConstantesAppServicio.LogError, ex);
        ...
        public void ProcesarMasivoTIE(ParametrosAnguloOptimo result, string usuario)
        {
            try
            {
                ...
                UtilCortoPlazoTnaV2.EnviarCorreoEjecucionReprocesoTIE(entitys);
            }
            catch (Exception ex)
            {
                Logger.Error(ConstantesAppServicio.LogError, ex);
        ...
        /// <param name="datos"></param>
        /// <param name="usuario"></param>
        public void ProcesarMasivoVA(string horas, DateTime fechaConsulta, string usuario, int version)
        {
            Logger.Info("T001");
            try
            {
                ...
                string pathTrabajo = ConfigurationManager.AppSettings["PathCostosMarginalesFS"];
                ...
                Logger.Info("T002");
                Logger.Info("T002 (horas):\t" + horas);
                Logger.Info("T002 (fechaConsulta):\t" + fechaConsulta);
                Logger.Info("T002 (version):\t" + version);
                Logger.Info("T002 (procesos):\t" + procesos.Count);

                foreach (int correlativo in correlativos)
                {
                    Logger.Info("T002: ETA 1");
                    Logger.Info("T002: ETA 1 (correlativo):\t" + correlativo);
                    CmCostomarginalDTO cm = procesos.Where(x => x.Cmgncorrelativo == correlativo).FirstOrDefault();
                    Logger.Info("T002: ETA 2");
                    DateTime fecha = fechaConsulta;
                    Logger.Info("T002: ETA 3");

                    if(cm == null)
                    {
                        Logger.Info("T002: ETA CM ES NULL");
                    }
                    
                    if (cm.Cmgnfecha.Hour == 23 && cm.Cmgnfecha.Minute == 59) fecha = fechaConsulta.AddDays(1);
                    Logger.Info("T002: ETA 4");
                    ...
                    Logger.Info("T003 (path):\t" + path);
                    Logger.Info("T003 (pathTrabajo):\t" + pathTrabajo);
                    List<FileData> files = FileServerScada.ListarArhivos(path, pathTrabajo);
                    //List<FileData> filesFS = files = FileServerScada.ListarArhivos(path, pathCostosMarginalesFS);
                    Logger.Info("T003 (files):\t" + files.Count);
                    //Logger.Info("T003 (filesFS):\t" + filesFS.Count);
                    ...
                    if (dat == null) Logger.Info("T004 (dat) ES NULO");

                    if (dat != null)
                    {
                        string fileName = ConstantesCortoPlazo.FileForzarVA;
                        string pathRaw = String.Format(ConfigurationManager.AppSettings[ConstantesCortoPlazo.PathExportacionTNA] + "{0:0000}\\{1:00}\\{2:00}\\", fecha.Year, fecha.Month, fecha.Day);

                        if (FileServerScada.VerificarExistenciaFile("forzarva", fileName, pathRaw))
                        {
                            FileServerScada.DeleteBlob(@"forzarva\" + fileName, pathRaw);
                        }

                        //FileServerScada.CopiarFileAlter(path + dat.FileName, @"forzarva\", fileName, path);
                       
                        FileServerScada.UploadFromStream(FileServerScada.DownloadToStream(path + dat.FileName, pathTrabajo), @"forzarva\", fileName, pathRaw);

                        bool indicadorProceso = false;
                        string mensaje = string.Empty;
                        DateTime fechaProceso = cm.Cmgnfecha;
                        if (cm.Cmgnfecha.Hour == 23 && cm.Cmgnfecha.Minute == 59)
                        {
                            fechaProceso = new DateTime(cm.Cmgnfecha.Year, cm.Cmgnfecha.Month, cm.Cmgnfecha.Day).AddDays(1);                            
                        }

                        this.ProcesarGeneral(fechaProceso, 4, true, false, true, out indicadorProceso, false, string.Empty, true,
                            0, usuario, 4, fileName, out mensaje);

                        ResultadoProcesoMasivo itemMensaje = new ResultadoProcesoMasivo();
                        itemMensaje.FechaProceso = cm.Cmgnfecha.ToString(ConstantesAppServicio.FormatoFechaHora);
                        itemMensaje.Resultado = indicadorProceso;
                        itemMensaje.Mensaje = mensaje;
                ...
                Logger.Info("T005");
                UtilCortoPlazoTnaV2.EnviarCorreoEjecucionReprocesoVA(entitys);
        ...
        {
            Logger.Info("CostoMarginalTnaV2AppServicio (ProcesarGeneral)");
            ...
            try
            {
                ...
                int periodo = UtilCortoPlazoTnaV2.CalcularPeriodo(fechaProceso);
                ...
                FileHelperTnaV2.VerificarExistenciaReprograma(fechaProceso, pathRaiz, indicadorNCP, out reprograma, rutaNCP, flagMD, out topologiaFinal, idEscenario, periodo);

                //- Carpeta con los datos de la corrida
                bool flagPrograma = false;
                string pathTrabajo = FileHelperTnaV2.EstablecerCarpetaTrabajo(fechaProceso, pathRaiz, correlativo, out flagPrograma, flagMD, topologiaFinal);

                Logger.Info("rutaNCP:\t" + rutaNCP);
                Logger.Info("fileReproceso:\t" + fileReproceso);
                Logger.Info("pathRaiz:\t" + pathRaiz);
                Logger.Info("flagPrograma:\t" + flagPrograma.ToString());

                if (flagPrograma)
                {

                    //- Obtenemos el archivo .raw
                    string filePsse = ConstantesCortoPlazo.ArchivoPSSE + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionRaw);
                    Logger.Info("raw => " + filePsse + " - " + pathTrabajo + " - " + pathRaiz + " - " + fileReproceso);
                    //-- Cambio para nuevo TNA --//
                    bool indicadorPsse = FileHelperTnaV2.ObtenerArchivoRaw(filePsse, pathTrabajo, pathRaiz, fechaProceso, indicadorPSSE, fileReproceso);

                    Logger.Info("indicadorPsse:\t" + indicadorPsse.ToString());
                    ...
                    if (indicadorPsse)
                    {
                        ...
                        //-- Cambio para nuevo TNA --//
                        List<NombreCodigoBarra> relacionBarra = FileHelperTnaV2.ObtenerBarras(pathTrabajo + filePsse, pathRaiz);

                        //- Obtenemos la configuracion de las líneas
                        //- Cambio para nuevo TNA --//
                        List<NombreCodigoLinea> relacionLinea = FileHelperTnaV2.ObtenerLineas(pathTrabajo + filePsse, pathRaiz, relacionBarra);

                        //- Obtenemos la relacion de los trafos 2D y 3D
                        //- Cambio para nuevo TNA --//
                        List<TrafoEms> relacionTrafo = FileHelperTnaV2.ObtenerListadoTrafo(pathTrabajo + filePsse, pathRaiz, relacionBarra);
                        ...
                        List<string[]> datosEPPS = FileHelperTnaV2.ObtenerDatosEPPS(pathTrabajo + filePsse, pathRaiz);
                        ...
                        List<EqRelacionDTO> list = this.ObtenerRelacion(relacionBarra, datosEPPS, fechaProceso, ref log, equivalenciaBarraCentral, reproceso, out flagFenixValidacion,
                            out listaRestricciones, flagMD, listaRelBarraCentral);

                        #region CMgCP_PR07
                        List<CmVolumenInsensibilidadDTO> listVolumenInsensibilidad = FactorySic.GetCmVolumenInsensibilidadRepository().ObtenerRegistros(fechaProceso);
                        ...
                        try
                        {
                            ...
                            Logger.Info("ProcesarCmVolumenDetalle:\tfechaProceso\n" + fechaProceso.ToString());
                            Logger.Info("ProcesarCmVolumenDetalle:\tperiodo\n" + periodo);
                            Logger.Info("ProcesarCmVolumenDetalle:\treproceso\n" + reproceso.ToString());

                            (new HidrologiaAppServicio()).ProcesarCmVolumenDetalle(fechaProceso, periodo, reproceso);
                        ...
                        List<CmVolumenDetalleDTO> listVolumenDetalle = (new HidrologiaAppServicio()).ObtenerVolumenProceso(fechaProceso, periodo, reproceso);

                        #endregion


                        //- Evaluacion de unidades para determinar cuales participan en la evaluacion de CM              
                        this.EvaluacionUnidades(fechaProceso, datosPotHidraulico, datosVolumenHidraulico, ref list, periodo, ref log, 
                            lstRecursosVolMaxMin, lstVolumenesEmbalses, flagMD, listVolumenInsensibilidad, listVolumenDetalle);
                        ...
                        #region Conjunto de Líneas de Flujo Mínimo

                        //- Obtenemos los datos de congestion conjunta
                        List<PrCongestionDTO> resultadoCongestionGrupoFlujoMinimo = this.ObtenerCongestionGrupoFlujoMinimo(fechaProceso, relacionBarra, relacionLinea, ref log, congestionTrafo3D);
                        ...
                        string fileResultado = ConstantesCortoPlazo.ArchivoProprocesador + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionGen);
                        FileHelperTnaV2.GenerarArchivoResultado(resultadoGeneracion, resultadoCongestion, resultadoCongestionGrupo, resultadoCongestionGrupoFlujoMinimo, resultadoForzada,
                            pathRaiz, pathTrabajo + fileResultado);


                        #region Quitamos las secciones temporalmente

                        //- Generamos el archivo Log de datos
                        string fileLog = ConstantesCortoPlazo.ArchivoLOG + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        FileHelperTnaV2.GenerarArchivoLog(pathTrabajo + fileLog, pathRaiz, log);

                        //- Generamos el archivo DAT que será entrada al GAMS

                        //- Creamos los objetos: contienen sus propiedades calculadas
                        List<LineaEms> objetoLinea = UtilCortoPlazoTnaV2.ObtenerObjetoLinea(relacionLinea);

                        //- Obtenemos las cargas
                        List<Carga> relacionCarga = UtilCortoPlazoTnaV2.ObtenerCargas(pathTrabajo + filePsse, pathRaiz, relacionBarra);

                        //- Obtener los shunts
                        List<Shunt> relacionShunt = UtilCortoPlazoTnaV2.ObtenerShunts(pathTrabajo + filePsse, pathRaiz);

                        ////- Obtener datos de generadores
                        //List<Generator> relacionGeneracion = UtilCortoPlazoTnaV2.ObtenerGeneracion(datosEPPS);

                        ////- Lectura de Switch dinámicos
                        //List<SwitchedShunt> relacionShuntDinamicos = UtilCortoPlazoTnaV2.ObtenerShuntsDinamicos(pathTrabajo + filePsse, pathRaiz);
                        ...
                        double paramM = (double)(new ParametroAppServicio()).ObtenerValorParametro(ConstantesCortoPlazo.IdParametroM, fechaProceso);
                        if (paramM == 0)
                            paramM = 0;

                        string entradaGams = UtilCortoPlazoTnaV2.GenerarEntradaGams(relacionCarga, relacionBarra, relacionShunt, objetoLinea, relacionTrafo, resultadoGeneracion,
                            resultadoForzada, resultadoCongestion, resultadoCongestionGrupo, congestionRegionSeguridad, resultadoCongestionGrupoFlujoMinimo, relacionBarra, paramM, tipo);
                        #endregion

                        ////- Escribimos en una ruta el archivo entrada
                        string fileEntradaGams = ConstantesCortoPlazo.ArchivoEntradaGams + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionDat);
                        bool flagEntradaGams = FileHelperTnaV2.GenerarArchivoEntradaGams(pathTrabajo + fileEntradaGams, pathRaiz, entradaGams);

                        //- Ejecucion gams
                        string fileSalidaGams = ConstantesCortoPlazo.ArchivoResultadoGams + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        string fileSalidaGamsAnalisis = ConstantesCortoPlazo.ArchivoResultadoGams2 + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        //string fileSalidaGamsAlternativo = ConstantesCortoPlazo.ArchivoResultadoAlternativoGams + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        //string fileSalidaGamsAlternativoAnalisis = ConstantesCortoPlazo.ArchivoResultadoAlternativoGamsAnalisis + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ConstantesCortoPlazo.ExtensionCsv);
                        ...
                        try
                        {
                            ...
                            workspace = "Workspace\\";
                            int resultadoGams = EjecucionGamsV2.Ejecutar(fechaProceso, pathRaiz, pathTrabajo + fileEntradaGams, pathTrabajo + fileSalidaGams, pathTrabajo + fileSalidaGamsAnalisis, pathTrabajo + workspace);
                            listaResultado = FileHelperTnaV2.ObtenerResultadoGams(pathTrabajo + fileSalidaGams, pathRaiz);

                            string fileGams = "MODELO_" + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ".gms");
                            string fileLst = "LST_" + UtilCortoPlazoTnaV2.ObtenerFileName(fechaProceso, ".lst");
                            FileHelperTnaV2.CopiarArchivoLST(pathTrabajo, pathTrabajo + workspace, fileGams, fileLst, pathRaiz);
                        ...
                        catch (Exception ex)
                        {
                            ...
                            Logger.Error("EjecucionGamsV2.Ejecutar" + fechaProceso, ex);
                        }
                        ...
                        try
                        {
                            ...
                            versionPrograma.Cmveprversion = ConstantesCortoPlazo.VersionCMPR07;
                            //versionPrograma.Cmveprtipocorrida = tipo.ToString();
                            ...
                            Logger.Info("toFS...");
                            Logger.Info("toFS...workspace: " + pathRaiz + " | " + pathTrabajo + " | " + workspace);
                            EjecucionGamsV2.toFS(pathRaiz, pathTrabajo + workspace);
                            Logger.Info("FileHelperTnaV2.CopiarArchivoLST...end");
                        }
                        catch (Exception ex)
                        {
                            Logger.Error("Error grabar datos adicionales (3): " + fechaProceso, ex);
                        }
            ...
            if (flagInconsistencias && flagModosOperacion && flagOperacionEMS)
            {
                ...
                string mensajeInconsistencia = UtilCortoPlazoTnaV2.ObtenerMensajeInconsistencia(logInconsistencias);
                string mensajeModoOperacion = UtilCortoPlazoTnaV2.ObtenerMensajeModoOperacion(logModosOperacion);
                string mensajeOperacionEMS = UtilCortoPlazoTnaV2.ObtenerMensajeOperacionEMS(logOperacionEMS);
            ...
            else if (flagInconsistencias && !flagModosOperacion && !flagOperacionEMS)
            {
                ...
                mensaje = UtilCortoPlazoTnaV2.ObtenerMensajeInconsistencia(logInconsistencias);
            }
            else if (!flagInconsistencias && flagModosOperacion && !flagOperacionEMS)
            {
                indicadorOperacion = ConstantesCortoPlazo.ErrorModosOperacion;
                mensaje = UtilCortoPlazoTnaV2.ObtenerMensajeModoOperacion(logModosOperacion);
            }
            else if (!flagInconsistencias && !flagModosOperacion && flagOperacionEMS)
            {
                indicadorOperacion = ConstantesCortoPlazo.ErrorOperacionEMS;
                mensaje = UtilCortoPlazoTnaV2.ObtenerMensajeOperacionEMS(logOperacionEMS);
            }
            else if (flagInconsistencias && flagModosOperacion && !flagOperacionEMS)
            {
            ...
            string mensajeEmail = UtilCortoPlazoTnaV2.ObtenerMensajeCorreo(indicadorOperacion, mensaje, fechaProceso, flagValidacionDatos, flagWeb, flagValoresNegativos);
            mensajeResultado = mensajeEmail;
            try
            {
                if (!indicadorMasivo)
                {
                    UtilCortoPlazoTnaV2.EnviarCorreo(fechaProceso, mensajeEmail, reproceso);
                }
            }
            catch (Exception ex)
            {
        ...
        public string ValidacionProceso(DateTime fechaProceso)
        {
            ...
            int validacionPSSE = FileHelperTnaV2.VerificarExistenciaRaw(fechaProceso, out pathpsse, out filepsse);

            //- Verificamos Horas de Operación
            //- Obtenemos la configuracion de los nombres y códigos de las barras
            List<NombreCodigoBarra> relacionBarra = FileHelperTnaV2.ObtenerBarras(filepsse, pathpsse);
            ...
            if (!string.IsNullOrEmpty(validacion))
            {
                ...
                string mensaje = UtilCortoPlazoTnaV2.ObtenerCuerpoCorreoValidacion(validacion, fechaProceso);
        ...
        {
            //- Verificamos existencia de NCP
            int validacionNCP = FileHelperTnaV2.ValidarExistenciaNCP(fechaProceso);
            ...
            if (validacionPSSE == 1)
            {
                //- Obtenemos la configuracion de los nombres y códigos de las barras
                List<NombreCodigoBarra> relacionBarra = FileHelperTnaV2.ObtenerBarras(filepsse, pathpsse);
                List<string[]> datosEPPS = FileHelperTnaV2.ObtenerDatosEPPS(filepsse, pathpsse);
                ...
                int validacionPSSEAnterior = FileHelperTnaV2.VerificarExistenciaRaw(fechaProcesoAnterior, out pathpsseAnterior, out filepsseAnterior);

                if (validacionPSSEAnterior == 1)
                {
                    List<NombreCodigoBarra> relacionBarraAnterior = FileHelperTnaV2.ObtenerBarras(filepsseAnterior, pathpsseAnterior);
                    List<string[]> datosEPPSAnterior = FileHelperTnaV2.ObtenerDatosEPPS(filepsseAnterior, pathpsseAnterior);
            ...
            //-Preparamos el correo a enviar
            ResultadoValidacion validacion = UtilCortoPlazoTnaV2.ObtenerDetalleValidacion(validacionPSSE, validacionNCP, horasOperacion, listadoRsf,
        ...
        public List<EqRelacionDTO> ObtenerRelacion(List<NombreCodigoBarra> list, List<string[]> datosPSSE, DateTime fechaDatos, ref string log,
            ...
            bool flagMD, List<CpRecursoDTO> listaBarraCentral)
        {
            ...
            //- Obtenemos las unidades cuya centra es de pasada
            List<PrGenforzadaDTO> listForzada = FactorySic.GetPrGenforzadaRepository().ObtenerUnidadesPasada();
            ...
            double tipoCambio = parameterGeneral.GetEvaluate(ConstantesCortoPlazo.PropTipoCambio);
            double cvh = parameterGeneral.GetEvaluate(ConstantesCortoPlazo.PropValorAgua);

            //-- Inicio de duraceli- sev

            #region Codigo caso especial

            EqRelacionDTO genFenixG11 = relacion.Where(x => x.Equicodi == 13604).First();
            EqRelacionDTO genFenixG12 = relacion.Where(x => x.Equicodi == 13603).First();
            EqRelacionDTO genFenixTV = relacion.Where(x => x.Equicodi == 13602).First();

            string tension1 = string.Empty;
            //NombreCodigoBarra adicional = null;
            //genFenixG11.Codbarra = UtilCortoPlazoTnaV2.ObtenerCodigoBarra(genFenixG11.Nombarra, list, out tension1, out adicional);
            //genFenixG12.Codbarra = UtilCortoPlazoTnaV2.ObtenerCodigoBarra(genFenixG12.Nombarra, list, out tension1, out adicional);
            //genFenixTV.Codbarra = UtilCortoPlazoTnaV2.ObtenerCodigoBarra(genFenixTV.Nombarra, list, out tension1, out adicional);

            string[] datosFenixG11 = UtilCortoPlazoTnaV2.ObtenerValorPSSE(genFenixG11.Nombretna, datosPSSE);
            string[] datosFenixG12 = UtilCortoPlazoTnaV2.ObtenerValorPSSE(genFenixG12.Nombretna, datosPSSE);
            string[] datosFenixTV = UtilCortoPlazoTnaV2.ObtenerValorPSSE(genFenixTV.Nombretna, datosPSSE);
            ...
            foreach (EqRelacionDTO entity in relacion)
            {
                ...
                if (true)
                {
                            ...
                            if (datos.Length == 3)
                            {
                                if (datos[0] != null && datos[1] != null)
                                {
                                    if (flagPotencia) //-cambio realizado
                                    {
                                        ...
                                        //- Inicio ticket REQ 2024-000114

                                        List<EqRelacionTnaDTO> listaTnaAdicional = FactorySic.GetEqRelacionTnaRepository().GetByCriteria(entity.Relacioncodi);
                                        
                                        if(listaTnaAdicional.Count > 0)
                                        {
                                            entity.IndOperacion = 1.ToString();
                                        }
                                        
                                        foreach(EqRelacionTnaDTO itemAdicional in listaTnaAdicional)
                                        {
                                            string[] datosAdicional = UtilCortoPlazoTna.ObtenerValorPSSE(itemAdicional.Reltnanombre, datosPSSE);
                                            if (datosAdicional.Length == 3)
                                            {
                                                if (datosAdicional[0] != null && datosAdicional[1] != null)
                                                {
                                                    if (datosAdicional[1] == 1.ToString())
                                                    {
                                                        entity.PotGenerada = entity.PotGenerada + double.Parse(datosAdicional[0]);
                                                    }
                                                }
                                            }
                                        }

                                        //- Fin ticket REQ 2024-000114
                    ...
                    string tension = string.Empty;
                    entity.Nombarra = UtilCortoPlazoTnaV2.ObtenerCodigoBarra(entity.Codbarra, list, out tension);
                    ...
                    if (entity.IndTipo == ConstantesCortoPlazo.TipoTermica)
                    {
                        ...
                        if (entity.Indcoes == ConstantesAppServicio.SI)
                        {
                            ...
                            else
                            {
                                ...
                                double formulaPCICostoCombustibleVarios = Convert.ToDouble(listFormulas.FirstOrDefault(x => x.Concepcodi == ConstantesCortoPlazo.ConcepCodi_PCIPVariosSI)?.Formuladat);
                                double factor = 1;
                    ...
                    else if (entity.IndTipo == ConstantesCortoPlazo.TipoHidraulica)
                    {
                        if (entity.Indcoes == ConstantesAppServicio.SI)
                        {
                            ...
                            //- EqRelacionDTO propVelDescarga = propHidraulicas.Where(x => x.Equicodi == entity.Equicodi && x.Propcodi == ConstantesCortoPlazo.IdPropVelDescargaH).FirstOrDefault();
                            EqRelacionDTO propRendimiento = propHidraulicas.Where(x => x.Equicodi == entity.Equicodi && x.Propcodi == ConstantesCortoPlazo.IdPropRendimiento).FirstOrDefault();
                            ...
                            double rendimiento = this.ObtenerPropiedadHidraulico(propRendimiento, out flagRendimiento);
                            ...
                            entity.Rendimiento = rendimiento;
                            ...
                            //- Cambios por CMgCP_PR07
                            if (listForzada.Where(x => x.Equicodi == entity.Equicodi).Count() > 0)
                                entity.IndPasada = ConstantesAppServicio.SI;
        ...
        public void EvaluacionUnidades(DateTime fechaProceso, List<string[]> datosPotHidraulico, List<string[]> datosVolumenHidraulico, ref List<EqRelacionDTO> list,
            ...
            List<CmVolumenInsensibilidadDTO> listVolumenInsensibilidad, List<CmVolumenDetalleDTO> listVolumenDetalle)
        {
                ...
                if (item.IndTipo == ConstantesCortoPlazo.TipoHidraulica)
                {
                    ...
                    if (flagMD)
                    {
                        ...
                        if (lstPropiedadEmbalse.Any())
                        {
                            ...
                            double volumenProgramado = 0; // (new McpAppServicio()).ObtenerValorHxPorRecurso(lstVolumenesEmbalses, periodo, lstPropiedadEmbalse.First().Recurcodi) ?? 0;

                            CmVolumenDetalleDTO volDetalle = listVolumenDetalle.Where(x => x.Recurcodi == lstPropiedadEmbalse.First().Recurcodi).FirstOrDefault();

                            if (volDetalle != null)
                            {
                                volumenProgramado = (double)volDetalle.Volumen;
                            }
                            ...
                            double ViSup = 0;
                            double ViInf = 0;
                            double volumen = volumenProgramado; //- Debemos definir la forma cómo se obtendrá este valor

                            CmVolumenInsensibilidadDTO entityVolIns = listVolumenInsensibilidad.Where(x => x.Recurcodi == lstPropiedadEmbalse.First().Recurcodi).FirstOrDefault();

                            if (entityVolIns != null)
                            {
                                ViSup = (double)entityVolIns.Volinsvolmax;
                                ViInf = (double)entityVolIns.Volinsvolmin;
                            }
                            else
                            {
                                if (item.Rendimiento != 0)
                                {
                                    ViSup = Math.Min(item.PotenciaMaxima - item.PotenciaMinima, minutos * (double)item.VelocidadDescarga) * 600 / (item.Rendimiento * Math.Pow(10, 6));
                                    ViInf = Math.Min(item.PotenciaMaxima - item.PotenciaMinima, minutos * (double)item.VelocidadCarga) * 600 / (item.Rendimiento * Math.Pow(10, 6));
                                }
                            }


                            double VLSup = VolMax - ViSup;
                            double VLInf = VolMin + ViInf;

                            if (volumen >= VLInf && volumen <= VLSup)
                            {
                                item.IndVolumenB = ConstantesAppServicio.SI;
                            }
                            ...
                            "VOL CALCULADO :" + volumen + ", VOLMIN: " + VolMin + ", VOLMAX: " + VolMax +
                            ", VISup: " + Math.Round(ViSup, 6) + ", VIInf: " + Math.Round(ViInf, 6) + ConstantesCortoPlazo.SaltoLinea;
        ...
        /// <param name="relacion"></param>
        /// <returns></returns>
        private List<PrCongestionDTO> ObtenerCongestionGrupoFlujoMinimo(DateTime fechaDatos, List<NombreCodigoBarra> relacionBarra,
            List<NombreCodigoLinea> relacionLinea, ref string log, List<PrCongestionDTO> congestionTrafo3D)
        {
            DateTime fecha = (fechaDatos.Hour == 0 && fechaDatos.Minute == 0) ? fechaDatos.AddMinutes(-1) : fechaDatos;
            //- Agregamos una lista con las congestiones conjuntas para los trafos 3D

            List<PrCongestionDTO> list = FactorySic.GetPrCongestionRepository().ObtenerCongestionConjuntoRegistro(fecha, "2");
            List<PrCongestionDTO> result = new List<PrCongestionDTO>();

            log += "*** OBTENER CONGESTION CONJUNTO DE LÍNEAS FLUJO MÍNIMO" + ConstantesCortoPlazo.SaltoLinea;
            ...
            List<NombreCodigoBarra> lista, List<string[]> datosEPPS, List<EqRelacionDTO> listaForzada, ref string log, out List<PrGenforzadaDTO> forzadaInicial)
        {
            ...
            List<PrGenforzadaDTO> entitys = FactorySic.GetPrGenforzadaRepository().ObtenerGeneracionForzadaProcesoV2(fechaProceso)
            ...
            {
                string[] datos = UtilCortoPlazoTnaV2.ObtenerValorPSSE(entity.Nombretna, datosEPPS);
                ...
                string tension = string.Empty;
                entity.Nombarra = UtilCortoPlazoTnaV2.ObtenerCodigoBarra(entity.Codbarra, lista, out tension);
                entity.Tension = tension;
                //entity.Valor = 0;
            ...
            foreach (EqRelacionDTO item in list)
            {
                ...
                if (!item.IndEspecial)
                {
                    ...
                    if (item.IndTipo == ConstantesCortoPlazo.TipoHidraulica)
                    {
                        ...
                        #region Ticket_6245
                        entity.IndCmgh = ConstantesAppServicio.NO;
                        #endregion
                        ...
                        if (flagMD)
                        {
                            ...
                            #region Ticket_6245
                            entity.IndCmgh = ConstantesAppServicio.SI;
                            #endregion

                            //- CMgCP_PR07
                            if (item.Cvh > cmgPENMwh && item.IndVolumenB == ConstantesAppServicio.SI && item.IndPasada != ConstantesAppServicio.SI)
                            {
                                ci = item.Cvh.ToString();
                                #region Ticket_6245
                                entity.IndCmgh = ConstantesAppServicio.NO;
                                #endregion
                            }
                            if (item.IndVolumenB != ConstantesAppServicio.SI || item.IndPasada == ConstantesAppServicio.SI)
                            {
                                ci = item.Cvh.ToString();
                                entity.PotenciaMaxima = (decimal)item.PotGenerada;
                                entity.PotenciaMinima = (decimal)(item.PotenciaMinima / (1 - rpf));
