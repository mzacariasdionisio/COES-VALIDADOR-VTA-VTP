...
{
    public class EjecucionGamsV2
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(EjecucionGamsV2));
        private static string UserLogin = ConfigurationManager.AppSettings["UserFsScada"];
        private static string Domain = ConfigurationManager.AppSettings["DomainFsScada"];
        private static string Password = ConfigurationManager.AppSettings["PasswordFsScada"];
        private static string pathCostosMarginalesFS = ConfigurationManager.AppSettings["PathCostosMarginalesFS"];

        public static void toFS(string path, string workspace)
        {
            using (new Impersonator(UserLogin, Domain, Password, pathCostosMarginalesFS))
            {
                Task.Delay(TimeSpan.FromMinutes(1));
                string pathRoot = workspace.Replace("Workspace\\", "");
                string origen = path + pathRoot;
                string destino = pathCostosMarginalesFS + pathRoot;
            ...
            int resultado = -1;
                ...
                string pathWorkspace = ConfigurationManager.AppSettings["PathWorkspaceCM"] + workspace;
                    ...
                    using (GAMSOptions opt = ws.AddOptions())
                    {
                        ...
                        //Directory.CreateDirectory(pathCostosMarginalesFS + workspace);
                        Logger.Info(" - Ejecutando el modelo");
                        Logger.Info(" - path:\t" + path);
                        Logger.Info(" - parametros:\t" + parametros);
                        Logger.Info(" - salida:\t" + salida);
                        Logger.Info(" - salida2:\t" + salida2);
                        Logger.Info(" - pendiente:\t" + pendiente);
                    ...
                    if (FileServerScada.VerificarExistenciaFile(string.Empty, salida, path))
                    {
                        resultado = 1;
                    }
                    else
        ...
        public static string ObtenerModelo(string parametros, string salida, string salida2, string pendiente)
         {
             ...
             string result = @"
*******************************************************************************
* MODELO PARA OBTENER COSTO MARGINAL DE ENERGIA (INCLUIDA COMPONENTE P�RDIDAS)
* En el marco del Mercado Mayorista de Electricidad
* Elaborado por : COES SINAC
* Fecha: Octubre 2017
* actualizaci�n1: abril 2018
* actualizaci�n2: abril 2019
* actualizaci�n3: enero 2021
* actualizaci�n4: febrero 2021
*                              DERECHOS RESERVADOS
*******************************************************************************
option limrow=10000;
...
SCALAR
 ...
 PI /3.1416/
 Pendiente /0.95/
 CRacionamiento /2760.95/
 DRacionamiento /2760.95/
 Tolerancia /0.05/
 Variacion  /1/
 Variacion2 /0.15/;
...
**** NUEVO PR-07
* Para demanda de holgura (adicional)
  Demf(i), Demf1(i),Demf2(i),DRacTotal
****
...
***********************************************************************************
    ...
    loop(k, loop(l,  SFx[l,k]= EnlB[l]*sum{ (i,j)$FBus(l,i,j)  ,-(zbus[i,k] - zbus[j,k])} );
            ...
            loop(rd $CONGmin(rd,'Cong'),               SFrdx[rd,k]=sum{l$(CONGRuta_RS_D(rd,l) ),   SFx[l,k]} );
...
****RS Vertical
  RSdCongesMax[rd], RSdCongesMin[rd], RSdCongesMax2[rd], RSdCongesMin2[rd],
...
Operacion..
  costo =e=
   ...
   + sum{h$((PhData(h,'Cmgh'))), PhData(h,'CI')*( Ph(h) + (0.5*(M)* (Ph(h)-PhData(h,'Pgen'))*(Ph(h)-PhData(h,'Pgen')) /(PhData(h,'Pgen')) ) ) }
   + sum{h$(not(PhData(h,'Cmgh'))), PhData(h,'CI')*(Ph(h))}
...
*************Regiones de seguridad restricci�n vertical ******************


RSdCongesMax [rd]$(CONGMin(rd,'Cong'))..  sum(l$(CONGRuta_RS_D(rd,l)), F[l]+0.5*LossTF[l]+0.5*LossLF[l])  =g=  CONGMin(rd,'Pmin');
RSdCongesMin [rd]$(CONGMin(rd,'Cong'))..  - sum(l$(CONGRuta_RS_D(rd,l)),-F[l]+0.5*LossTF[l]+0.5*LossLF[l])  =g=  CONGMin(rd,'Pmin');


RSdCongesMax2 [rd]$(CONGMin(rd,'Cong'))..  sum(l$(CONGRuta_RS_D(rd,l)), F[l]+0.5*LossTF[l]+0.5*LossLFx[l])  =g=  CONGMin(rd,'Pmin');
RSdCongesMin2 [rd]$(CONGMin(rd,'Cong'))..  - sum(l$(CONGRuta_RS_D(rd,l)),-F[l]+0.5*LossTF[l]+0.5*LossLFx[l])  =g=  CONGMin(rd,'Pmin');
...
Balance2[i]..   RacP[i]+sum{ (t)$(PtBus(t,i)),Pt[t] }+sum{ (h)$(PhBus(h,i)),Ph[h] }
                ...
                =e=   Dem(i)-Falla[i] + DRac(i)+Demf(i);
**Perdidas[l]..   LossLFx[l]   =e=  FData(l,'R0')/Pbase * F(l)*F(l);
Perdidas[l]..   LossLFx[l]   =e=  (2*EnlG[l])* sum((i,j)$FBus(l,i,j),(1-cos(Ang[i] - Ang[j])) )*Pbase  ;
...
MODEL DCIter    /Operacion,Flujo,Balance1,                PotTer,TerFor,HidFor,EnlCongMax,EnlCongMin,CongesMax,CongesMin,RSdCongesMax,RSdCongesMin,Region1CongArribaMax,Region1CongAbajoMax/;
MODEL DCDirec   /Operacion,Flujo,Balance2,Perdidas,       PotTer,TerFor,HidFor,EnlCongMax2,EnlCongMin2,CongesMax2,CongesMin2,RSdCongesMax2,RSdCongesMin2,Region1CongArribaMax2,Region1CongAbajoMax2/;
MODEL LmpSF     /Operacion,EcBalance, EcEnergia, FlujoSF ,PotTer,TerFor,HidFor,EnlCongMax,EnlCongMin,CongesMax,CongesMin,RSdCongesMax,RSdCongesMin,Region1CongArribaMax,Region1CongAbajoMax/;

*DCIter.OptFile=1;

**************************************************************************************************
*  Flujo DC con p�rdidas iterativas: aproximaci�n para Flujo DC p�rdidas cuadr�ticas
**************************************************************************************************
    RacP.up(i)=inf;   RacP.lo(i)=0;   Falla(i)=0;

**** NUEVO PR-07
    DRac.up(i)=inf;   DRac.lo(i)=0;   Demf(i)=0;
****
...
display LossLF,F0,Ang.l;
    ...
    display  RacP.l,LossLFx.l,DRac.l;
***********************************************************************************
*  Flujo metodo DC con perdidas cuadr�ticas
***********************************************************************************
     Solve DCDirec using nlp minimizing costo;
     ABORT$(DCDirec.modelstat <> 2) 'ERROR FASE2: SOLUCION NO FACTIBLE)';

**** NUEVO PR-07
     display  RacP.l,LossLFx.l,DRac.l,Balance2.m;

*   Eliminar generacion por holgura en caso hubiera
*    Falla[i]=RacP.l[i]*1.001; RacP.fx(i)=0; FallaTotal=sum{(i),Falla[i]};

*   Eliminar generacion por holgura en caso hubiera. Se incluye e-3 al par�metro falla, en caso el error fuera menor a e-3.
...
**** NUEVO PR-07
*   Aumentar demanda en caso . Se incluye e-3 al par�metro Demf, en caso el error fuera menor a e-3.
    Demf[i]=DRac.l[i]*1.01;Demf1[i]=DRac.l[i]*1.01;Demf2[i]=Demf1[i]-DRac.l[i];DRac.fx(i)=0; DRacTotal=sum{(i),Demf1[i]};
    loop(i$((Demf2[i] lt 0.001)and(Demf2[i]gt 0)),Demf[i]=(Demf1[i]+0.01));
    loop(i$(Demf2[i] ge 0.001),Demf[i]=Demf1[i]);
****
...
*    DCDirec.optfile=1;
    ...
    if ((DRacTotal gt 0),  SOLVE DCDirec USING nlp MINIMIZING costo;);
...
display Falla,Demf,RacP.l,LossTotalL;

***********************************************************************************
*  Flujo metodo Shift Factors
***********************************************************************************
*   Eliminar generacion por holgura en caso hubiera
    Dem(i)=Demanda(i,'Pc')+PerdT(i) - Falla[i]+Demf[i];
...
**********************Region de seguridad**********************
    SFrd(rd,i)=SFrdx(rd,i)-sum{(j),W(j)*(SFrdx[rd,j])};
...
***********************************************************************************
   congestion(l)=0; congestionc(c)=0; congestionrd(rd)=0; congestionra(ra)=0; congestionrb(rb)=0;
   ...
   loop(rd$(CONGMin(rd,'Cong')), congestionrd(rd)=1$(RSdCongesMax.m(rd) - RSdCongesMin.m(rd))<>0);
   ...
   CMGcongestion(i)= sum{ (l)$(FData(l,'Cong'))  , (EnlCongMax.m(l)- EnlCongMin.m(l))*(SF [l,i]- sum{j,D(j)*(SF [l,j])}) }
                   ...
                   + sum{ (rd)$(CONGMin(rd,'Cong')), (RSdCongesMax.m(rd) - RSdCongesMin.m(rd))*(SFrd[rd,i]- sum{j,D(j)*(SFrd[rd,j])}) }
   ...
   display CMGsf,CMGenergia,CMGcongestion,Falla,Demf,RacP.l;
***********************************************************************************
...
********************************************************************************************************************
********************************************************************************************************************
put /'Congestion Region de Seguridad (vert)'/;
put '------------------'/;
put 'Barras 1','Limite','Envio','Recepcion','Congestion'/;
loop(rd$(CONGMIN(rd,'Cong')),put rd.te(rd), CONGMIN(rd,'Pmin'):8:3;
          put (sum{l$(CONGRuta_RS_D(rd,l) ),  F.l[l]+0.5*LossLFx.l[l]+0.5*LossTF[l]}):8:3;
          put (sum{l$(CONGRuta_RS_D(rd,l) ),  F.l[l]-0.5*LossLFx.l[l]-0.5*LossTF[l]}):8:3
          put congestionrd(rd)/);
********************************************************************************************************************
...
loop( h$(not(PhData(h,'Forzada'))),put h.te(h),
       ...
       Phmax(h):8:3,
       PhData(h,'CI'):8:4,
       (PhData(h,'CI')*(1+(M)*((Phdirec[h]-PhData(h,'Pgen'))/(PhData(h,'Pgen'))))):8:4,
       (sum{i$(PhBus(h,i)), cmg0(i)}):12:4,
       (PhData(h,'CI')*(1+(M)*((Ph.l[h]-PhData(h,'Pgen'))/(PhData(h,'Pgen'))))):8:4,
...
put  /'Demf'/;
put  '-------'/;
put 'Barra','Demf por barra'/;
loop( i$(Demf(i)),put i.te(i),Demf(i):8:7 /);
put ''/;

put  /'DemRacionamiento por barra'/;
put  '-------'/;
put 'Barra','DemRacionamiento por barra'/;
loop( i$(DRac.l(i)),put i.te(i),DRac.l(i):8:7 /);
put ''/;
...
*Reporte de flujo y p�rdidas finales
...
";
        ...
        {
            Logger.Info("Gams_v2 (CopiarDirectorio):\t" + destino);
            ...
            foreach (string archivo in Directory.GetFiles(origen))
            {
                ...
                Logger.Info("Gams_v2 (Directory.GetFiles):\t" + archivo + " | " + nombreArchivo);
                File.Copy(archivo, destinoArchivo, true); // true para sobrescribir
            }

            // Copiar subdirectorios recursivamente
            foreach (string subdirectorio in Directory.GetDirectories(origen))
            {
                string nombreSubdirectorio = Path.GetFileName(subdirectorio);
                string destinoSubdirectorio = Path.Combine(destino, nombreSubdirectorio);
                Logger.Info("Gams_v2 (Directory.GetDirectories):\t" + destino + " | " + nombreSubdirectorio);
