...
{
    public class FileHelperTnaV2
    {
        private static readonly ILog Logger = LogManager.GetLogger(typeof(FileHelperTnaV2));
        ...
        public static bool ObtenerArchivoRaw(string filePsse, string pathTrabajo, string pathRaiz, DateTime fechaProceso, int indicadorPSSE, string fileReproceso)
        {
            ...
            Logger.Info("ObtenerArchivoRaw - pathExportacion:\t" + pathExportacion);
            Logger.Info("ObtenerArchivoRaw - pathRespaldoExportacion:\t" + pathRespaldoExportacion);
            Logger.Info("ObtenerArchivoRaw - indicadorPSSE:\t" + indicadorPSSE);
            ...
            if (indicadorPSSE == 0)
            {
                ...
                List<FileData> list = FileServerScada.ObtenerArchivosPorCriterio(string.Empty, ConstantesCortoPlazo.ExtensionRaw, fechaProceso.AddMinutes(-30), fechaProceso, pathExportacion);

                if (list.Count > 0)
                {
                    string file = list[0].FileName;
                    Logger.Info("ObtenerArchivoRaw - pathTrabajo:\t" + pathTrabajo);
                    Logger.Info("ObtenerArchivoRaw - file:\t" + file);
                    Logger.Info("ObtenerArchivoRaw - pathRaiz:\t" + pathRaiz);
                    Logger.Info("ObtenerArchivoRaw - filePsse:\t" + filePsse);
                    //- Copiando y renombrando los archivos utilizados
                    int rs =  FileServerScada.CopiarFileAlterFsToLocalDisk(pathExportacion, pathTrabajo, file, pathRaiz);

                    Logger.Info("rs - rs:\t" + rs);
                ...
                if (list.Count == 0)
                {
                    Logger.Info("Se consulta al directorio de respaldo al no obtener .raw's del directorio principal.");
                    list = FileServerScada.ObtenerArchivosPorCriterio(string.Empty, ConstantesCortoPlazo.ExtensionRaw, fechaProceso.AddMinutes(-30), fechaProceso, pathRespaldoExportacion);

                    if (list.Count > 0)
                    {
                        string file = list[0].FileName;
                        //- Copiando y renombrando los archivos utilizados
                        FileServerScada.CopiarFileAlterFsToLocalDisk(pathRespaldoExportacion, pathTrabajo, file, pathRaiz);
                        FileServerScada.RenameBlob(pathTrabajo, file, filePsse, pathRaiz);
            ...
            else if (indicadorPSSE == 4)
            {
                pathExportacion = pathExportacion + @"forzarva\";
                string fileName = fileReproceso;
                FileServerScada.CopiarFileAlter(pathExportacion, pathTrabajo, fileName, pathRaiz);
                FileServerScada.RenameBlob(pathTrabajo, fileName, filePsse, pathRaiz);
        ...
        public static int VerificarExistenciaRaw(DateTime fechaProceso, out string pathRuta, out string filename)
        {
            ...
            if (list.Count == 0)
            {
                list = FileServerScada.ObtenerArchivosPorCriterio(string.Empty, ConstantesCortoPlazo.ExtensionRaw, fechaProceso.AddMinutes(-10), fechaProceso, pathRespaldoExportacion);
                if (list.Count > 0)
                {
                    filename = list[0].FileName;
                    resultado = 1; //- Existe archivo raw
        ...
        public static void GenerarArchivoResultado(List<RegistroGenerado> consumo, List<PrCongestionDTO> congestion,
            List<PrCongestionDTO> congestionConjunta, List<PrCongestionDTO> congestionConjuntaFlujoMinimo, List<PrGenforzadaDTO> generacionForzada, string pathTrabajo, string fileName)
        {
            ...
            using (System.IO.StreamWriter file = FileServerScada.OpenWriterFile(fileName, pathTrabajo))
            {
                ...
                //Incio de pintado de los datos de congestión de conjunto de líneas
                file.WriteLine(ConstantesCortoPlazo.InicioDatosCongestionGrupoFlujoMinimo);
                //- Pendiente de modificar
                foreach (PrCongestionDTO itemCongestion in congestionConjuntaFlujoMinimo)
                {
                    texto =
                        itemCongestion.Flujo.ToString(ConstantesCortoPlazo.FormatoDecimal).PadRight(8, ' ') + ConstantesCortoPlazo.CaracterComma +
                        itemCongestion.ListaItems.Count + ConstantesCortoPlazo.CaracterComma;
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    if (contador >= 3)
                    {
                        ...
                        if (flag == false)
                        {
                            ...
                            {
                                str = line.Substring(0, 53) + angulo.PadLeft(9, ' ') + " " + line.Substring(63, line.Length - 63);
                                flag = true;
                            }
                        }
                    }
