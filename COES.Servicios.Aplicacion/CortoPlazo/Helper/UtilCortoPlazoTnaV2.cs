...
{
    public class UtilCortoPlazoTnaV2
    {
        /// <summary>
        ...
        public static string GenerarEntradaGams(List<Carga> listaCarga, List<NombreCodigoBarra> listaBarra, List<Shunt> listaShunt,
            ...
            List<PrCongestionDTO> listaCongestionRegionSeguridad, List<PrCongestionDTO> listaCongestionGrupoFlujoMinimo, List<NombreCodigoBarra> relacionBarra, 
            double paramM, int tipo)
        {
            double pot;//, g, b;
            ...
            AgregaLinea(ref log, "                Pgen       Pmin      Pmax        CI         Forzada      Sist      Cmgh");
            ...
            foreach (RegistroGenerado item in listaGenerador)
            {
                ...
                if (item.IndCmgh == ConstantesAppServicio.SI) cmgh = "yes";

                if (tipo == 4)
                {
                    if (cmgh == "yes") genFor = "yes";
                }
                ...
                if ((item.Tipo == "H") && (item.IndOpe == 1) && (conectado) && (item.Potencia > 0))
                {
                    AgregaLinea(ref log, "        H" + String.Format("{0:000}", item.Correlativo) +
                        ...
                        "        " + UtilCortoPlazo.FormatearCadena(cmgh, 3));
                }
            }
            ...
            #region Conjunto de Enlaces de Flujo Minimo

            AgregaLinea(ref log, "*********************************************");
            AgregaLinea(ref log, "* Congestion de varios enlaces en simultaneo - Flujo Mínimo");
            AgregaLinea(ref log, "*********************************************");
            AgregaLinea(ref log, "SET RS_CONG /");

            numgrupoLinea = 0;

            foreach (PrCongestionDTO item in listaCongestionGrupoFlujoMinimo)
            {
                numgrupoLinea++;
                string lineanomb = "       C1_MIN_" + String.Format("{0:000}", numgrupoLinea) + "    ";
                string nombreconjunto = string.Empty;
            ...
            if (numgrupoLinea == 0)
            {
                AgregaLinea(ref log, "C1_MIN_001 CONGESTION L1");
            }
            AgregaLinea(ref log, "        /;");

            AgregaLinea(ref log, "SET CONGRuta_RS_D(RS_CONG,ENL)");
            AgregaLinea(ref log, "      /");
            ...
            {
                numgrupoLinea++;
                string lincjt = "       C1_MIN_" + String.Format("{0:000}", numgrupoLinea) + ".(";
            ...
            if (numgrupoLinea == 0)
            {
                ...
                AgregaLinea(ref log, "C1_MIN_001.(E00" + esta + ")");                    //'cambio3

            }

            AgregaLinea(ref log, "      /;");

            AgregaLinea(ref log, "TABLE CONGmin(RS_CONG,*)");
   
            AgregaLinea(ref log, "                 Pmin     Cong");
            ...
            {
                numgrupoLinea++;
                string lincjt = "     C1_MIN_" + String.Format("{0:000}", numgrupoLinea) + "  " + String.Format("{0:0000.000}", item.Flujo) + "   yes";
                AgregaLinea(ref log, lincjt);
            }

            if (numgrupoLinea == 0)
            {
                AgregaLinea(ref log, "     C1_MIN_001  130        no");
            }
            ...
            AgregaLinea(ref log, " M /" + String.Format("{0:0.00}", paramM) + "/");
        ...
        public static void EnviarCorreo(DateTime fechaProceso, string mensaje, bool reproceso)
        {
            ...
            string subject = ConstantesCortoPlazo.AsuntoCorreo + " - TNA - V2.0";
        ...
        {
            string subject = string.Format(ConstantesCortoPlazo.AsuntoReprocesoMasivoV2, fechaInicio.ToString("dd/MM/yyyy"),
        ...
        {
            string subject = ConstantesCortoPlazo.AsuntoReprocesoMasivoModificadoTIEV2;
            string email = ConfigurationManager.AppSettings[ConstantesCortoPlazo.EmailNotificacionCMgN];
            string[] split = email.Split(ConstantesCortoPlazo.CaracterComa);
            List<String> emails = split.ToList();
        ...
        /// </summary>
        /// <param name="list"></param>
        public static void EnviarCorreoEjecucionReprocesoVA(List<ResultadoProcesoMasivo> list)
        {
            string subject = ConstantesCortoPlazo.AsuntoReprocesoMasivoModificadoVA2;
            string email = ConfigurationManager.AppSettings[ConstantesCortoPlazo.EmailNotificacionCMgN];
            string[] split = email.Split(ConstantesCortoPlazo.CaracterComa);
            List<String> emails = split.ToList();

            string mensaje = ObtenerCorreoReprocesoMasivoModificacionVA(list);
        ...
        /// <param name="fechaFin"></param>
        /// <returns></returns>
        public static string ObtenerCorreoReprocesoMasivoModificacionVA(List<ResultadoProcesoMasivo> list)
        {
            string texto = "";
            ...
            texto += "<strong>Por favor revise el resultado de la ejecución de reprocesos masivos por modificación VA.</strong>";
            texto += "<br /><br />";
