        ...
        private void escribirTablaCapacidadAcoplamientoDevExpress(Document document, Field field, int columnsCount, List<EprRepLimitCapDTO> list)
        {
            if (field != null)
            {
                if (list.Count > 0)
                {
                    ...
                    // --------------------------------------------------------------------------------------------
                    escribirCabeceraDevExpress(document, table, 0, 0, "EMPRESA");
                    escribirCabeceraDevExpress(document, table, 0, 1, "TENSIÓN (KV)");
                    escribirCabeceraDevExpress(document, table, 0, 2, "SUBESTACIONES (SE1-SE2)");
                    escribirCabeceraDevExpress(document, table, 0, 3, "CAPACIDAD DE TRANSMISIÓN (AMP)");
                    escribirCabeceraDevExpress(document, table, 0, 4, "CAPACIDAD DE TRANSMISION (MVA)");
                    escribirCabeceraDevExpress(document, table, 0, 5, "FACTOR LIMITANTE FINAL");
                    escribirCabeceraDevExpress(document, table, 0, 6, "OBSERVACIONES");
                    // --------------------------------------------------------------------------------------------
                    var listaCodigos = string.Join(",", list.Select(p => Convert.ToInt32(p.Equicodi)));
                    ...
                    foreach (var item in list)
                    {
                        ...
                        fila++;
                        escribirCeldaDevExpress(document, table, fila, 0, item.Empresa ?? "");
                        escribirCeldaDevExpress(document, table, fila, 1, item.Tension);
                        escribirCeldaDevExpress(document, table, fila, 2, item.Subestacion ?? "");
                        escribirCeldaDevExpress(document, table, fila, 3, item.CapacidadTransmisionA);
                        escribirCeldaDevExpress(document, table, fila, 4, item.CapacidadTransmisionMVA ?? "");
                        escribirCeldaDevExpress(document, table, fila, 5, item.FactorLimitanteFinal);
                        escribirCeldaDevExpress(document, table, fila, 6, item.Observaciones);
