...
using COES.WebService.SCOSinac.Contratos;
using COES.WebService.SCOSinac.Resultados;
using log4net;
using System;
...
namespace COES.WebService.SCOSinac.Servicios
{
    /// <summary>
    ...
    public class SCOSinacServicio : ISCOSinacServicio
    {
        ...
        /// <summary>
        /// Devuelve listado de parametros generales con su respectivo valor
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        public List<AddInParametroGeneral> ListaScoSinacGetParametrosGeneralesCV(CVDataRequest data)
        {            
            MigracionesAppServicio servMigr = new MigracionesAppServicio();
            n_parameter l_PGenerales = servMigr.ListarDataParametrosGeneralesCV(data.Repcodi);

            List<AddInParametroGeneral> lstSalida = new List<AddInParametroGeneral>();
            int fila = 2;
            int columna = 2;
            bool cambiaColumna = false;
            foreach (DictionaryEntry item in l_PGenerales)
            {
                if (fila == 9)
                {
                    fila = 2;
                ...
                AddInParametroGeneral regP = new AddInParametroGeneral();
                string val = item.Key.ToString();
                regP.AbrevGeneral = val;
                //regP.Formula = l_PGenerales.EvaluateFormula(val);
                regP.Fila = fila;
                regP.Columna = columna;

                lstSalida.Add(regP);

                AddInParametroGeneral regPV = new AddInParametroGeneral();
                //regPV.AbrevGeneral = val;
                regPV.Formula = l_PGenerales.EvaluateFormula(val);
                regPV.Fila = fila;
                regPV.Columna = columna + 1;
        ...
        /// <summary>
        /// Devuelve valores de cada celda de parametros de modos con su respectivo valor
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        public DatoModo ListaScoSinacGetParametrosModoCV(CVDataRequest data)
        {
            MigracionesAppServicio servMigr = new MigracionesAppServicio();
            DatoModo salida = new DatoModo();


            List<COES.Servicios.Aplicacion.Migraciones.Helper.AddInColModo> listaModoDesdeExcel = ObtenerFormatoListadoModoAObtener(data.LstParamModo);            
            List<COES.Servicios.Aplicacion.Migraciones.Helper.AddInColModo> listaWeb = servMigr.ListarDataParametrosModoCV(data.Repcodi, data.FlagIncluirMONoActivos, listaModoDesdeExcel, out string conceptosInvalidos);

            List<AddInColModo> lstSalida = ObtenerFormatoListadoModoAPintar(listaWeb);
            salida.FilasModo = lstSalida;
            salida.StrConceptosInvalidos = conceptosInvalidos;
